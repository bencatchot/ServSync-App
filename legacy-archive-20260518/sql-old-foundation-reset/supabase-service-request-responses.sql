-- ServSync contractor responses to routed service requests.
-- Paste this into Supabase SQL Editor after the service request routing SQL.
-- Safe to re-run.

alter table public.service_request_routes
  add column if not exists response_message text not null default '',
  add column if not exists response_next_action text not null default '',
  add column if not exists estimate_range text not null default '',
  add column if not exists responded_at timestamptz;

create or replace function public.respond_to_service_request_route(
  p_route_id uuid,
  p_response_message text,
  p_response_next_action text default '',
  p_estimate_range text default ''
)
...
begin
  if coalesce(trim(p_response_message), '') = '' then
    raise exception 'Response message is required.';
  end if;

  update public.service_request_routes
     set status = 'responded',
         response_message = trim(p_response_message),
         response_next_action = coalesce(trim(p_response_next_action), ''),
         estimate_range = coalesce(trim(p_estimate_range), ''),
         responded_at = now(),
         updated_at = now()
   where id = p_route_id
  ...
  insert into public.property_access_audit_events (
    ...
    'service_request_route_response_sent',
    jsonb_build_object(
      'request_id', v_route.request_id,
      'route_id', v_route.id,
      'next_action', v_route.response_next_action,
      'estimate_range', v_route.estimate_range
    )
  );
...
grant execute on function public.respond_to_service_request_route(uuid, text, text, text) to authenticated;
