...
- Stored PDFs from existing document/report downloads use the same native PDF adapter, with guarded concurrent fetching and stable PDF filenames. Android and physical-iPhone homeowner Documents/Home History acceptance passed; the website and non-PDF downloads retain their existing anchor behavior.
...
This is a feasibility prototype, not a store-ready app. Remaining checks are tracked in the current acceptance report. Browser-relative `/api/*` routes include ordinary Help video playback, external email delivery, guest document views, payment and Marketing operations. These need a deliberate native/server integration and CORS/cookie assessment; they must not be represented as working in the prototype. Stored-PDF opening, native rendering, retry/cancel and cleanup passed on Android and physical-iPhone homeowner paths. Each iPhone download preserves its corresponding original file bytes. The contractor finalized-report caller still needs a legitimate existing Job fixture for runtime acceptance. No global fetch interception, origin spoofing, or authentication workaround has been added. Core Supabase-backed sign-in and connected-account record screens are the intended acceptance path.

Native universal/app links, password recovery callbacks, notification providers, offline synchronization, biometric login, production signing and store submission remain follow-ups. Login persistence uses the existing Supabase client behavior. On the paired physical iPhone, existing contractor/homeowner login and role switching passed; homeowner Documents/session survived background/resume and a verified fresh-process restart. Mirroring pointer/wheel and paced keyboard checks do not establish finger touch/typing, camera permissions or VoiceOver acceptance. Those checks and physical Android acceptance remain open; see the current report for exact evidence and limits.
