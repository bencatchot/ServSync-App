-- ServSync estimate/invoice property context.
-- Paste this into the Supabase SQL Editor for the ServSync project.
--
-- Adds nullable home context to estimates and invoices, then updates the
-- existing conversion RPCs so property context follows the request -> estimate
-- -> job -> invoice workflow without filtering old records.
...
create index if not exists estimates_homeowner_home_updated_idx
  on public.estimates(homeowner_user_id, home_id, updated_at desc)
  where homeowner_user_id is not null;

create index if not exists invoices_homeowner_home_updated_idx
  on public.invoices(homeowner_user_id, home_id, updated_at desc)
  where homeowner_user_id is not null;

create index if not exists estimates_local_contact_home_updated_idx
  on public.estimates(local_contact_id, local_home_id, updated_at desc)
  where local_contact_id is not null;

create index if not exists invoices_local_contact_home_updated_idx
  on public.invoices(local_contact_id, local_home_id, updated_at desc)
  where local_contact_id is not null;
