-- ServSync Contractor-Local Customer and Property Archive/Restore v1.
-- Adds reversible lifecycle metadata, controlled lifecycle RPCs, active-only
-- directory reads, work-linked historical context, and canonical new-assignment
-- enforcement without restoring direct browser table privileges.
--
-- Apply after:
...
begin
  ...
  if to_regprocedure('public.servsync_current_contractor_profile()') is null
     ...
     or to_regprocedure('public.servsync_private_local_customer_read_context()') is null then
...
drop trigger if exists servsync_guard_local_draft_assignment on public.contractor_work_drafts;
  ...
  if tg_table_name = 'estimates' then
    select exists (
      select 1
       ...
       where launch.contractor_id = v_contractor_id
         and launch.launched_estimate_id = v_row_id
         and launch.requested_output = 'estimate'
         and launch.status = 'succeeded'
         and draft.local_contact_id = v_local_contact_id
         and draft.local_home_id is not distinct from v_local_home_id
         and draft.created_at <= v_archived_at
    ) into v_has_lineage;
    ...
    select exists (
      select 1
        ...
        join public.contractor_work_drafts draft
         ...
         and draft.contractor_id = launch.contractor_id
       where launch.contractor_id = v_contractor_id
         and launch.launched_invoice_id = v_row_id
    ...
    select exists (
      select 1
        ...
        join public.contractor_work_drafts draft
         ...
         and draft.contractor_id = launch.contractor_id
       where launch.contractor_id = v_contractor_id
         and launch.launched_job_id = v_row_id
...
begin
  ...
  return jsonb_build_object(
    'draft_count', (
      select count(*) from public.contractor_work_drafts draft
       where draft.contractor_id = v_contractor_id
         and draft.local_contact_id = p_local_contact_id
         and (p_local_home_id is null or draft.local_home_id = p_local_home_id)
...
begin
  ...
  with work_contacts as (
    select draft.local_contact_id, draft.local_home_id from public.contractor_work_drafts draft
     where draft.contractor_id = v_contractor_id and draft.local_contact_id is not null
    union
  ...
  with visible_contacts as (
    select contact.* from public.contractor_local_contacts contact
     where contact.contractor_id = v_contractor_id
       ...
       and (v_access_role <> 'viewer' or exists (
         select 1 from public.contractor_work_drafts draft where draft.contractor_id = v_contractor_id and draft.local_contact_id = contact.id
         union all select 1 from public.inspections job where job.contractor_id = v_contractor_id and job.local_contact_id = contact.id
         union all select 1 from public.estimates estimate where estimate.contractor_id = v_contractor_id and estimate.local_contact_id = contact.id
         union all select 1 from public.invoices invoice where invoice.contractor_id = v_contractor_id and invoice.local_contact_id = contact.id
       ))
  )
    ...
    from (
      ...
      left join lateral (
        ...
        where home.contractor_id = contact.contractor_id
          ...
          and home.claimed_at is null
          and (v_access_role <> 'viewer' or exists (
            select 1 from public.contractor_work_drafts draft where draft.contractor_id = v_contractor_id and draft.local_contact_id = contact.id and draft.local_home_id = home.id
            union all select 1 from public.inspections job where job.contractor_id = v_contractor_id and job.local_contact_id = contact.id and job.local_home_id = home.id
            union all select 1 from public.estimates estimate where estimate.contractor_id = v_contractor_id and estimate.local_contact_id = contact.id and estimate.local_home_id = home.id
            union all select 1 from public.invoices invoice where invoice.contractor_id = v_contractor_id and invoice.local_contact_id = contact.id and invoice.local_home_id = home.id
          ))
      ) home_rows on true
    ) rows;
  return v_result;
