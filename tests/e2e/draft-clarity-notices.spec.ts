...
test.describe('Bundle 3B-1 draft clarity notices', () => {
  ...
  test('privacy and scope boundaries remain source-enforced', () => {
    ...
    expect(app).toContain('title="Draft service plan"');
    expect(app).toContain('title="Draft report"');
    expect(app).toContain('title="Draft Home Map update"');
    expect(app).not.toMatch(/DraftNotice[\s\S]{0,160}(Sent to homeowner|Active plan|Accepted|Declined|Expired|Withdrawn|Closed for review|Report sent|Filed to Documents|Submitted for homeowner review|Approved by homeowner|Proposal revoked)/i);
