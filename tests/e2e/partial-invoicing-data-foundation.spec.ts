...
test.describe('partial invoicing data foundation', () => {
  ...
  test('creates draft invoice from completed billable items only and snapshots open backlog', async () => {
    ...
    try {
      ...
      const createInvoice = await contractorA.client.rpc('servsync_create_partial_invoice_from_job', {
        p_inspection_id: jobId,
        p_work_item_ids: [completedItem.id],
      });
      expect(createInvoice.error, 'Partial invoice RPC should create draft invoice').toBeNull();
      const invoiceId = createInvoice.data.invoice_id as string;
