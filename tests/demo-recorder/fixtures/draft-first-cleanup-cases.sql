-- Run against an isolated PostgreSQL database only.
create function pg_temp.assert(ok boolean, message text) returns void language plpgsql as $$
begin if ok is distinct from true then raise exception '%', message; end if; end $$;
select public.servsync_demo_start_run('draft_first_estimate','test','isolated','seed','draft_ready',now(),'bdytwgejqnlblhrnqxkp', '{"contractor_id":"00000000-0000-4000-8000-000000000001","homeowner_user_id":"00000000-0000-4000-8000-000000000002","home_id":"00000000-0000-4000-8000-000000000003"}') as run_id \gset
...
create function pg_temp.refuse_reset(run_id uuid) returns void language plpgsql as $$
...
select pg_temp.refuse_reset(:'run_id');
rollback; -- automatic private Estimate actor attribution is outside approved allowlist
begin;
...
delete from demo_scenario_records where table_name in ('contractor_work_draft_launches','estimates','estimate_line_items');
