...
- Tests/checks run: Dedicated SQL, gate-truth-table, parser, rendered loading/error, tenant/session race, focus/visibility refresh, homeowner/admin isolation, and App integration tests were added. Existing Shared Composer, 2C-B, and Work dashboard source-contract suites were updated only for the approved centralized exposure decision. Final typecheck, build, regression, SQL-hash, scope, and sensitive-value validation is recorded in the draft PR.
- Known risks or follow-ups: The new SQL source is not applied to Sandbox or Production, no tenant is enrolled, and all three global gates remain off. Separate approvals are required for SQL application, controlled exact-UUID enrollment, Sandbox validation, Production application, gate rollout, and external beta. Structured privacy-safe telemetry remains required before external beta.
- Backlog impact:
