...
- Summary of change: PR #320 is merged and Slice 2C-C1 is complete on `main`. Slice 2C-C2 is implemented on this branch for draft review: eligible same-session confirmed Create and explicit Retry outcomes now wait for canonical consumed adoption, then use a one-time memory-only intent and the existing side-effect-free Estimate/Job loaders to open the exact output. External, mount, list, lifecycle-only, and background recovery remain explicit-only. Manual and automatic Open share one token-owned operation; stale Back, unmount, contractor, client, target, and gate changes cancel adoption and focus. Loader or deletion failures preserve the consumed Draft and explicit fallback without relaunching or creating a key. No Draft-to-Invoice path was added.
...
- Tests/checks run: Dedicated pure and rendered C2 navigation coverage includes fresh Estimate/Job, same-key and already-consumed retry, explicit-only recovery, duplicate prevention, loader/deletion failure, shared manual Open, stale context, unmount, focus, gate-off, and no-Invoice behavior. Complete C1 and PR #319 workspace regressions, typecheck, builds, SQL hashes, protected-file checks, and documentation checks are required before draft-PR handoff.
