...
- Summary of change: Implements Contractor-Local Customer and Property Archive/Restore v1 in repository source. The additive migration (SHA-256 `6e688b24b89c9f9e2ce633978660c0e72d50737ffd1e08f0e9a52774b2428849`) adds reversible archive metadata, a private append-only lifecycle-event table, manager-only impact/archive/restore RPCs, active-only and archived-directory reads, redacted work-linked historical context, canonical assignment guards, active-subject enforcement for home-scoped inspection templates, and visit-event subject binding to existing Jobs. The property lifecycle guard preserves the existing trusted transition that claims/maps an active local property while keeping already archived or mapped properties immutable. Owner, active Admin, and active Office may manage the lifecycle; Field Technician, Viewer, inactive/removed members, homeowners, anonymous callers, and cross-tenant callers fail closed. Claimed or mapped local records remain outside this lifecycle.
...
- Sandbox correction and rollout: Independent source review found and corrected two in-scope gaps before final validation: home-scoped inspection templates and visit events now participate in the canonical assignment boundary, and the local-home lifecycle trigger preserves the existing trusted claim/mapping transition from an active property while keeping already unavailable rows immutable. The exact corrected migration (SHA-256 `6e688b24b89c9f9e2ce633978660c0e72d50737ffd1e08f0e9a52774b2428849`) was applied successfully to Sandbox project `zpzdkoaubyjtsomccxya` from `2026-08-06T01:50:50Z` through `2026-08-06T01:50:52Z`. Sandbox is temporarily ahead of main for PR #382 validation; Demo and Production remain untouched, and no environment or project configuration changed.
- Tests/checks run: TypeScript and the Production build passed. Focused customer-management/archive coverage passed 45/45, Draft/inspection/PDF/claim/multi-property regressions passed 43/43, authenticated Sandbox role/lifecycle probes passed 4/4, and exact-head Preview lifecycle coverage passed 4/4 across desktop, mobile, Field Technician, and Viewer scenarios. The Sandbox security catalog passed 25 tests with one unrelated gated recipient-session concurrency skip; all 17 deployed function bodies matched the final migration. Live preservation probes kept eight existing work records operable, rejected eight new or forged assignments, confirmed two lower-role redacted historical contexts, and completed four archive/claim plus four archive/new-Draft races without invalid state. Counts and stable-ID fingerprints returned to their pre-validation values and no lifecycle events or tagged fixtures remained. Full ESLint remains blocked by the inherited ESLint 9 `allowShortCircuit` startup incompatibility; focused changed-source lint found no new error, with the two reported constant-expression errors unchanged from the starting SHA.
- Known risks or follow-ups:
  - The migration is installed only in Sandbox for validation. PR #382 remains draft and unmerged; no Production or Demo SQL/application rollout is authorized or complete.
