...
- Summary of change: Replaced the sandbox-authenticated k6 placeholder with a guarded sandbox-only read-load foundation. The script requires an ignored local credential file, sandbox anon key, sandbox Supabase ref, read-only opt-in flag, and setup-time login before running direct Supabase REST/RPC read bundles for homeowner and contractor dashboard-style data.
...
- Known risks or follow-ups:
  ...
  - v1 reads top-level dashboard-style records and approved read-only RPCs only; storage downloads, nested line-item stress, mutating workflows, credential seeding, email/Stripe/notification side effects, and production authenticated load remain excluded.
