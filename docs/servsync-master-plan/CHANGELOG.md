...
- Summary of change: Added the contractor-side multiple-invoices frontend MVP for estimate-origin invoice creation. Saved/focused estimate records now show a linked invoice summary for all non-void invoices, including type/status/total/date/open actions plus estimate total, linked invoice total, remaining balance, and draft-inclusive copy. `Create invoice from estimate` now opens an invoice type chooser for total, deposit, progress, or final invoice drafts instead of opening the first existing linked invoice. Deposit invoice drafts now ask for a dollar amount or percentage before opening the composer, then seed one clear deposit line instead of copying every estimate line item. The existing editable invoice composer is still used with `sourceEstimate`, selected `invoice_type`, optional next `invoice_sequence`, copied estimate context where appropriate, draft-inclusive remaining summary, and a warning-only over-estimate notice. The invoice list heading was also changed from internal `First-class invoices` copy to contractor-facing `INVOICES`.
...
- Known risks or follow-ups:
  ...
  - Deposit drafts support a simple upfront dollar/percentage amount step. Progress/final amounts remain manually editable; ServSync shows warnings but does not enforce remaining balance, track invoiced estimate line items, collect payments, or manage multi-invoice schedules yet.
- Backlog impact:
