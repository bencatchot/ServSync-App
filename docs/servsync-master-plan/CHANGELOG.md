...
- Tests/checks run: Pure launch-machine/error tests and expanded rendered C1 cases cover legal and illegal reducer transitions, gate/capability boundaries, confirmation, local-new/dirty/clean save sequencing, verified removal, post-success storage failure, fresh/same-key/already-consumed outcomes, lifecycle reconciliation, malformed/transport ambiguity, remount/cross-tab recovery, active-to-consumed list authority, explicit output opening, and stale token/context ownership. Full validation results are recorded on draft PR #320.
