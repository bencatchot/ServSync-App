...
- Summary of change: Implements source-only Draft-Optional Contractor-Customer Migration Compatibility for curated environments such as Demo. Versioned read/list migration `servsync-contractor-local-customer-read-list-parity-draft-optional.sql` (SHA-256 `fb37f5717d6b269d65f2f04c52ba978be51f7bc0e729130d1d0a1678a0e27197`) and archive migration `servsync-contractor-local-customer-property-archive-restore-draft-optional.sql` (SHA-256 `cc0bb59feae28c0b58c3a8f691d1418ccebb867b911188e68ad29fb1ffa2016f`) supersede the immutable, checksum-recorded files for future alignment without altering their deployed bytes. The corrected classifier accepts either no Durable Draft relations or the canonical three-table/RPC foundation. Canonical classification verifies required columns, exact key and foreign-key relationships/actions/validation state, composite contractor matching, table ownership and ACLs, RLS and exact read policies, one supported overload for each required RPC, `postgres` ownership, `SECURITY DEFINER`, fixed `search_path=public`, JSON response type, canonical argument names/defaults, authenticated execution, and PUBLIC/anon denial. Partial, structurally drifted, or insecure foundations fail transactionally before compatibility functions or archive lifecycle DDL are installed; the archive migration independently recounts the Draft relations and invokes the canonical assertion before applying lifecycle DDL.
...
- Tests/checks run: A disposable PostgreSQL 16 harness applied the exact customer-management edit, versioned read/list, Admin/Office creation, direct-table cleanup, and versioned archive chain. Draft-absent and canonical complete-Draft installation plus repeated application passed. Twelve partial/security/constraint cases each failed cleanly before compatibility state: partial relations, incompatible column nullability, weakened tenant-match FK, wrong referenced column, unvalidated FK, unexpected RPC overload, PUBLIC RPC execution, anon RPC execution, missing authenticated RPC execution, unexpected authenticated table UPDATE, disabled RLS, and missing canonical policy. A thirteenth post-read RPC-grant drift case proved the archive migration also rejects drift before lifecycle DDL. Draft-free runtime checks covered Owner, Admin, Office, Field Technician, Viewer, inactive, removed, and cross-tenant contexts across directory scope, management/creation/archive authority, work-linked Viewer filtering, redacted summaries/history, invitation revocation, direct-table ACL cleanup, and historical isolation. Focused compatibility/customer/archive/management/creation/ACL/claim/multi-property/unified-Draft checks passed 80/80, including desktop and mobile browser harnesses; the compatibility assertions passed 7/7 within that set. TypeScript and the Production build passed, and the 26-test security catalog compiled/listed without remote execution. Focused changed-test lint passed with only the inherited invalid `@typescript-eslint/no-unused-expressions` option bypassed; full lint remains blocked at startup by the unchanged ESLint 9 `allowShortCircuit` incompatibility. `bash -n`, Markdown relative links/anchors/tables, migration hashes, historical blob identity, package-lock dry-run consistency, `git diff --check`, correction-scope and added-line credential scans passed. `shellcheck` was unavailable. The Production dependency audit retains the inherited low-severity DOMPurify advisory. Generated browser/build artifacts were removed before the final commit.
