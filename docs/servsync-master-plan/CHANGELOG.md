...
- Summary of change: Implemented Slice 2C-C1 in open, draft, unmerged PR #320 after PR #319 merged. The default-off durable path now offers explicit `Create Estimate` or `Create Job` confirmation with output-specific capability checks, canonical save-before-launch, verified browser idempotency-attempt storage, same-key uncertain retry and remount/multi-tab recovery, canonical consumed adoption, active/consumed list synchronization, and the existing explicit Open Estimate/Open Job action. The final safety correction gives lifecycle and canonical-reconciliation locks precedence over external prepared, launching, or ambiguous recovery while still allowing validated external success to begin canonical consumed reconciliation. It also keeps lifecycle contradictions, discarded/not-found state, malformed canonical data, and reload failures noneditable; makes malformed/unreadable cross-tab state fail closed; enforces reducer transitions and invariants; and verifies prepared-attempt removal by readback. C1 never auto-launches or auto-opens an output, adds no Draft-to-Invoice path, and keeps private Draft content out of recovery storage.
...
- Tests/checks run: 99 focused C1 tests and the 246-test aggregate passed. Pure launch-machine/error tests and expanded rendered C1 cases cover legal and illegal reducer transitions, lifecycle-lock precedence, gate/capability boundaries, confirmation, local-new/dirty/clean save sequencing, verified removal and output-family change, post-success storage failure, fresh/same-key/already-consumed outcomes, lifecycle reconciliation, malformed/transport ambiguity, remount/cross-tab recovery, stale save/resume rejection, Back/unmount/client/target/listener races, active-to-consumed list authority, explicit output opening, and stale token/context ownership. Typecheck, default build, gated build, SQL hashes, protected-file scope, and documentation links passed; lint retains the main-identical pre-source ESLint 9/TypeScript-plugin startup failure. Full validation results are recorded on draft PR #320.
