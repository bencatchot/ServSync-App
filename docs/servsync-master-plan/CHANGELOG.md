...
- Environment rollout: Exact migration SHA-256 `165208592583b97cbd7abe95fb3ef95ff8adff93141ea5a4038cedd1f45cae93` was applied only to Sandbox `zpzdkoaubyjtsomccxya` on 2026-08-11 from approximately `03:41:31Z` through `03:41:37Z`. Catalog/security and rollback-only accepted-Plan preparation, replay, draft editing, approval, concurrency, authorization, and cleanup checks passed. Sandbox retains zero Plans, Directions, Direction revisions, packages, content records, and status events. Demo and Production remain pending and unchanged.
- Tests/checks run: Strict TypeScript; Production build; focused Marketing Directions/planning/content/workspace Playwright at desktop and `390x844`; local PostgreSQL 16 migration, role/security, replay, concurrency, terminal-state, repeat-application, and residue validation; live Sandbox catalog/security and rollback-only validation; package-contract unit tests; backend parity and rollout-ledger validation; JSON and Bash parsing; sensitive-value and scope scans; and `git diff --check`.
- Known risks and follow-up: Direction quality remains a human editorial responsibility. Production's accepted six-item planner-v3 Plan is unchanged and has no Direction rows until the exact migration receives separate Demo-first/Production-second rollout authorization and a separately approved operational preparation. Approved-Direction-to-package creation, contractor/homeowner runtime, provider generation, publishing, scheduling, campaigns, outreach, analytics, and billing remain pending.
- Backlog impact:
  - BACKLOG FILE UPDATED: YES
  - REASON: FB-037 records the Sandbox-only Slice 5 foundation and keeps environment rollout and execution capabilities as explicit next gates.
- Master plan impact:
...
- Files changed:
  ...
  - `src/features/marketing/marketingPlanning.ts`
  - `tests/e2e/internal-marketing-planning.spec.ts`
  - `tests/sql/marketing-planner-coherence-relevance-v3-validation.sql`
  - `scripts/validation/validate-marketing-planner-coherence-relevance-v3.sh`
  - `package.json`
  - `config/backend-environment-rollouts.json`
  - `docs/marketing/ServSync_Codex_Assisted_Marketing_Runbook.md`
...
- Environment rollout: Exact migration SHA-256 `c05d5e84704d15ccc134970fd71dd297f26e936bbd4091e5a860d40a8ca2800` was applied to Sandbox `zpzdkoaubyjtsomccxya` from `2026-08-10T18:55:42.015Z` through `18:55:42.590Z`, Demo `bdytwgejqnlblhrnqxkp` from `21:07:08.739Z` through `21:07:09.034Z`, and Production `uqgtheclhxqlnjpfmheq` from `21:09:14.501Z` through `21:09:14.871Z`. Demo passed before Production was touched. Rollback-only v2 planning, omitted-version v1 compatibility, version/replay rejection, v3 ingestion, authorization, and zero-residue checks passed. Demo retained its zero-plan/content baseline. Production retained its historical v1 plan, two packages, ten content records, sixteen events, and exact Marketing/unrelated-business fingerprints.
...
- Environment rollout: Exact migration SHA-256 `c6417504384a78f7ed41da83b214702a36e71749ea4fae8ed26f99d66bf29cda` was applied to Sandbox `zpzdkoaubyjtsomccxya` from `2026-08-10T14:44:11Z` through `14:44:19Z`, Demo `bdytwgejqnlblhrnqxkp` from `15:12:41.963Z` through `15:12:42.143Z`, and Production `uqgtheclhxqlnjpfmheq` from `15:16:30.723Z` through `15:16:31.191Z`. Demo passed before Production was touched. Catalog/security, v1 replay, owner-led/recommended v2 ingestion, immutable direction brief, unsafe direction/copy and approval-injection rejection, atomicity, idempotency, draft-only status, platform-admin authority, contractor/homeowner/anonymous and direct-table denial, and rollback-only cleanup passed. Demo retains one workspace and zero packages/content/events. Production retains one workspace, historical Package #1, five drafts, and five initial draft events with exact pre/post package/content/event fingerprints. Unrelated business counts and identifier fingerprints remained exact; no provider or environment configuration changed.
...
- Known risks and follow-up: Direction quality remains human/Codex authored rather than autonomous runtime AI. Truth Pack v2 is a maintained product-truth snapshot and requires versioned review as capabilities change. Package #2 remains a separately authorized operating test. Runtime generation, approval automation, publishing, scheduling, integrations, campaigns, outreach, analytics, and contractor Business Marketing remain future work.
- Backlog impact:
  - BACKLOG FILE UPDATED: YES
...
- Environment rollout: Final migration SHA-256 `e062367a4e060820dbdf0cbe73f4d961d9d3e3891717abd46d058b3945e51ce2` corresponds to Sandbox `zpzdkoaubyjtsomccxya` after base application at `2026-08-10T00:32:57Z` and exact ingestion-function hardening at `00:40:55Z` through `00:40:56Z`. Demo `bdytwgejqnlblhrnqxkp` received the exact artifact at `2026-08-10T00:50:55Z`, followed by Production `uqgtheclhxqlnjpfmheq` at `2026-08-10T00:53:21Z`. Catalog/security, platform-admin ingestion, unauthorized denial, replay, malformed/unsafe rejection, immutable provenance, and existing approval-workflow checks passed in rollback-only transactions. All three environments retain one internal workspace and zero preparation packages, content rows, or status events; unrelated Demo and Production business fingerprints remained exact.
- Tests/checks run: Clean PostgreSQL 16 migration-over-Slice-2, role/ACL/RLS, atomicity, replay, malformed/unsafe input, recipe correspondence, append-only provenance, approval-regression, repeated-install, missing-prerequisite, and residue checks; live Sandbox, Demo, and Production catalog/security checks; anonymous PostgREST denial; rollback-only platform-admin/unauthorized validation; provider/configuration non-impact and business-data preservation checks; local Truth Pack/recipe/package/operator tests; focused Marketing Playwright on desktop and `390x844`; TypeScript; Production build; backend parity/rollout validation; JSON/Bash parsing; sensitive-value/scope scans; and `git diff --check`. Focused ESLint still fails before linting changed and unchanged TypeScript files because the inherited ESLint 9 / typescript-eslint `no-unused-expressions` versions are incompatible.
- Known risks and follow-up: The Truth Pack is a bounded maintained snapshot and requires versioned review as product claims change. Runtime AI generation, approval automation, publishing, scheduling, integrations, campaigns, outreach, analytics, and contractor Business Marketing remain future slices.
- Backlog impact:
  - BACKLOG FILE UPDATED: YES
  - REASON: FB-037 now records the all-environment Codex-assisted preparation foundation and exact remaining automation boundaries.
- Master plan impact:
  - MASTER PLAN UPDATED: YES
  - REASON: Records the provider-neutral preparation loop, human authority, and completed migration-first rollout without claiming runtime generation or publishing.
...
- Lifecycle and product boundary: Assets use active/retired lifecycle rather than destructive deletion, and history survives disconnection, claim, retirement, Trade Pack downgrade, and later catalog changes. Contractor mutation cannot replace homeowner-private notes; another contractor's identity is redacted from history projections. Property Asset identity and history are independent of Stripe, subscription, billing, and Trade Pack capability. The current homeowner Assets & Systems client intentionally remains byte-identical to `main` and continues using the legacy direct-table contract; it does not call the Sandbox-only bridge RPCs.
- Merge-readiness compatibility correction: The original draft head unconditionally changed the homeowner client to four bridge RPCs that do not exist in Production or Demo. That cutover, its expanded client types, and its rewritten UI contract test were removed before merge. Focused regression coverage now requires the legacy client path and rejects bridge RPC calls, keeping the foundation hidden and inert until a later coordinated Production/Demo migration and client transition.
- Sandbox rollout: Final corrected exact migration `servsync-property-asset-bridge.sql` (SHA-256 `31e787b0d9317a84ed93e94dee3f98af7ffba379f80b06b31a0b7eee76473d8b`) was applied only to Sandbox `zpzdkoaubyjtsomccxya` on `2026-08-08T21:52:44Z` through `21:52:47Z`, after the zero-row prior bridge was safely rolled back. Live catalog correspondence confirmed two postgres-owned forced-RLS/policy-free tables, zero browser table grants, the audited service-role ACL, five authenticated RPCs, zero PUBLIC/anon RPC execution, six bridge-added triggers, and invoker-context guards that reject direct `service_role` writes and truncation despite forged private session settings. Populated active/archived fixtures also proved that schema backfill preserves original UUIDs, room links, private notes, archive state, and timestamps in both the asset and initial historical snapshot. Transactional role, claim, disconnection, lifecycle, revision, concurrency, and service-role immutability checks rolled back to zero asset/revision rows. Sandbox retained zero capability grants and zero enabled Trade Pack work types.
- Parity evidence: Production versus Demo remains `PASS WITH INTENTIONAL DIFFERENCES` with the same 129 exact Demo additions. Production versus Sandbox remains the expected `FAIL` with 597 exact approved additions and 34 visible findings. Twelve new findings are the shared `home_assets` definition/security changes from this Sandbox-only bridge; the prior 22 unrelated Stripe, function, and policy findings remain unresolved. The exact manifest approves additions only and does not hide changed Production-supported objects.
...
- Scope boundary and follow-up: No client runtime cutover, visible Property Asset/HVAC UI, runtime Trade Section, live Draft/Job asset association, enabled HVAC content, report/PDF/history projection, capability grant, or Production/Demo SQL/configuration occurred. The guarded rollback is pre-use only; once durable history exists, corrections must be additive. The next bounded slice after this merge remains `Durable Trade Section Instances v1`.
- Backlog impact:
  - BACKLOG FILE UPDATED: YES
...
- Provider acceptance correction files:
  ...
  - `config/backend-environment-parity.json`
  - `config/backend-environment-rollouts.json`
  - active Stripe/parity/planning documentation
- Summary of change: Completed provider-backed Stripe TEST acceptance in Sandbox. The ServSync-created connected account reached Active with card and ACH capabilities, Stripe fee/loss/requirements collection, and full Dashboard access. Real direct-charge card success/failure and ACH Processing-to-Paid/Failed flows reconciled through signed Connect webhooks; duplicate delivery remained idempotent, invalid signatures failed, mixed manual/online payment totaled correctly, and a test refund reversed only the Stripe-accounted amount.
- Provider compatibility correction: Actual ACH provider events used Stripe `py_` payment record identifiers where the original database constraint accepted only `ch_` charge identifiers. Additive migration `servsync-stripe-connect-provider-payment-id-compatibility.sql` (SHA-256 `f03eb5b754132501629fc6594b78fd8c6708085f646187026489d3dd20f064df`) permits exact `ch_` or `py_` prefixes, rejects other provider-ID shapes, and refuses to replace an unvalidated or drifted prior constraint. The original foundation remains byte-identical at SHA-256 `41a6a6fcd69fa8a171d3a9477efc5513def40c3a3d3d3a05169f74f504b2a374`.
- Fee and launch evidence: The test card charge, Stripe fee, and net proceeds appeared on the contractor connected account. Provider objects had no application fee, destination, transfer data, or `on_behalf_of`; ServSync application fee remained `$0`. Production and Demo SQL/configuration were untouched, and live payment creation remains disabled.
- Tests/checks run: PostgreSQL foundation-plus-compatibility validation; 15 focused Stripe Playwright checks; 98 payment, PDF, request-free Invoice, guest Estimate response, and finalized-report regressions; Owner-authorized/Admin-Office-Field-Viewer-denied live role probe; actual signed card/ACH/refund webhook reconciliation; typecheck; NodeNext typecheck; Production build; 16 backend parity tests; rollout-ledger validation; live Production/Demo and Production/Sandbox parity; sensitive-value scan; and `git diff --check`. One unrelated mutating Job smoke remains blocked by the existing Sandbox contractor-workspace load state.
- Backlog impact:
  - BACKLOG FILE UPDATED: YES
  - REASON: FB-014 now distinguishes completed Sandbox provider acceptance from the still-prohibited live-money activation gate.
- Master plan impact:
  - MASTER PLAN UPDATED: YES
  - REASON: Records actual direct-charge, fee attribution, asynchronous ACH, webhook, mixed-accounting, and reversal evidence.
...
- Provider validation boundary at initial implementation: Provider-backed behavior was initially left fail-closed pending approved TEST configuration. The later 2026-08-08 entry records completed Sandbox provider acceptance and cleanup. Production and Demo remain intentionally deferred, and Production Pay Online remains incapable of activation through this source boundary.
...
- Three-environment rollout and preservation: After transaction and browser cleanup, Sandbox returned to Estimates 138, payment schedules 3, Deposit rows 1, linked rows 0, Invoices 83, Deposit Invoices 2, partial Invoices 0, paid Invoices 2, and offline payment records 0. The exact migration was then applied to Production `uqgtheclhxqlnjpfmheq` at `2026-08-08T01:00:40Z` through `01:00:42Z` and Demo `bdytwgejqnlblhrnqxkp` at `2026-08-08T01:01:14Z` through `01:01:15Z`. Both matched Sandbox across all five function definitions, postgres ownership, fixed `search_path=public`, intended definer/invoker posture, overloads/grants, and the postgres-owned forced-RLS/no-policy 11-column ledger with zero non-owner table/column privileges, four indexes, nine constraints, and one immutable trigger. Production remained at profiles 18, contractors 7, local contacts 3, local homes 12, Estimates 22, schedule items 12, Invoices 11, Invoice lines 31, workflow events 22, and zero offline-payment rows; Demo remained at profiles 8, contractors 3, local contacts 15, local homes 21, and zero Estimates, schedules, Invoices, Invoice lines, workflow events, or offline-payment rows. Every captured identifier/status/amount/link digest was exact.
- Production behavior and validation: A rollback-only Production fixture proved persisted fixed and percentage amounts, one unsent linked draft under repeated requests, lower-role and cross-tenant denial, idempotent partial payment, exact full-payment transition, immutable Invoice amount after Estimate revision, and unpaid-void replacement with both financial-history rows retained. The transaction rolled back and left zero tagged Estimates, Invoices, payments, or activity events. Production had zero legacy partially paid Invoices before and after rollout, so no backfill or historical balance reinterpretation occurred. Admin/Office, true concurrency, Job-state independence, request-free delivery, and full desktop/mobile behavior continue to rely on the exact deployed function/source correspondence plus completed Sandbox and exact-head Preview evidence rather than manufactured Production memberships or persistent financial fixtures.
- Validation: Disposable PostgreSQL 16 migration/reapplication, fixed/percentage/no-deposit, exact linking, real concurrent request, partial/full payment, append-only history, void/re-request, role/tenant, and unchanged Job-authority checks passed. TypeScript and the Production build passed. The related payment-schedule, PDF, Invoice, request-free delivery, and secure guest regression set passed 118/118. Exact-head Vercel Preview validation at 1440px and 390x844 proved deliberate Deposit Invoice creation, review-before-send behavior, PDF Preview/Download, Job availability, zero horizontal overflow, and zero material console/network failures. Production versus Demo returned `PASS WITH INTENTIONAL DIFFERENCES` with the same 129 approved Demo additions and no unexplained supported-schema drift. Sandbox retained 297 approved Project Collaboration additions and its prior 22 unrelated findings.
- Known risks or follow-ups: Online payment processing, Pay Now, receipts, refunds, chargebacks, automatic payouts, historical payment-ledger backfill, and broader accounting/provider reconciliation remain separate. The implementation intentionally allows Job creation regardless of deposit state. No Stripe/provider configuration or business data changed.
- Backlog impact:
  - BACKLOG FILE UPDATED: YES
  - REASON: FB-014 records the three-environment provider-neutral deposit Invoice and offline-payment foundation while keeping online payment processing and financial-provider work active.
- Master plan impact:
  - MASTER PLAN UPDATED: YES
...
- Production and Demo rollout: The exact migration was applied to Production `uqgtheclhxqlnjpfmheq` and Demo `bdytwgejqnlblhrnqxkp` on `2026-08-07` at approximately `23:19Z`. Both environments match Sandbox across all eight audited function hashes, postgres ownership, fixed `search_path=public`, intended definer/invoker posture, service-role-only response lookup/write execution, private-helper denial, the postgres-owned forced-RLS/no-policy response table, zero non-owner table/column grants, one immutable trigger, six indexes, and 12 constraints. Production versus Demo remains `PASS WITH INTENTIONAL DIFFERENCES` with 129 exact approved Demo additions, all 331 Production functions and all supported objects matching, and zero unexplained drift. Demo received schema only; Demo Presentation, flags, environment variables, provider configuration, payment safeguards, and existing scenarios were unchanged.
- Production behavior and preservation: A rollback-only Production fixture proved blank, short, and oversized change-request rejection; valid Request changes; `sent -> draft`; sanitized message/version history; optional-null Decline; `sent -> declined`; acceptance regression; mutual exclusion; idempotent replay; stale-snapshot denial; and revoked-delivery denial. The probe committed no Customer, property, Estimate, line, link, session, email-attempt, acceptance, response, notification, or workflow-event row. Production remained at profiles 18, contractors 7, memberships 3, local contacts 3, local homes 12, Estimates 22, line items 39, schedules 12, notifications 69, workflow events 22, and zero request-free Estimate delivery rows; all captured identifier/status digests were exact. Demo remained at profiles 8, contractors 3, memberships 0, local contacts 15, local homes 21, notifications 5, and zero Estimates or request-free Estimate delivery rows, with exact identifier digests. Production concurrency/race and complete role/UI evidence continue to rely on the exact deployed-source correspondence plus completed Sandbox validation; no persistent Production fixture was created.
...
- Production/Demo rollout: The same exact migration was applied to Production `uqgtheclhxqlnjpfmheq` at `2026-08-07T20:18:38Z` through `20:18:40Z` and Demo `bdytwgejqnlblhrnqxkp` at `2026-08-07T20:20:47Z` through `20:20:49Z`. All three environments match on the four tables, 16 function definitions and security metadata, two triggers, 14 indexes, 45 constraints, forced RLS/no-policy posture, narrow grants, and absent non-owner table/column privileges. Production versus Demo remains `PASS WITH INTENTIONAL DIFFERENCES` with the exact 129 approved Demo scenario additions and no unexplained supported-schema drift. Demo received schema only: no Resend credential or feature/configuration change was made, so outbound report email remains intentionally fail-closed there.
- Preservation and provider validation: Production remained at profiles 18, contractors 7, memberships 3, local contacts 3, local homes 12, inspections 9, estimates 22, invoices 11, Drafts 7, claim invitations 1, and filed report objects 0; Demo remained at profiles 8, contractors 3, memberships 0, local contacts 15, local homes 21, inspections/estimates/invoices/Drafts/filed report objects 0, and claim invitations 8. Identifier digests and non-feature catalog fingerprints were preserved. Disposable Production probes were removed and the same counts/digests returned. Vercel keeps `RESEND_API_KEY` as a sensitive Production-only value; the already-live secure Estimate endpoint exercised that runtime secret and Resend accepted the approved disposable test message. Report-specific provider acceptance remains established by the authenticated Sandbox suite until the report source deploys. Provider acceptance is evidence of handoff, not independently confirmed inbox receipt.
- Rollout status and boundary: Sandbox, Production, and Demo now carry the exact migration. Production uses the existing approved server-side Resend configuration; Demo remains provider-disabled. This slice does not add report acknowledgment, acceptance, signatures, payments, replies, SMS, broad guest history/navigation, anonymous report search, Service Plans, or delivery of draft/internal/technician-only content.
- Backlog impact:
  - BACKLOG FILE UPDATED: YES
  - REASON: FB-003B now records the completed three-environment database rollout and Production provider readiness while retaining later channels, recipient actions, and additional document classes as active work.
- Master plan impact:
  - MASTER PLAN UPDATED: YES
  - REASON: Records the filed-PDF recipient boundary, contractor role policy, three-environment rollout, evidence limits, and remaining capability work without claiming broader guest access.
...
- Production and Demo rollout: The exact migration was applied to Production `uqgtheclhxqlnjpfmheq` at `2026-08-07T18:39:48Z` through `18:39:50Z` and Demo `bdytwgejqnlblhrnqxkp` at `2026-08-07T18:40:29Z` through `18:40:31Z`. Both environments match Sandbox across the four audited function hashes, one overload per function, postgres ownership, fixed `search_path=public`, intended definer/invoker state, authenticated-only preparation, service-role-only result recording, private-helper denial, the postgres-only forced-RLS attempt table, zero policies, the immutable trigger, and four expected indexes. Existing contractor-local ACL and policy fingerprints were unchanged. Demo received no provider credential and remains intentionally fail-closed for outbound email.
- Production provider validation: The approved existing Resend credential was added only to the Production Vercel project as a sensitive server-side `RESEND_API_KEY`; no `VITE_` variable, Demo secret, provider account, domain, billing, or unrelated configuration changed. A clearly tagged disposable contractor Owner created one Customer/Estimate, established prior recipient access, sent through the exact server handler to the approved test recipient, observed sanitized `Sent` history, preserved the Customer profile email, replaced the prior bearer/session, and revoked the replacement link. Resend accepted the request; the send-only credential cannot query or independently confirm inbox receipt. The recipient snapshot remained document-only with no private Customer/property fields or broader record access.
- Preservation and cleanup: Sandbox profiles 19, contractors 6, memberships 5, local contacts 299, local homes 320, and Estimates 138 plus all captured identifier digests matched the pre-application baseline. Production returned to profiles 18, contractors 7, memberships 3, local contacts 3, local homes 12, Estimates 22, and line items 39 with its captured identifier digests unchanged. Demo retained profiles 8, memberships 0, local contacts 15, local homes 21, and zero Estimates/line items. All temporary Production and Sandbox users, contractors, customers, properties, Estimates, line items, links, sessions, and email attempts were removed; Production and Demo final links, sessions, and attempts were zero. No Customer profile identity, existing business data, feature flag, entitlement, membership, or non-email configuration changed.
- Remaining boundary: No fake-success fallback is included. SMS, finalized-report delivery, anonymous PDF download, Estimate acceptance/acknowledgment, signatures, payments, replies, Service Plans, and broader guest access remain separate work. Connected Customer authenticated delivery/approval and request-free Invoice delivery remain unchanged.
- Backlog impact:
  - BACKLOG FILE UPDATED: YES
  - REASON: FB-003B now records Production-active direct Estimate email delivery and Demo schema parity while retaining the remaining channels, documents, and guest actions as active work.
- Master plan impact:
  - MASTER PLAN UPDATED: YES
...
- Sandbox rollout and validation: Exact migration `servsync-request-free-local-estimate-delivery.sql` (SHA-256 `016a532e5b0db0ea48966a04091a741806406e1d32601b11885474547c2b29ad`) was applied transactionally to Sandbox `zpzdkoaubyjtsomccxya` after the `2026-08-07T14:59:18Z` baseline. All 13 functions and three private tables matched the intended ownership, `SECURITY DEFINER`/invoker, fixed `search_path=public`, overload, grant, forced-RLS, ACL, and trigger posture. Authenticated Owner/Admin/Office management passed; Field Technician, Viewer, inactive, cross-tenant, homeowner, and anonymous callers were denied. Direct Owner and anonymous table reads were denied 3/3 each. Recipient bootstrap/session, snapshot allowlist, rotation, revocation, and session invalidation passed. Contacts 299, homes 320, Estimates 138, line items 317, schedules 3, claim invitations 13, and all captured identifier digests were restored exactly; delivery rows and tagged fixtures returned to zero.
- Production and Demo rollout: The same exact migration was applied transactionally to Production `uqgtheclhxqlnjpfmheq` and Demo `bdytwgejqnlblhrnqxkp` on 2026-08-07 before source merge. Each environment matched Sandbox across all 13 function definitions, one overload per function, postgres ownership, fixed `search_path=public`, authenticated management grants, service-role gateway grants, private helper denial, three private forced-RLS tables, and the validation trigger. Production preserved contacts 3, homes 12, Estimates 22, line items 39, schedules 12, claim invitations 1, and all captured identifier digests; Demo preserved contacts 15, homes 21, Estimates/line items/schedules 0, claim invitations 8, and its captured identifiers. Both environments began with and retained zero delivery links, sessions, and rate buckets. No existing Customer, property, Estimate, claim, membership, feature flag, environment variable, or configuration changed. Demo received schema parity only and remains fail-closed if its existing server-side gateway configuration does not support the route.
- Validation: TypeScript and the Production build passed. The focused Estimate delivery suite passed 11/11, and combined Estimate plus established request-free Invoice delivery coverage passed 67/67. Disposable PostgreSQL 16 install and functional checks passed, including immutable snapshot content, sent-state publication, role denial, one-active-link rotation, and no legacy private line detail. Exact-head Preview recipient rendering passed at desktop and 390x844 mobile with immediate token clearing, no overflow, no console errors, and no application API/asset failures; normal PR checks passed. The pre-rollout Preview token lookup correctly failed closed before its configured backend received the migration.
- Scope boundary: This slice provides contractor-controlled secure Estimate delivery for Not connected Customers. It does not add Estimate acceptance, acknowledgment, signatures, payments, email/SMS providers, anonymous PDF download, report delivery, a guest portal, Service Plan delivery, or any change to Connected Customer authentication, Home History, approvals, notifications, claims, sharing, or property authority.
- Backlog impact:
  - BACKLOG FILE UPDATED: YES
...
- Validation: A disposable PostgreSQL 16 clean-install-prefix harness applied the real historical chain through the formerly overriding attribution migration and then the canonical reconciliation twice. It passed normal homeowner/contractor and Supabase Auth-role creation, safe fallback for platform-admin/team/unknown/malformed/app metadata, existing administrator preservation, same-user and cross-user profile escalation denial, safe missing-profile recovery, ordinary profile updates, universal/one-time/permanent referral attribution, missing-prerequisite rollback, ownership/security/search-path/grant/overload/trigger posture, and idempotence. Independent Sandbox validation found that the original permanent-referral branch used an unqualified `gen_random_bytes` call that was unavailable under fixed `search_path=public` in Supabase's extension layout. The failed account creation rolled back completely. The corrected trigger uses `pg_catalog.gen_random_uuid()` to preserve the 12-character uppercase invite-code contract, and the harness now installs pgcrypto in the Supabase-style `extensions` schema. The corrected migration was reapplied successfully at `2026-08-07T08:41:46Z` through `08:41:47Z`; permanent, one-time, and universal referrals, authenticated self-update/upsert denial, cross-user denial, direct invocation denial, role fallback, existing-admin preservation, catalog correspondence, and cleanup passed. Focused Playwright source/catalog checks passed 6/6 and pin historical migration hashes plus installer finality; Backend Parity Guard tests passed 16/16; TypeScript, Production build, focused changed-test lint, shell syntax, rollout-ledger rendering, `git diff --check`, Markdown consistency, and sensitive-value checks passed. Full lint remains blocked by the inherited ESLint 9 `allowShortCircuit` startup incompatibility.
- Rollout status: Sandbox `zpzdkoaubyjtsomccxya` retains the exact corrected migration and is temporarily ahead of main for PR #387 validation. Production and Demo were untouched and remain `Pending`; no Vercel configuration or deployment changed. The next controlled operation must apply and verify the same exact migration in Production and Demo, reverify the unchanged PR, and then proceed through normal merge/deployment. Neither vulnerable environment should remain intentionally behind once that rollout is authorized.
- Backlog impact:
  - BACKLOG FILE UPDATED: YES
  - REASON: FB-016 now tracks the Sandbox-validated signup security correction as the priority pending coordinated Production/Demo rollout while preserving unrelated Sandbox parity work separately.
- Master plan impact:
  - MASTER PLAN UPDATED: YES
  - REASON: Records the canonical self-service role policy, profile-role immutability boundary, referral preservation, completed Sandbox validation, and remaining coordinated Production/Demo rollout gates.
...
- Files changed:
  - `servsync-demo-production-schema-parity-reconciliation.sql`
  - `tests/e2e/demo-production-schema-parity-reconciliation.spec.ts`
  - `tests/e2e/security-catalog.spec.ts`
  - `docs/demo/validation/Demo_Production_Schema_Parity_2026-08-06.md`
  - `docs/servsync-master-plan/CHANGELOG.md`
  - `docs/servsync-master-plan/ServSync_Feature_Backlog.md`
  - `docs/servsync-master-plan/ServSync_Master_Plan_v1_0.md`
- Summary of change: Brought the curated Demo Supabase backend `bdytwgejqnlblhrnqxkp` to current Production-supported schema compatibility without changing Demo Presentation, Vercel settings, rollout settings, feature flags, entitlements, email/payment safeguards, or curated scenario data. The ordered repository migration chain installed the missing Durable Draft, Partial Invoicing/data, customer-management, request-free delivery data, integration, document-upload, referral/review, scheduling, price-book import, and related workflow foundations. Project Collaboration was not installed because `public.projects` is absent in both Production and Demo.
- Reconciliation: Added the Demo-target-guarded migration `servsync-demo-production-schema-parity-reconciliation.sql` (SHA-256 `6e2641f94c96075b20b3a27ff483c84541a06c83c3ddd4edb805c8675d9d701f`). It requires the private Demo scenario foundation and canonical Durable Draft tables, rejects missing or overloaded appointment RPCs, aligns exactly four legacy appointment RPC definitions/owners/security/search paths/grants with the read-only Production authority, and removes three superseded Draft-optional classifier helpers. It creates no table, policy, trigger, Project Collaboration object, or business record.
- Catalog result: Every Production relation, column, constraint, index, trigger, policy, function, type, extension, and default ACL is now represented with the same definition in Demo. Demo's only catalog additions are its three private scenario registry tables, six trusted scenario operator functions, and their supporting columns/constraints/indexes/triggers. Production-owned policy and function comparison has zero missing or differing definitions; no Project Collaboration object exists. The complete ordered migration/checksum ledger and validation matrix are recorded in [Demo Production Schema Parity Validation](../demo/validation/Demo_Production_Schema_Parity_2026-08-06.md).
- Runtime and role validation: Disposable authenticated Demo probes passed Owner/Admin/Office creation, profile/property management, archive/restore, directory, and invitation behavior; Field Technician, Viewer, inactive, removed, homeowner, anonymous, unauthenticated, and cross-tenant cases failed closed or received only their intended redacted/work-linked result. Direct local-contact/home table reads were denied across browser roles, pending invitations were revoked without revival, and request-free/Durable Draft/price-book foundations were present. The controlled result totals were helper matrix 9, creation success 3, creation denial 6, management success 6, management denial 12, archive/restore success 12, archive denial 12, direct ACL denial 20, directory checks 5, invitation checks 2, and Draft backend checks 2.
- Preservation and browser validation: All temporary auth users, memberships, customer/property/invitation/archive fixtures, credentials, and generated evidence were removed. Existing counts and identifier digests remained unchanged, including 15 local contacts, 21 local homes, 8 claim invitations, 12 invitation-home mappings, 8 auth users, 6 storage buckets, 1 Demo scenario, 71 runs, and 3 scenario records. Current-main Demo browser validation passed contractor Customers at `1440x900` and `390x844` plus homeowner Properties at `1440x900`, with no horizontal overflow, console errors, or server failures.
- Intentional environment differences: Demo Presentation remains enabled. Durable Draft schema/RPC support is installed, but the server rollout setting remains disabled and the three Draft UI gates remain false/absent, so no visible Draft rollout is claimed. Request-free invoice-delivery database support is installed but remains operationally fail-closed because no Demo server credential or Firewall configuration was authorized. Project Collaboration remains absent. These are configuration/product-rollout differences, not unexplained schema drift; full visible or operational Demo parity is not claimed.
- Tests/checks run: Exact Production/Demo catalog comparison; migration checksum and scope checks; authenticated role/security/workflow probes; desktop/mobile browser validation without retained screenshots/traces/videos; preservation counts and digests; 145/145 focused source/rendered regression checks; TypeScript; Production build; and the linked Demo security catalog at 26 passed with one separately gated concurrency skip. The security catalog now treats Project Collaboration as either fully absent or fully installed with RLS and conditionally requires its RPCs/assignment trigger, so the Production-authoritative project-absent shape is tested without accepting partial installation. Markdown link/anchor/table checks, `git diff --check`, package audit, sensitive-value scan, generated-artifact cleanup, and final clean-worktree verification also ran. Full lint reproduced the inherited ESLint 9 `allowShortCircuit` startup incompatibility; focused changed-test lint passed with only that invalid inherited rule bypassed. Production dependency audit retains one inherited low-severity DOMPurify advisory and no package file changed.
- Backlog impact:
  - BACKLOG FILE UPDATED: YES
  - REASON: Records current Demo backend schema compatibility while keeping visible Durable Draft rollout, request-free server configuration, Project Collaboration, and full Demo parity as separate decisions.
- Master plan impact:
  - MASTER PLAN UPDATED: YES
  - REASON: Distinguishes backend schema parity from unchanged Demo presentation, rollout, entitlement, and server-configuration boundaries.
...
- Tests/checks run: A disposable PostgreSQL 16 harness applied the exact customer-management edit, versioned read/list, Admin/Office creation, direct-table cleanup, and versioned archive chain. Draft-absent and genuine canonical complete-Draft installation plus repeated application passed. Thirty-eight independent partial/structural/security/grant drift cases failed cleanly before compatibility state, covering missing/type/null/default/check drift, FK shape/reference/action/validation drift, RPC overload/argument/return/body/security/search-path/owner drift, PUBLIC/anon/missing-auth or missing-service execution, authenticated and service-role grant options, missing or expanded service-role table access, unexpected grantees, table and column ACL/grant-option drift, RLS, and missing/command/expression policy drift. A post-read grant drift case separately proved the archive migration rejects a changed shared contract before lifecycle DDL. The real-chain canonical fixture and an independent Supabase-style ACL scenario both accepted only authenticated `SELECT`/RPC execution and service-role CRUD/RPC execution without non-owner grant options. Draft-free runtime checks retained Owner, Admin, Office, Field Technician, Viewer, inactive, removed, and cross-tenant coverage across directory scope, management/creation/archive authority, work-linked Viewer filtering, redaction, invitation revocation, direct-table ACL cleanup, and historical isolation. The focused compatibility assertions passed 7/7, the full disposable PostgreSQL harness passed absent/canonical/reapply plus 38 fail-closed drift cases, TypeScript passed, and the Production build passed. The linked Demo security catalog passed 11, failed 14 for separately tracked missing Demo foundations, and skipped one gated test; the customer ACL/claim checks passed, while the generic archive catalog expected Draft/Project triggers that are intentionally absent in this supported subset. Full lint remains blocked at startup by the unchanged ESLint 9 `allowShortCircuit` incompatibility.
- Demo migration application: The exact reviewed six-step chain was applied to Demo Supabase `bdytwgejqnlblhrnqxkp` on `2026-08-06`: local-property edit (SHA-256 `26c8c9bef284e1e7e98fe829220c1c112d5b32487cdace1768cb15bc0785872f`), customer-management edit boundary (`b86b3af7524d3d0122f97e21609569dfa316b7f87d2d2df213c5e73478d66427`), Draft-optional read/list (`5f172d7915f74e8738cd9dbf78f640d681f125770eeb488eb837e3905572f622`), Admin/Office creation (`ce3fc2b8e79bab14790c27577e66d2e7d6c0a27cd4c1cf1ffe8856877531b78b`), direct-table privilege cleanup (`c01332b1422c136cdb6fed5d580c7deacf5cf49ac1debfaf02be06f7e3a5cf64`), and corrected Draft-optional archive (`2994161c0096787001b13fe7224d7ee75aa2491429e4be98f712e0a60838002b`). The first archive attempt failed transactionally on the previously unmodeled Supabase helper ACL and left no archive residue; the corrected exact migration applied successfully from `19:39:00Z` through `19:39:03Z`.
- Demo validation and security: Authenticated Demo probes passed seven grouped customer-backend checks covering role-shaped directory access, transactional creation, profile/property management, archive/restore role boundaries, invitation revocation with empty Draft results, claimed-record protection, and direct browser ACL denial. Owner, Admin, and Office management paths succeeded; Field Technician, Viewer, inactive, removed, homeowner, anonymous, unauthenticated, and cross-tenant paths failed closed or received the intended redacted/work-linked result. `PUBLIC`, `anon`, and `authenticated` retain no direct table or column privilege on the two contractor-local tables; `service_role` retains trusted CRUD only. Sixteen Draft-free/project-free archive functions, ten applicable non-Draft/non-project assignment triggers, six archive indexes, private forced-RLS lifecycle events, and stable zero/empty Draft and Project results were verified. No Durable Draft or Project object was created.
- Demo preservation and UI: Temporary users, memberships, customers, homes, invitations, and lifecycle events were removed. Final counts and identifier digests exactly matched the pre-application baseline: 15 local contacts, 21 local homes, 8 claim invitations, 12 invitation-home mappings, 2 homeowner connections, 1 Demo scenario, 71 scenario runs, and 3 scenario records. Current-source Customers behavior passed at desktop `1440x1000` and mobile `390x844` with no overflow, page errors, request failures, or server failures. Demo Presentation remains enabled, customer creation stays suppressed by presentation mode, and Durable Draft gates remain false/absent. One inherited `contractor_price_book_items` HTTP 400 occurred per viewport and remains part of the broader missing-Demo-backend inventory rather than this customer compatibility slice.
- Environment and rollout boundary: Demo now has the current contractor-local customer-management backend as a curated supported subset. Durable Draft, Partial Invoicing, Project Collaboration, and broader missing RPC groups remain intentionally absent; full Demo parity is not claimed. No feature flag, rollout mode, entitlement, environment variable, or Vercel configuration changed. Production and Sandbox were not targeted or changed by this Demo operation.
- Backlog impact:
  - BACKLOG FILE UPDATED: YES
  - REASON: Records the completed Draft-free Demo customer backend alignment while keeping Durable Draft, Partial Invoicing, Project Collaboration, and broader Demo parity separate.
- Master plan impact:
  - MASTER PLAN UPDATED: YES
