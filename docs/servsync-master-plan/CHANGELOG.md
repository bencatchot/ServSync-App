...
- Tests/checks run:
  - Focused executable adapter tests cover exact RPC calls, structured errors, connected/local/consumed/deleted-output mappings, item identity and removal, non-positional reconciliation, malformed response rejection, throwing storage, validated idempotency retention/cleanup, hidden scope, and SQL hash integrity.
