...
The repository migration is `servsync-stripe-connect-online-payments-foundation.sql` (SHA-256 `41a6a6fcd69fa8a171d3a9477efc5513def40c3a3d3d3a05169f74f504b2a374`). It is applied to Sandbox with catalog/security, rollback-only accounting, event-ordering, and preservation validation complete. Real Stripe test-mode onboarding and hosted Checkout remain fail-closed until an approved existing test key and Connect webhook secret are configured. Production and Demo schema rollout requires separate authorization. Live payment activation requires a later owner decision covering live credentials, connected-account onboarding, operational support, receipts, refunds, disputes, chargebacks, and launch monitoring.
