...
Marketing Direction Slice 3A uses one additive compatibility migration because the deployed ingestion RPC intentionally pinned Truth Pack v1. Exact SHA-256 `c6417504384a78f7ed41da83b214702a36e71749ea4fae8ed26f99d66bf29cda` is applied only in Sandbox. It permits historical v1 replay plus v2 ingestion, applies the new competitor-framing guard only to v2 direction/copy, and retains the same platform-admin-only RPC, tables, transaction, roles, and grants. Demo and Production remain pending, so the Slice 3A PR must not merge until a separately authorized migration-first rollout is complete. No runtime generation UI or new authorization surface is included.
