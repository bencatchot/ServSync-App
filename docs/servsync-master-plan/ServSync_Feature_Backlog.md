...
- Admin/Office Customer Creation Parity v1 is implemented in repository source on `codex/admin-office-customer-creation-parity-v1`, but remains unapplied, undeployed, and not live. The additive `servsync-admin-office-customer-creation-parity.sql` migration (SHA-256 `ce3fc2b8e79bab14790c27577e66d2e7d6c0a27cd4c1cf1ffe8856877531b78b`) derives tenant identity server-side, reuses the established customer-management policy, atomically creates a local customer and initial property for Owner/Admin/Office, denies every lower or inactive role, accepts no caller-selected contractor/parent identifier, and returns an explicit bounded DTO. All normal creation entry points share the same UI policy. Independent source review and separately authorized Sandbox SQL/catalog/role/Preview validation remain required before merge or rollout. Archive/restore, direct-table privilege cleanup, Demo disposition, broader customer-model maturity, the non-owner appointment-window 401, and ESLint 9 repair remain active; full customer-management maturity is not complete.
