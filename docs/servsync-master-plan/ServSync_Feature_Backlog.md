...
- Bundle 3 — Empty States & Draft Clarity is split into Slice 3A Empty State Foundation + Primary No-Data Surfaces, Slice 3B Draft Clarity Notices, and Slice 3C Search, Filter & Permission Empty States. Slice 3A adds a shared reusable empty-state component with standard and compact variants, migrates primary homeowner and contractor true no-data surfaces, and keeps loading, error, search/filter no-results, permission messaging, draft notice migration, workflow actions, routing, SQL/RLS/RPCs, Supabase/Vercel settings, and infrastructure behavior unchanged. Bundle 3 remains incomplete: draft clarity belongs to 3B and search/filter/permission empty-state polish belongs to 3C.
