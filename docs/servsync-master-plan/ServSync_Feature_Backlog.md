...
| FB-035 | Unified Start New Job Workflow | Estimates, jobs, invoices, job scope, approvals, billing history | Cohort Gate Implemented In Source / Rollout Pending | High | PRs #317-#321 are merged and Slice 2C-C is complete on `main`. Production has the corrected canonical durable Draft foundation installed and verified, its durable Draft tables remain empty, and the three global Draft/Work gates remain absent/off. The cohort-gating source adds a dedicated default-false tenant entitlement, narrow owner/active-team-member RPC, neutral fail-closed frontend loading, exact tenant/session invalidation, and legacy Jobs fallback. It is not applied; no contractor is enrolled. Pending gates are SQL review/application, Sandbox matrix validation, explicit exact-UUID tenant approval/enrollment, Production application, global-gate rollout, and structured privacy-safe telemetry before external beta. No Draft-to-Invoice path exists. |
