...
| FB-020 | Security, Records Reliability, Backup/Restore, and Scale Readiness | Security, RLS, storage, records, operations, performance | Controlled Beta Baseline Met / Follow-up Backlog | High | Controlled private beta baseline is met through public smoke coverage, required authenticated production read-only smoke, no-mutation smoke boundaries, backup/restore templates, artifact guardrails, restore-drill planning, and a first non-production schema restore drill. Controlled Operations evidence is documented through Slice 3 with a local/provider-neutral foundation complete through Slice 2D-A: operation packets, command/browser/provider-neutral evidence, browser-attempt lineage/reconciliation, freeze/manifest/seal verification, expected external digest verification, local fake provider, and local external-anchor custody reference behavior. FB-020 is not fully complete; optional role/stable smoke records, full data/auth restore, storage restore verification, backup/PITR verification, live provider adapters, public go-live readiness, paid-subscription readiness, and broader readiness remain follow-up backlog. |
...
| FB-035 | Unified Start New Job Workflow | Estimates, jobs, invoices, job scope, approvals, billing history | Internal Cohort / App Wiring Source Draft / Entitlement Hardening Pending SQL | High | Durable Draft-first Work remains limited to entitled contractor cohorts behind global gates. Draft-to-Estimate, Draft-to-Job, Inspection Checklist, and Draft-to-Invoice have passed controlled internal/Sandbox validation paths, and source app wiring now exists behind gates, durable cohort entitlement, resolved contractor context, and billing authority. Sandbox role/profile testing found that non-entitled contractors remain UI-off and isolated, but direct durable Draft save/launch RPCs require server-side entitlement hardening before broader beta; branch `codex/durable-draft-rpc-entitlement-hardening` adds that source correction. SQL application/runtime validation, Sandbox role-matrix follow-up, telemetry, and external beta remain separate approval gates. |
...
- Local customer claim-invite manual delivery hardening has completed Production SQL/source/token-containment rollout. The approved boundary remains manual Copy Link / QR only for contractor owner, admin, and office roles; field tech/viewer users must not prepare or reveal raw claim tokens. Normal invite list/detail reads must stay token-free, Copy/QR must go through the guarded prepare-delivery RPC, and direct table-token access must not be restored.
- Local customer multi-property claim redesign is source/Sandbox/preview gated before Production: one invite carries an explicit contractor-selected set of unclaimed local properties, homeowners must create or match every included property to a distinct destination before atomic acceptance, single-property invites remain compatible, and older accept clients must reject multi-property invites before mutation.
- Broader operational estimate/invoice/document delivery authorization and send-RPC hardening remain separate from local customer profile editing and local claim-invite manual sharing. Do not use this slice as approval to add email/SMS/provider delivery, delivery-attempt tracking, transactional outbox behavior, or delivery after revoked connections, stale invites, unshared properties, or inconsistent contractor team-role states.
...
- Controlled Operations evidence now has an authoritative policy/runbook in `docs/CONTROLLED_OPERATIONS_RUNBOOK.md`. Slices 1, 2A, 2B, 2C-A, 2C-B, 2C-C1, 2C-C2, 2D-A, and Slice 3 documentation are complete for local/provider-neutral use. The foundation covers packet evidence, sanitized command output, browser evidence, packet-bound browser evidence, attempt lineage, interrupted-attempt reconciliation, latest authorized terminal attempt selection, workflow freeze, manifests, seals, expected external digest verification, a provider-neutral adapter contract, local fake provider behavior, and local external-anchor custody reference behavior. It does not provide live provider authority, credential resolution, Production/Sandbox/Preview/Demo/Vercel/Supabase authority, SQL/deployment authority, automatic rollback, or host-compromise nonrepudiation.

Recommended future implementation sequence:

1. Keep the controlled-operations runbook current as local/provider-neutral policy evolves; require a separate gate before any Slice 2D-B live provider adapter.
...
Status: Internal Cohort / Invoice App Wiring Source Draft
...
Product goal:
Contractors should eventually enter work scope once in a shared Draft composer, choose an intended output, and launch exactly one initial first-class workflow record. The first approved live launch outputs are Estimate and Job. The Draft-to-Invoice SQL/RPC foundation is installed and runtime validated in Production, and branch `codex/draft-to-invoice-app-wiring` adds source-only visible app wiring for draft Invoice launch behind the existing Draft-first Work gates, durable cohort entitlement, resolved contractor context, and billing authority. Outcome-specific validation should happen at the launch boundary while the form remains shared. The goal is to reduce duplicated entry and inconsistent direct-job behavior without weakening approval, billing, duplicate prevention, or homeowner visibility rules.
...
Slice 2B implements the corrected durable Draft Launch Foundation in merged PR #317. It adds dedicated `contractor_work_drafts`, `contractor_work_draft_items`, and private success-only `contractor_work_draft_launches` tables, durable nullable intended output, contractor-private notes and subject snapshots, consumed launch state, deletion-compatible private Estimate/Job linkage with immutable output-ID snapshots, stable Draft items, one-successful-launch and idempotency protection, atomic rollback, tenant authorization, relationship locking/revalidation, legacy import, Job property-sharing parity, operational contractor/service-request Job deduplication, retention, contractor-only RLS/grants, and save/get/import/launch RPCs. ServSync Sandbox has the foundation and permission correction installed; the full runtime matrix passed, including adversarial rollback, concurrency, relationship locking, PostgREST, deletion/retry, privacy, tenant isolation, RLS/grants, and the owner/admin/office/field-technician/viewer/platform-admin compatibility matrix. Production has the corrected canonical foundation installed and verified from hash `ac9e600ece3075e2d171da5571aab2b26e7a1f6f234239b02194fa7be3d2354f`; Production durable Draft tables remain empty and no authenticated Production durable workflow has run. The Job-write-only Draft-persistence defect is resolved, Estimate authority remains unchanged, and no known Slice 2B runtime blocker remains. Current permission helpers remain compatibility mechanisms rather than permanent role policy, and no universal field-technician Estimate prohibition was introduced. Production remains on legacy Jobs because all three global gates are absent/off. Sandbox previously received the earlier foundation and subsequently received validated correction hash `b4a98f33acd99083bea4497268393f6277ef330cdcb31bdc5d253adb94b14c7f`; neither file should be reapplied there, and Production must not receive the Sandbox-only correction.

The Slice 2C architecture audit produced bounded implementation stages. Slice 2C-A added hidden typed durable API contracts, pure mappings, explicit local-versus-durable item identity, structured errors, helper-derived capabilities, and privacy-safe fail-closed attempt storage; PR #318 merged. Slice 2C-B merged in PR #319 with canonical gated save/resume/list/import, active/consumed presentation, guarded existing-output opening, and token-owned async safety. Slice 2C-C is complete on `main`: C1 merged in PR #320 with explicit confirmation, save-before-launch, distinct Estimate/Job authority, verified idempotency persistence/removal, same-key retry/recovery, fail-closed lifecycle and cross-tab reconciliation, canonical consumed adoption, list synchronization, and explicit Open Output; C2 merged in PR #321 with canonical-consumed-first same-session automatic navigation for explicit Create/Retry only, memory-only eligibility, shared manual/automatic output loading, stale-context cancellation, focus transfer, and explicit fallback. Draft PR #322's cohort gate is installed and validated in Sandbox with strict canonical UUID parsing and exact-context in-flight refresh coalescing. The controlled staged matrix passed and was fully rolled back: no Sandbox tenant remains enrolled, branch gates are off, and Production remains unchanged. Production cohort SQL/application, exact-UUID Production enrollment, global-gate rollout, structured privacy-safe telemetry, and external beta remain separately approval-gated.

Draft branch `codex/draft-inspection-checklist-job-path-v1` adds the hidden source-only Inspection Checklist work-format path for durable Drafts. It snapshots eligible starter, contractor-owned, and Home-specific Inspection Checklists into contractor-private Draft metadata, saves/resumes them through checklist-specific durable Draft contracts, and can launch only one Job with checklist/report structure once its additive SQL source is separately reviewed, approved, and applied. Remediation tightens the hidden source path so Home-specific checklists bind to the exact selected property, starter snapshots match the canonical catalog, launched checklist prompts begin as `Not Recorded` instead of pre-answered or substantive findings, and duplicate room/item structures are rejected. The SQL is not applied by the implementation task, no Draft/Work gates are enabled, no operational report is created before launch, checklist Drafts do not create Estimates or Invoices, and the legacy workflow remains the production default until a later explicit rollout gate.

The Draft-to-Invoice backend foundation introduces Invoice-specific live/snapshot linkage on durable Work Drafts and launch ledger rows, extends backend output constraints to include `invoice`, requires existing billing authority to save or launch Invoice-intended standard Drafts, creates exactly one draft Invoice with ordered invoice line items through a private helper, keeps launch idempotency and consumed-Draft recovery semantics, and returns `invoice_id` in the launch JSON. The SQL/RPC foundation has passed Sandbox correction/validation, Production preflight/application/static review, and controlled Production runtime validation. Branch `codex/draft-to-invoice-app-wiring` now wires the visible Draft-first app path in source so billing-capable entitled contractors can choose draft Invoice intent, route New invoice into Draft-first Invoice planning, save/resume Invoice-intended Drafts, launch exactly one draft Invoice, and focus the created draft Invoice. Sandbox role/profile testing later found a direct backend cohort-control gap: a non-entitled comparison contractor remains UI-off and tenant-isolated but direct durable Draft save/launch RPC calls for its own contractor context need an explicit server-side entitlement guard. Branch `codex/durable-draft-rpc-entitlement-hardening` adds source-only SQL/RPC hardening so durable Draft save/launch require `durable_draft_beta_enabled` before mutation and report `DRAFT_ENTITLEMENT_REQUIRED`; SQL application and runtime validation remain separate approval gates. Production deployment/observation remains pending, and Draft-to-Invoice stays behind existing gates/cohort/authority until separately approved. No customer-facing Invoice send, payment, PDF/storage, notification, Home History, appointment, report finalization, gate, entitlement, or rollout change is included.
...
- `Save Draft` is the pause-and-return option, not a required step before direct outcome selection. `Create Job` now follows that model in code by persisting the current Draft state before activation. `Create Estimate` has durable launch/source handoff support in the enabled internal cohort. Source app wiring for direct Draft-to-Invoice launch exists only behind the existing Draft-first Work gates, durable cohort entitlement, resolved contractor context, and billing authority; Production deployment and app-flow observation remain pending separate approval.
- Durable Draft line-item detail parity is limited to fields already persisted by `contractor_work_draft_items`. Room, location, internal notes, line type, quantity, unit, price, and labor hours are supported; model/spec and supply-status details require a separately approved SQL/RPC/type extension before they can be exposed in Draft-first planning without silently dropping contractor input.
...
- Current Estimate, Job, and Draft permission helpers remain compatibility boundaries, not finalized role policy. Durable Draft persistence uses the union of existing billing-management and Job-write capabilities because it is shared planning for either output; launch remains output-specific. Slice 2B adds no role-name-based prohibition, does not broaden Estimate launch authority, and keeps view-only users unable to mutate unless an existing corresponding creation flow permits it.
...
6. Slice 2B: add the durable Draft Launch Foundation, including persisted intended output, supported notes, consumed state, launched output type/id, launched timestamp/user, private-ledger reverse linkage, duplicate-launch prevention, idempotency, atomic transaction boundaries, rollback behavior, consumed Draft exclusion, RLS/grants/RPC contracts, and sandbox-first SQL validation. Foundation and permission correction are installed and runtime-validated in Sandbox; the corrected canonical foundation is installed and verified in Production, where durable tables remain empty and gates remain off.
7. Slice 2C-A: add hidden durable frontend contract adapters and supporting pure utilities without UI wiring.
7a. Slice 2C-B: make durable save/resume/list/import and active/consumed presentation canonical. Merged in PR #319.
7b. Slice 2C-C1: merged PR #320 adds explicit gated launch commitment, save-before-launch, verified attempt recovery, canonical consumed adoption, list synchronization, and explicit Open Output without automatic navigation. Its final corrections give lifecycle/reconciliation locks precedence over weaker cross-tab recovery, make Workspace ownership mutation depend on the same reducer-derived acceptance decision, preserve local pending work when same-key, different-key, or cross-family recovery is rejected, preserve validated external-success reconciliation, and add stale save/resume, verified-discard, intent-mismatch, Back, unmount, client/target, listener-cleanup, and local-pending external-attempt regressions.
7c. Slice 2C-C2: merged PR #321 adds canonical-consumed-first automatic Estimate/Job opening only for same-session explicit Create/Retry, while preserving explicit-only background recovery and explicit Open fallback. Complete loaded output records are strictly runtime-validated before adoption; malformed records remain consumed and recoverable. Deferred focus targets the exact output heading under a unique operation token, with production focus-path coverage for same-family and cross-family races. The corrected suite records 33 pure, 78 rendered, 3 production focus-integration, and 1 source-integrity case (115 unique C2 tests).
7d. Slice 2C-D: complete Sandbox/Preview runtime and mobile validation plus documentation closeout before any rollout decision.
