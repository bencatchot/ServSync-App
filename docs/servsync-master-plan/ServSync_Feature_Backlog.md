...
Current next step:
Create ServSync Product Information Architecture v1 before the next Draft-first Jobs redesign implementation slice. It should cover platform-level principles, contractor portal architecture, homeowner portal architecture, sidebar/navigation structure, cross-profile workflows, and module ownership/boundaries. Do not re-enable the Production Draft UI, ship Create Estimate, ship Create Invoice, add invoice allocation changes, replace direct-job creation, or add additional contextual Draft entry points until separately approved.
