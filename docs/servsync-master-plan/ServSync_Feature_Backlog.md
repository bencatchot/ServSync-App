...
Last reconciled against `origin/main` at `c1f06fdf971b4b0218c81b823408473b932b1fa4` (merged PR #359).
...
| FB-024 | Price Book / Reusable Estimate Content Maturity | Estimates, saved charges, trade libraries, Draft-first reusable work | Implementation In Progress | High | The contractor-owned Price Book foundation and frontend management cleanup are complete: visible terminology, search-first organization, existing-field filters, compact paged rows, progressive item forms, explicit states, secondary CSV tools, and owner/admin/office UI alignment. Next: add a separate Draft-first Price Book picker using existing editable snapshot semantics and safe-field mapping. Multi-select/quantity staging, assemblies, default quantity/tax/category behavior, favorites/usage, cost/margin, bulk tools/export, invoice selection, and broader trade-library decisions remain later work. Saved Charges and Inspection Checklists remain separate. |
...
| FB-035 | Draft-First Work Rollout | Contractor Jobs, Drafts, estimates, jobs, invoices | Implementation In Progress | High | Keep active. Source and SQL foundations exist in stages, but Production gates/cohorts and full rollout remain controlled. This branch implements the first redesigned Jobs landing: one live-count At a Glance tile system, bounded Needs Attention, dedicated Drafts, and role-safe Actions & Tools. It does not change rollout gates, backend authority, or Production state. See the rollout matrix below. |
...
| --- | --- | --- |
| Source status | Multiple Draft-first Work foundations, direct Draft-to-Invoice wiring, entitlement hardening, the private Preview/Sandbox all-contractor rollout mode, and Service Plans discoverability are merged through current `origin/main`. This branch redesigns only the gated landing UI under the visible `Jobs` identity: At a Glance, Actions & Tools, dedicated Drafts, and bounded Needs Attention. Existing RPCs, record destinations, role checks, and legacy fallback remain authoritative. | Validate the branch Preview and owner review before merge. Separately verify deployed Production source and gate behavior before any Production expansion. |
...
| Validated outputs | Draft-to-Estimate, Draft-to-Job, inspection checklist, and Draft-to-Invoice paths have source/Sandbox/internal evidence recorded in the changelog. Prior Preview evidence covers Draft persistence, clean Start New Draft behavior, shared composer navigation, and Service Plans discoverability. Service Plans remain a separate template/offer workflow and are not a Draft outcome. | Validate this branch's exact tile counts, dedicated Drafts/Needs Attention destinations, role-safe tools, responsive layout, and existing destination navigation in Preview. Runtime evidence remains bounded; do not claim universal concurrency, saved-output validation, or real-user rollout completion. |
| Remaining blockers | Branch Preview review, merge decision, Production rollout planning, Production SQL/setting decision, Production gate decision, role-matrix follow-up, telemetry, Production smoke, mobile review, and owner release decisions. | Complete controlled gates before describing Draft-first Work or the redesigned Jobs landing as live. |
