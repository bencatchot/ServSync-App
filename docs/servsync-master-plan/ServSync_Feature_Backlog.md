...
Status: Work Dashboard Slice 1 Draft PR / Production Disabled
...
4b. Plan the Jobs landing and contextual `Start New Draft` entry points against the Draft-First Work principle and Product Information Architecture before adding more creation buttons.
...
Current next step:
Perform a focused read-only audit of the Contractor Work Dashboard Slice 1 draft PR, then enable `VITE_CONTRACTOR_WORK_UI_ENABLED=true` only in an approved Preview/sandbox validation environment if the audit passes. Do not re-enable the Production Draft UI, ship Create Estimate, ship Create Invoice, add invoice allocation changes, replace direct-job creation, add additional contextual Draft entry points, rename global navigation, add a Work route, or add mobile navigation items until separately approved.
