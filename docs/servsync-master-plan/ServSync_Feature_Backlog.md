...
7b. Slice 2C-C1: open, draft, unmerged PR #320 adds explicit gated launch commitment, save-before-launch, verified attempt recovery, canonical consumed adoption, list synchronization, and explicit Open Output without automatic navigation. Its final correction gives lifecycle/reconciliation locks precedence over weaker cross-tab recovery, preserves validated external-success reconciliation, and adds stale save/resume, verified-discard, intent-mismatch, Back, unmount, client/target, and listener-cleanup regressions.
