...
| FB-035 | Unified Start New Job Workflow | Estimates, jobs, invoices, job scope, approvals, billing history | Slice 2C-C2 In Draft PR #321 | High | PRs #317-#320 are merged: the durable backend, frontend contracts, canonical workspace, and C1 launch commitment/recovery are complete on `main`. Slice 2C-C2 is implemented in open, draft, unmerged PR #321. Eligible same-session confirmed Create and explicit Retry outcomes wait for canonical consumed adoption before a memory-only, token-owned automatic Estimate/Job open; external, mount, list, lifecycle-only, and background recovery remain explicit-only. Explicit Open Output remains, navigation failure never retries launch or creates a key, and stale context cannot adopt or focus an output. No Draft-to-Invoice path, SQL/RPC change, gate rollout, Sandbox/Production access, or visible Production impact is included. Production durable Draft SQL remains unapplied, gates remain default-off, and 2C-D authenticated Preview/mobile validation remains required. |
...
7c. Slice 2C-C2: open, draft, unmerged PR #321 adds canonical-consumed-first automatic Estimate/Job opening only for same-session explicit Create/Retry, while preserving explicit-only background recovery and explicit Open fallback. Complete loaded output records are strictly runtime-validated before adoption; malformed records remain consumed and recoverable. Deferred focus targets the exact output heading under a unique operation token, with production focus-path coverage for same-family and cross-family races. The corrected suite records 33 pure, 78 rendered, 3 production focus-integration, and 1 source-integrity case (115 unique C2 tests).
...
Current next step:
Complete abbreviated read-only verification of open, draft, unmerged PR #321 for Slice 2C-C2 guarded same-session post-launch navigation, including strict output validation and exact-output/token-owned focus. PRs #317-#320 are merged; C2 remains unmerged and 2C-D authenticated runtime/mobile closeout has not begun. Sandbox foundation/runtime permission validation is complete, and neither Sandbox SQL artifact should be reapplied. Keep gates default-off and do not begin 2C-D, apply Slice 2B to Production, enable the shared Composer for ordinary Preview users, ship direct Draft-to-Invoice launch, add inspection/checklist format, replace direct-job creation, or enable feature gates until separately approved.
