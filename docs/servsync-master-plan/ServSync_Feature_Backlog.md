...
| FB-035 | Unified Start New Job Workflow | Estimates, jobs, invoices, job scope, approvals, billing history | Slice 2C-B Gated Durable Integration In Progress | High | The validated durable Draft foundation is merged in PR #317 and installed in sandbox with its permission correction; Production remains unapplied on the legacy workflow. PR #318 is merged and Slice 2C-A hidden contracts/utilities are complete. Slice 2C-B makes dedicated durable state authoritative only under the existing Shared + Draft Job gate combination, adding atomic save, canonical resume, active/consumed lists, lazy legacy import and de-duplication, contractor-private notes, and read-only consumed/deleted-output presentation. Gates remain default-off and no launch controls, launch calls, SQL, Supabase mutation, environment/Vercel change, or Production impact are included. Slice 2C-C remains coherent Estimate/Job launch UX; Slice 2C-D remains sandbox/Preview runtime/mobile validation and closeout. The full user-visible Draft-first workflow remains incomplete. |
...
The Slice 2C architecture audit required a four-part implementation. Slice 2C-A adds hidden typed durable API contracts, pure Draft/item/list/import mappings, explicit local-versus-durable item identity, structured errors, helper-derived compatibility capabilities, and privacy-safe per-Draft idempotency attempt storage. Its final safety correction rejects malformed canonical responses before state adoption, reconciles new item identities only by validated canonical sort order, and reports browser-storage failures without claiming retry durability. PR #318 remains open and draft pending an abbreviated read-only re-audit; Slice 2C-B has not started. Slice 2C-A does not connect these contracts to the Composer, Work Dashboard, routes, Draft lists, or visible controls. Slice 2C-B will establish canonical durable save/resume/list/import and active/consumed presentation. Slice 2C-C will implement both launch actions, idempotent execution, permission/error UX, and navigation. Slice 2C-D will complete sandbox/Preview runtime and mobile validation plus documentation closeout.
...
Current next step:
Review Slice 2C-A as hidden frontend contract work, then proceed to a separately approved 2C-B integration slice. PR #317 is merged and sandbox application/runtime permission validation are complete; do not reapply either sandbox SQL artifact. Do not apply Slice 2B to Production, re-enable the Production Draft UI, enable a half-complete shared composer for ordinary Preview users, ship visible Create Estimate/Create Job actions, ship direct Draft-to-Invoice launch, add inspection/checklist format, add Draft-list or active/consumed presentation, replace direct-job creation, or enable feature gates until separately approved.
