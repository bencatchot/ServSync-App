...
| --- | --- | --- | --- | --- | --- |
| FB-003B | Request-Free Operational Document Delivery Authorization | Local customers, estimates, invoices, operational delivery | Implementation In Progress | High | Draft PR #369 implements Request-Free Local Invoice Delivery Slice 1 and now places recipient lookup behind a same-origin service-role Vercel Function with an official Firewall entry check, private atomic database rate buckets, direct browser-RPC revocation, exact 100-line/262,144-byte bounds, and saturated open history. The original migration is installed only in Sandbox and remains byte-identical; the corrective migration is unapplied, the Function is undeployed, no Firewall rule or server secret is configured, and Demo/Production remain untouched. FB-003B stays active, draft, unmerged, and blocked from rollout pending independent source review plus separately authorized Sandbox corrective SQL, secret/configuration, Firewall, deployment, and live security/abuse-control validation. Anonymous invoice PDFs, immutable/versioned estimate delivery, finalized reports, claimed-account transition, email/SMS, reminders, approval/payment/reply actions, and broader delivery maturity remain future work. |
