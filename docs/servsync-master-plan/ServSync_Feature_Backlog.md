...
Current repository evidence through PR #349 shows these relevant states:
...
| --- | --- | --- |
| Source status | Multiple Draft-first Work foundations and follow-up fixes are merged through `origin/main` `74498d90466af4f134c170fd747fc2f8130624bd`; direct Draft-to-Invoice app wiring and entitlement-hardening source are included on main. | Verify the currently deployed Production source and gate behavior before expanding cohorts. |
| SQL application status | Durable Draft foundations and selected follow-up SQL have been reviewed/applied in prior controlled gates where noted in the changelog. Recent entitlement hardening and later Draft-to-Invoice-related SQL require their own installed-state verification before relying on them. | Do not assume a source file is live SQL. Re-prove installed functions, grants, RLS, and hashes before any rollout. |
| Gate state | Draft-first Work remains controlled by explicit global gates and cohort entitlements. | No gate expansion without separate owner approval, deployment/source verification, and rollback plan. |
| Cohort state | Internal/cohort-bound validation has occurred for selected paths; broad contractor beta is not enabled by this backlog entry. | Owner must approve any cohort enablement or expansion. |
| Validated outputs | Draft-to-Estimate, Draft-to-Job, inspection checklist, and Draft-to-Invoice paths have source/Sandbox/internal evidence recorded in the changelog. | Runtime evidence remains bounded. Do not claim universal concurrency or real-user rollout completion. |
| Remaining blockers | Entitlement and SQL installed-state checks, role-matrix follow-up, telemetry, Preview/Production smoke, mobile review, rollback-readiness, and owner release decisions. | Complete controlled gates before describing Draft-first Work as live. |
