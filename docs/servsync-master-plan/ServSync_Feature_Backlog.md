...
Last reconciled against `origin/main` at `b23e18708601c515ff218823caf8825d5d72fd00` (merged PR #362).
...
Current repository evidence through PR #362 shows these relevant states:
...
- Draft-first Work is now the standard Production Jobs experience for authorized contractor contexts. Production serves main SHA `b23e18708601c515ff218823caf8825d5d72fd00`, all three global gates are enabled, and the server rollout mode is `all_contractors`; public smoke passed 7/7 and dedicated contractor read-only smoke confirmed the Draft-first Jobs landing on desktop and mobile. This does not broaden output-specific permissions or prove mutating Production save/launch paths for every role.
...
| FB-007 | Trade Checklist Coverage and Draft-First Inspection Path | Estimates, inspections, checklists, Draft-first Work | Gated Rollout | High | Completed estimate recipe, Rooms, Assets, Home Map, and template-history work is archived in the changelog/completed archive. The Draft-first inspection/checklist source and reviewed Production SQL foundation are installed under the standard Draft-first exposure, while output-specific authority remains fail-closed. Remaining work is focused trade-checklist coverage plus authenticated Production save/launch evidence before treating the path as fully validated. |
...
| FB-024 | Price Book / Reusable Estimate Content Maturity | Estimates, saved charges, trade libraries, Draft-first reusable work | Implementation In Progress | High | The contractor-owned Price Book foundation, frontend management cleanup, and PR #362 Draft-first Price Book Picker v1 are complete. Price Book Organization Foundation v1 adds the provider-neutral `trade -> category -> subcategory` source model, management controls, current-page bulk organization/archive/restore actions, and generic CSV subcategory mapping; its additive migration is created but has not been applied to any environment. Standard Estimate-intent Drafts continue to append one safe, editable snapshot line without private organization metadata, IDs, or live linkage. Repeat-import reconciliation, update/merge behavior, import batches and rollback, cost/margin, Saved Charges, assemblies, favorites/usage, export, quantity staging, invoice selection, provider-specific integrations, and broader Price Book maturity remain future work. |
