...
| SQL application status | Durable Draft foundations and selected follow-up SQL have been reviewed/applied in prior controlled gates where noted in the changelog. The Preview/Sandbox rollout-mode SQL defaults to `cohort` and must be applied and explicitly set to `all_contractors` only in approved Sandbox validation. | Do not assume a source file is live SQL. Re-prove installed functions, grants, RLS, runtime setting, and hashes before any rollout. |
...
| Cohort state | Internal/cohort-bound validation has occurred for selected paths; broad contractor Production beta is not enabled by this backlog entry. Preview/Sandbox all-contractor validation is an in-progress gated pre-Production step. | Owner must approve any Production cohort enablement or expansion. |
...
| Remaining blockers | Rollout-mode Sandbox SQL application, Preview env-gate configuration, PR preview and PR #351 preview validation, role-matrix follow-up, telemetry, Production smoke, mobile review, rollback-readiness, and owner release decisions. | Complete controlled gates before describing Draft-first Work as live. |
