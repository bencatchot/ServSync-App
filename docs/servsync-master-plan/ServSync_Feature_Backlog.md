...
| FB-035 | Unified Start New Job Workflow | Estimates, jobs, invoices, job scope, approvals, billing history | Internal Cohort / Invoice SQL Source Draft | High | Durable Draft-first Work is enabled only for the controlled internal Production contractor behind global gates and one contractor entitlement. Draft-to-Estimate and Draft-to-Job source remediations are merged through PR #337, with Production observation handled by separate deployment/rollout gates. Draft branch `codex/draft-to-invoice-sql-foundation` adds source-only SQL/RPC foundation for future standard Draft-to-Invoice launch using invoice-specific linkage, billing-authority checks, idempotent launch lineage, draft Invoice creation, ordered invoice line mapping, deleted-output recovery, and private helper revocation. The Draft-to-Invoice SQL is not applied to any environment, app wiring remains pending, no visible Invoice Draft launch is live, and no customer-facing invoice/payment/send/PDF/notification behavior is enabled. Production SQL application, app wiring, runtime validation, broader role matrix, telemetry, and external beta remain separate approval gates. |
