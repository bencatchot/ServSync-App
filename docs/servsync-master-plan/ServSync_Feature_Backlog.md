...
| FB-024 | Price Book / Reusable Estimate Content Maturity | Estimates, saved charges, trade libraries, Draft-first reusable work | Implementation In Progress | High | The contractor-owned Price Book foundation, PR #361 management cleanup, PR #362 Draft-first Price Book Picker v1, and PR #365 Price Book Organization Foundation v1 are complete; the organization migration was applied through its controlled Sandbox and Production rollout. Price Book Repeat-Import Reconciliation v1 is now implemented in source with stable tenant-owned import sources, deterministic external-ID matching, classified New/Unchanged/Changed/Ambiguous/Invalid review, explicit Add/Update/Skip actions, three-way manual-edit preservation, duplicate/collision rejection, private mappings, idempotent transactional execution, and sanitized batch/row audit. Its new migration remains unapplied pending separate approval. Standard Estimate-intent Drafts continue to append safe editable snapshots without private organization/import metadata, IDs, or live linkage. Guarded batch rollback is the next focused slice. XLSX, cost/margin, Saved Charges, assemblies, favorites/usage, export, quantity staging, invoice selection, provider-specific parsers/APIs, scheduled sync, and broader maturity remain future work. |
