...
| --- | --- | --- | --- | --- | --- |
| FB-003B | Request-Free Operational Document Delivery Authorization | Local customers, estimates, invoices, operational delivery | Preview Testing | High | Draft PR #369 implements Request-Free Local Invoice Delivery Slice 1 behind a same-origin service-role Vercel Function with an official Firewall entry check, private atomic database rate buckets, direct browser-RPC revocation, exact 100-line/262,144-byte bounds, saturated open history, and a digest-only 30-minute recipient session. Independent source review passed at head `f1191f76bef525f3b76aabbd8102af635e3c8f87`. Sandbox `zpzdkoaubyjtsomccxya` now has all three reviewed migrations in order: the unchanged baseline was already installed, then gateway hardening and recipient sessions were applied and catalog-verified. Dedicated Sandbox Vercel project `servsync-app-development` has the four required Sandbox Supabase variables, a live IP-keyed 100-request/60-second rule for SDK ID `request-free-local-invoice-delivery`, and READY deployment `dpl_7aNcLLQ9frcfkFDh7b7xTvBejw37`. Capped live validation passed catalog/grants, owner/admin/office authority, field-tech/viewer/homeowner/anonymous/second-tenant denial, desktop/mobile token and cookie containment, concurrency, rotation/revocation invalidation, request/line/response bounds, Firewall 429, counter saturation, cleanup capacity, and complete fixture cleanup. PR #369 remains open, draft, unmerged, and absent from Demo and Production; the next gate is merge-readiness review, not rollout. Anonymous invoice PDFs, immutable/versioned estimate delivery, finalized reports, claimed-account transition, email/SMS, reminders, approval/payment/reply actions, and broader delivery maturity remain future work. |
