...
- The reminder-to-room backend foundation adds optional `home_reminders.home_room_id` support for future reminder room tagging. Room links are optional, same-home validation prevents reminders from pointing at rooms from another home, and the slice does not add UI, shared reminder shell expansion, contractor visibility, document/photo room tags, inspection room linking, job/work-item room tags, estimate/invoice room tags, Home Map, assets/systems, or key locations.
