...
Last reconciled against `origin/main` at `854372c8609a67d7de16dac33550762d9d750d74` (merged PR #372).
...
- Do not claim full customer-management role parity. Customer Management Edit Boundary Correction v1 is implemented in open, unmerged PR #374 and validated in Sandbox only: exact migration `servsync-customer-management-edit-boundary.sql` (SHA-256 `b86b3af7524d3d0122f97e21609569dfa316b7f87d2d2df213c5e73478d66427`) was applied successfully to project `zpzdkoaubyjtsomccxya` at `2026-08-05T02:34:31Z`. Direct authenticated validation confirmed owner/admin/office profile and local-property mutation authority; field tech, viewer, inactive, removed, cross-tenant, and anonymous denial; tenant-scoped authorization before locking; generic foreign/unknown UUID handling; claimed-record and token-containment protections; exact deployed-function correspondence; and cleanup back to the recorded pre-validation counts. Admin/Office end-to-end profile/property UI reachability was not exercised because the unchanged contractor-local customer read/list boundary remains owner-only; direct RPC and source capability validation passed. Sandbox is temporarily ahead of main, while Demo and Production were untouched and no manual deployment or Vercel configuration change occurred. Local-customer read/list parity, admin/office creation parity, archive/restore lifecycle, direct-table privilege cleanup, broader customer-model unification, final merge, controlled Production SQL and runtime validation, Demo SQL disposition, the inherited non-owner appointment-window 401, and the inherited ESLint 9 startup incompatibility remain separate approved-workflow gates.
