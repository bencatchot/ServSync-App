...
Status: Internal Cohort / Invoice SQL Source Draft
...
Product goal:
Contractors should eventually enter work scope once in a shared Draft composer, choose an intended output, and launch exactly one initial first-class workflow record. The first approved live launch outputs are Estimate and Job. Direct Invoice launch remains hidden and disabled in the app, but draft branch `codex/draft-to-invoice-sql-foundation` now implements the source-only SQL/RPC foundation for future standard Draft-to-Invoice launch using invoice-specific linkage, billing-authority checks, idempotency, and draft Invoice lineage. Outcome-specific validation should happen at the launch boundary while the form remains shared. The goal is to reduce duplicated entry and inconsistent direct-job behavior without weakening approval, billing, duplicate prevention, or homeowner visibility rules.
...
Draft branch `codex/draft-to-invoice-sql-foundation` adds the first source-only Draft-to-Invoice backend foundation. It introduces Invoice-specific live/snapshot linkage on durable Work Drafts and launch ledger rows, extends backend output constraints to include `invoice`, requires existing billing authority to save or launch Invoice-intended standard Drafts, creates exactly one draft Invoice with ordered invoice line items through a private helper, keeps launch idempotency and consumed-Draft recovery semantics, and returns `invoice_id` in the launch JSON. The SQL was not applied to Sandbox, Production, Preview, Demo, or any live database; no visible composer Invoice option, New Invoice Draft routing, customer-facing Invoice Draft launch, send/payment/PDF/notification/Home History/appointment side effect, Vercel gate, entitlement, or rollout change is live. App wiring, SQL application preflight/application, runtime validation, and rollout remain pending separate approval.
