...
5. Launch the shared Start New Draft composer outcomes incrementally, beginning with Create Job from the current Draft composer state.
6. Add durable Draft launch foundation and Create Estimate from the shared composer.
7. Add Job/Estimate-to-Invoice workflow wiring, with direct Draft-to-Invoice launch only if separately approved.
8. Add estimate-from-existing-job and additional-work approval.
9. Add invoice allocation, deposit, itemized progress, amount-only progress, and final reconciliation.
10. Add document revisions, comprehensive history, and explicit reopening.
11. Remove the legacy direct-job creation experience.

Current next step:
Perform a focused read-only implementation plan for the first shared Draft Composer launch slice after the Draft Launch Principle is documented. Do not re-enable the Production Draft UI, ship Create Estimate without durable launch/consumption approval, ship direct Draft-to-Invoice launch, add invoice allocation changes, replace direct-job creation, add additional contextual Draft entry points, rename global navigation, add a Work route, or add mobile navigation items until separately approved.
