...
| FB-035 | Unified Start New Job Workflow | Estimates, jobs, invoices, job scope, approvals, billing history | Slice 2C-B Gated Durable Integration In Draft PR #319 | High | The validated durable Draft foundation is merged in PR #317 and installed in sandbox with its permission correction; Production remains unapplied on the legacy workflow. PR #318 is merged and Slice 2C-A hidden contracts/utilities are complete. Draft PR #319 makes dedicated durable state authoritative only under the existing Shared + Draft Job gate combination, adding atomic save, canonical resume, active/consumed lists, lazy legacy import and exact de-duplication, contractor-private notes, read-only consumed/deleted-output presentation, context-safe async adoption, request-consistent subject edits, recovery actions, and rendered race tests. Gates remain default-off and no launch controls, launch calls, SQL, Supabase mutation, environment/Vercel change, or Production impact are included. Slice 2C-C remains coherent Estimate/Job launch UX; Slice 2C-D remains authenticated sandbox/Preview runtime/mobile validation and closeout. The full user-visible Draft-first workflow remains incomplete. |
...
The Slice 2C architecture audit required a four-part implementation. Slice 2C-A added hidden typed durable API contracts, pure Draft/item/list/import mappings, explicit local-versus-durable item identity, structured errors, helper-derived compatibility capabilities, and privacy-safe per-Draft idempotency attempt storage; PR #318 passed its safety re-audit and merged. Slice 2C-B now exists as open draft PR #319 and integrates canonical durable save/resume/list/import plus active/consumed presentation behind the existing default-off gates. Its audit correction adds target normalization, canonical first-save identity, contractor/editor request generations, hidden request clearing, guarded retry/import/open actions, and rendered race/recovery tests. Slice 2C-C will implement both launch actions, idempotent execution, permission/error UX, and navigation. Slice 2C-D will complete authenticated sandbox/Preview runtime and mobile validation plus documentation closeout.
...
7a. Slice 2C-B: make durable save/resume/list/import and active/consumed presentation canonical.
7b. Slice 2C-C: connect the shared Draft Composer to both complete launch outcomes together with idempotency, permission/error UX, and navigation.
7c. Slice 2C-D: complete sandbox/Preview runtime and mobile validation plus documentation closeout before any rollout decision.
...
Current next step:
Complete the abbreviated read-only re-audit of Slice 2C-B draft PR #319. PR #317 and PR #318 are merged; sandbox foundation/runtime permission validation are complete, and neither sandbox SQL artifact should be reapplied. Do not mark PR #319 ready or merge it before approval, begin Slice 2C-C launch UX, apply Slice 2B to Production, re-enable the Production Draft UI, enable the shared Composer for ordinary Preview users, ship visible Create Estimate/Create Job actions, ship direct Draft-to-Invoice launch, add inspection/checklist format, replace direct-job creation, or enable feature gates until separately approved.
