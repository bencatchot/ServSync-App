...
Slice 2B packages the durable Draft Launch Foundation for review without applying it to sandbox or Production. It proposes dedicated `contractor_work_drafts`, `contractor_work_draft_items`, and `contractor_work_draft_launches` tables, durable nullable intended output, contractor-private Draft notes, consumed launch state, launched Estimate/Job linkage, stable Draft item IDs, one-successful-launch constraints, idempotency protection, atomic launch rollback behavior, contractor-only RLS/grants, canonical save/resume RPCs, a public launch RPC, private Estimate/Job launch helpers, and a non-destructive legacy inspection-backed Draft Job import bridge. The packaged SQL remains hidden and unapplied until separately audited and approved for sandbox execution. It exposes no Create Estimate or Create Job UI, no Invoice launch, no inspection/checklist implementation, no Production workflow change, and no Production data or environment changes.
