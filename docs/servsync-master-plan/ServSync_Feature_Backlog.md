...
Last reconciled against `origin/main` at `e67ef9ceaa0f3bd948bdb0de33bbde6f11d8dd38` (merged PR #361).
...
Current repository evidence through PR #361 shows these relevant states:
...
| FB-035 | Draft-First Work Rollout | Contractor Jobs, Drafts, estimates, jobs, invoices | Implementation In Progress | High | Keep active. Source and SQL foundations exist in stages, but Production gates/cohorts and full rollout remain controlled. PR #360 merged the first redesigned Jobs landing: one live-count At a Glance tile system, bounded Needs Attention, dedicated Drafts, and role-safe Actions & Tools. It did not change rollout gates, backend authority, or Production state. See the rollout matrix below. |
...
| Validated outputs | Draft-to-Estimate, Draft-to-Job, inspection checklist, and Draft-to-Invoice paths have source/Sandbox/internal evidence recorded in the changelog. Prior Preview evidence covers Draft persistence, clean Start New Draft behavior, shared composer navigation, Service Plans discoverability, and the PR #360 Jobs landing. Service Plans remain a separate template/offer workflow and are not a Draft outcome. | Runtime evidence remains bounded; do not claim universal concurrency, saved-output validation, or real-user rollout completion. |
| Remaining blockers | Production rollout planning, Production SQL/setting decision, Production gate decision, role-matrix follow-up, telemetry, Production smoke, mobile review, and owner release decisions. | Complete controlled gates before describing Draft-first Work or the redesigned Jobs landing as broadly live. |
