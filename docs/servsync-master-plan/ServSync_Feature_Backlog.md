...
- The document-to-room backend foundation adds optional `home_documents.home_room_id` support for future homeowner document organization. Room links are optional, same-home validation prevents documents from pointing at rooms from another home, unassigned documents cannot point at rooms, room notes are not exposed through documents, and the slice adds no UI, no storage bucket/policy changes, no shared-home document expansion, no contractor visibility expansion, no photo tags, no report/job media tags, no floor-plan behavior, no Home Map, no assets/systems, and no key locations.
- The manual document upload room-registration foundation lets the prepare/register RPC path carry optional same-home room links from upload reservations into newly registered homeowner documents. Null room links remain supported, existing upload limits and storage path rules remain in place, no broad `home_documents` update grant is added, and document room picker/chips/filters/editing remain future UI work.
- This does not complete full trade-specific estimate/pre-visit checklist coverage.

Current next step:
Validate the estimate start-choice, Save as template modal, Advanced Trade Tools cleanup, clarified Jobs/Templates taxonomy, Saved Work Template action clarity, customer-context-only manual invoice draft creation, selected-context manual job form starts, name/scope-only manual-job template-limit copy, Home Setup guide shell, `home_rooms` RLS foundation, first homeowner Rooms UI, reminder-to-room SQL/UI foundation, document-to-room SQL foundation, and manual document upload room-registration foundation with connected homeowners, shared-home roles, contractor-private local customers, inline-created local customers, and homeowner beta fixtures in preview/beta. Keep global template-to-invoice without selected customer context, global direct-job-from-template, full line-item-to-job-work-item mapping, true Home Setup Templates/Home Map, assets/systems, key locations, room-linked reminder edit assignment/grouping, document room UI/chips/filters/editing, photo room tagging, report/job media tagging, contractor room/document visibility expansion, broader recipe-library expansion, homeowner pre-visit checklist work, pricing suggestions, recipe catalog UX, and trade-coverage expansion separately approved.
