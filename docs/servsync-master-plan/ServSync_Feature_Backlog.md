...
| FB-035 | Unified Start New Job Workflow | Estimates, jobs, invoices, job scope, approvals, billing history | Slice 2C-C1 Launch Commitment In Draft PR #320 | High | PRs #317-#319 are merged: the durable backend, hidden frontend contracts, and gated canonical save/resume/list/import workspace are complete. Slice 2C-C is split. Open, draft, unmerged PR #320 adds C1 gated explicit Create Estimate/Create Job confirmation, save-before-launch, refreshed distinct output authority, verified privacy-safe idempotency attempts and removal, same-key ambiguous/remount/multi-tab recovery, fail-closed lifecycle and cross-tab reconciliation, canonical consumed adoption, active/consumed list synchronization, and explicit Open Output only. It adds no automatic output navigation and no Draft-to-Invoice path. C2 remains automatic navigation and final navigation/mobile/accessibility polish; 2C-D remains authenticated Sandbox/Preview runtime/mobile validation and closeout. SQL is unchanged, Production durable Draft SQL remains unapplied, gates remain default-off, and the full user-visible redesign is not complete. |
...
The Slice 2C architecture audit produced bounded implementation stages. Slice 2C-A added hidden typed durable API contracts, pure mappings, explicit local-versus-durable item identity, structured errors, helper-derived capabilities, and privacy-safe fail-closed attempt storage; PR #318 merged. Slice 2C-B merged in PR #319 with canonical gated save/resume/list/import, active/consumed presentation, guarded existing-output opening, and token-owned async safety. Slice 2C-C is split: C1 is implemented in open, draft, unmerged PR #320 with explicit confirmation, save-before-launch, distinct Estimate/Job authority, verified idempotency persistence/removal, same-key retry/recovery, fail-closed lifecycle and cross-tab reconciliation, canonical consumed adoption, list synchronization, and no automatic output navigation. C2 remains automatic post-launch navigation and final navigation/mobile/accessibility polish. Slice 2C-D remains authenticated Sandbox/Preview runtime and mobile validation plus documentation closeout.
...
Current next step:
Complete the final abbreviated read-only verification of Slice 2C-C1 draft PR #320 after its Workspace external-attempt acceptance and local-operation ownership correction. PRs #317-#319 are merged; PR #320 remains open, draft, and unmerged. Sandbox foundation/runtime permission validation is complete, and neither Sandbox SQL artifact should be reapplied. Keep gates default-off and do not begin C2 automatic navigation, begin 2C-D authenticated runtime closeout, apply Slice 2B to Production, enable the shared Composer for ordinary Preview users, ship direct Draft-to-Invoice launch, add inspection/checklist format, replace direct-job creation, or enable feature gates until separately approved.
