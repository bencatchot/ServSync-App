...
| FB-011A | Backlog-Aware Partial Invoicing | Jobs, work items, invoices, backlog closeout | Phase 2B-1 Pending SQL Validation | High | Phase 1 adds SQL/type/test foundation for durable job work items, invoice line linkage, backlog snapshots, and a guarded partial-invoice RPC. Phase 2A wires durable work items into contractor job detail when rows exist, creates draft invoices from selected completed/priced items, and displays backlog snapshots separately on contractor/homeowner invoice views and PDFs. Phase 2B-1 adds a pending SQL patch/test so accepted-estimate job creation can seed one durable work item per estimate line after environment-specific SQL approval. Broad legacy backfill, simple task sync, inspection finding conversion, manual work-item editing, and broader closeout UI remain future approval-gated slices. |
