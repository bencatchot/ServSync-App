...
Homeowner invoice closeout now makes the final handoff clearer: once an eligible invoice is durably filed to Home History, the invoice card shows a `View Home History` next action and compact copy explaining that the homeowner can add a manual follow-up reminder there. Contractor completed-job closeout now also shows compact recommended-path copy: work-item jobs point contractors toward completed, priced work-item invoicing while legacy/no-work-item jobs point toward one completed-job invoice. These remain display/navigation polish only; they do not add automatic reminders, recurring reminders, payment collection, notifications, invoice eligibility changes, or duplicate filing behavior.
