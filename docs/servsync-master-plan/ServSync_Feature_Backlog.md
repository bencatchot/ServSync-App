...
| FB-035 | Unified Start New Job Workflow | Estimates, jobs, invoices, job scope, approvals, billing history | Slice 2C-A Hidden Durable Frontend Contracts In Progress | High | The validated durable Draft foundation is merged in PR #317 and installed in sandbox with its permission correction; Production remains unapplied on the legacy workflow. PR #318 now includes the focused response-validation, non-positional reconciliation, and fail-closed storage correction and remains draft pending abbreviated re-audit. The approved Slice 2C split is 2C-A hidden frontend contracts/utilities, 2C-B canonical durable save/resume/list/import and consumed presentation, 2C-C coherent Estimate/Job launch UX, and 2C-D sandbox/Preview runtime/mobile validation and closeout. Slice 2C-A adds no UI wiring, visible action, SQL, feature-gate, environment, Vercel, sandbox, or Production change. The full user-visible Draft-first workflow remains incomplete. |
