...
| FB-035 | Unified Start New Job Workflow | Estimates, jobs, invoices, job scope, approvals, billing history | Cohort Gate Sandbox-Validated / PR Draft | High | PRs #317-#321 are merged and Slice 2C-C is complete on `main`. Production has the corrected canonical durable Draft foundation installed and verified, its durable Draft tables remain empty, and the three global Draft/Work gates remain absent/off. Draft PR #322 adds the default-false tenant entitlement, strict canonical UUID parsing, coalesced one-minute focus/visibility refresh, exact tenant/session/client generation ownership, and legacy Jobs fallback. The cohort SQL is installed in Sandbox and the controlled staged role/privacy/session/direct-entry/Draft-to-Estimate matrix passed; runtime fixtures were cleaned, the target was restored false, all Sandbox billing rows ended false, and branch gates were removed. Production cohort SQL, Production enrollment, global-gate rollout, structured privacy-safe telemetry, and external beta remain separate approval gates. No Draft-to-Invoice path exists. |
...
Status: Slice 2C-A Hidden Durable Frontend Contracts / Production Disabled
