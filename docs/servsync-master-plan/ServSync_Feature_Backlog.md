...
| FB-011A | Backlog-Aware Partial Invoicing | Jobs, work items, invoices, backlog closeout | Phase 2C-1 Sandbox Validated | High | Phase 1 adds SQL/type/test foundation for durable job work items, invoice line linkage, backlog snapshots, and a guarded partial-invoice RPC. Phase 2A wires durable work items into contractor job detail when rows exist, creates draft invoices from selected completed/priced items, and displays backlog snapshots separately on contractor/homeowner invoice views and PDFs. Phase 2B-1 seeds durable work items from accepted estimate lines when approved SQL is applied. Phase 2B-2 adds a pending SQL/RPC/app foundation for syncing simple service `Work Items` tasks into durable source-backed work items after environment-specific SQL approval. Phase 2C-1 adds a sandbox-validated SQL/RPC/app foundation for contractor-created manual work items, with safe unbilled-only edit/remove rules and no invoice/PDF redesign; production SQL rollout remains pending separate approval. Broad legacy backfill, general inspection finding conversion, and broader closeout UI remain future approval-gated slices. |
