...
| FB-026 | Review + Referral Flow | Discover, contractor profiles, completed jobs, trust | Started / Partial | High | Existing ServSync review capture is hardened around completed-work eligibility, private party visibility, external-review separation, review table/function grant posture, pending moderation SQL/RPC foundation, and an internal platform-admin moderation UI. The contractor-to-contractor referral foundation slice packages a dedicated SQL/RLS/RPC backend for `contractor_refers_contractor` referral invite leads, kept separate from homeowner invite leads, contractor-homeowner invites, reward referrals, contractor team invites, connection requests, service requests, billing, and email. Public ServSync review averages, counts, kudos, and snippets remain paused during beta; admin approval records moderation status only and does not publish reviews publicly yet. Future work includes approved-only public display/public-rating policy, contractor referral UI/admin UI after SQL approval, recommendation/referral product decisions, and optional Google review handoff. No fake ratings, paid ranking, rewards, credits, billing, paywalls, automatic outreach, or premature badges. |
...
Current next step:
Review the internal admin moderation workflow and the contractor-to-contractor referral SQL/RLS/RPC package, then separately decide whether to apply the contractor referral SQL to sandbox for runtime probes or add an approved-only public review display slice. Defer contractor referral UI, admin referral tracking UI, homeowner recommendations/referrals, Google review handoff, badges/awards, contractor disputes/responses, and any Discover ranking changes to later approved slices.

Contractor-to-contractor referral foundation:

- Adds a proposed `servsync_referral_invites` backend table for typed referral/invite leads, initially constrained to `contractor_refers_contractor`.
- Adds planned RPC boundaries for contractor owner/admin/office submission and platform-admin listing/update.
...
- No contractor-facing UI, platform-admin UI, public landing page, email/SMS/push delivery, rewards, credits, subscriptions, billing, paywalls, automatic contractor profile creation, contractor team access, homeowner connection, service request, job, estimate, invoice, notification, or production SQL apply is included in the foundation package.
- Next step after review should be sandbox SQL apply/probe only, then separately approved contractor submit UI and platform-admin tracking UI slices.
