...
Current repository evidence through PR #366 shows these relevant states:
...
| FB-007 | Trade Checklist Coverage and Draft-First Inspection Path | Estimates, inspections, checklists, Draft-first Work | Implementation In Progress | High | Draft-first Inspection Path Completion v1 is being implemented as a focused vertical slice: contextual connected/local customer starts converge on the shared Draft composer, home-specific checklist choices remain bound to the exact selected property, `Not Applicable` becomes a neutral recorded finding state, and report lifecycle copy reflects actual filing/notification behavior. A source-only additive hardening migration narrows template mutation and report/media writes to the intended contractor roles; it is not applied by this work. FB-007 remains active pending review, SQL rollout, authenticated desktop/mobile Preview validation, and broader trade-checklist maturity. |
