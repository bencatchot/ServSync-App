...
Slice 2B packages the corrected durable Draft Launch Foundation. It proposes dedicated `contractor_work_drafts`, `contractor_work_draft_items`, and private success-only `contractor_work_draft_launches` tables, durable nullable intended output, contractor-private Draft notes and subject audit snapshots, consumed launch state, deletion-compatible private-ledger Estimate/Job linkage with nullable live FKs and immutable output-ID snapshots, stable Draft item IDs, one-successful-launch constraints, contractor-serialized idempotency protection, atomic launch rollback behavior, non-enumerating tenant authorization, launch-time relationship locking/revalidation, legacy-import relationship validation before snapshotting, Job property-sharing parity, shared database-level contractor/service-request operational-Job deduplication, contractor-history retention, contractor-only RLS/grants, canonical save/resume RPCs, a public launch RPC, private Estimate/Job launch helpers, and a non-destructive legacy inspection-backed Draft Job import bridge whose intended output defaults to null. Deleting an eligible launched draft output does not reactivate its consumed Draft or permit another launch. Private Draft IDs are not placed on homeowner-readable Estimate or Job rows. The reviewed foundation hash is installed in ServSync Sandbox and passed the completed runtime matrix except for a Draft-persistence permission-parity defect: Job-authorized users could create normal Jobs but could not save the durable Draft required by Draft-first Job launch. The corrected canonical foundation now permits Draft persistence through the union of the existing billing-management and Job-write capability helpers, without role-name checks. Estimate launch remains separately governed by the normal Estimate-creation boundary, Job launch remains governed by Job-write capability, and detailed employee-role policy remains future work. PR #317 remains open and draft. Its narrow sandbox correction package remains unapplied; the exact-predecessor guard now fingerprints public RPC defaults so any default-expression drift blocks application. Another short read-only audit and owner approval are required before application, followed by the focused permission matrix. Production remains untouched and will use the corrected canonical foundation in any future separately approved application. Slice 2B remains hidden and exposes no Create Estimate or Create Job UI, Invoice launch, inspection/checklist implementation, Production workflow change, or Production data/environment change.
...
6. Slice 2B: add the durable Draft Launch Foundation, including persisted intended output, supported notes, consumed state, launched output type/id, launched timestamp/user, private-ledger reverse linkage, duplicate-launch prevention, idempotency, atomic transaction boundaries, rollback behavior, consumed Draft exclusion, RLS/grants/RPC contracts, and sandbox-first SQL validation. The reviewed foundation is installed in sandbox; the permission-parity correction requires a focused read-only audit, separate owner approval, application of only the narrow correction SQL, and focused runtime permission validation. Production remains unapplied.
...
Current next step:
Keep PR #317 open and draft while performing one short read-only audit of the hardened Draft-persistence permission-parity correction before authorizing its sandbox-only application. After any separate approval and application, rerun the focused owner/admin/office/field-technician/viewer/platform-admin permission matrix and confirm Estimate launch remains unbroadened. Do not reapply the full foundation to sandbox, re-enable the Production Draft UI, enable a half-complete shared composer for ordinary Preview users, ship Create Estimate without durable launch/consumption approval, ship direct Draft-to-Invoice launch, add invoice allocation changes, replace direct-job creation, add contextual Draft entry points, rename global navigation, add a Work route, or add mobile navigation items until separately approved.
