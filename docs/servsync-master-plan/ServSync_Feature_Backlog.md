...
- Local customer claim-invite manual delivery hardening is source-implemented but still requires SQL-first review/application/validation before deployment. The approved boundary is manual Copy Link / QR only for contractor owner, admin, and office roles; field tech/viewer users must not prepare or reveal raw claim tokens. Normal invite list/detail reads should stay token-free, Copy/QR must go through a guarded prepare-delivery RPC, and final direct table-token containment must be applied after the app no longer depends on table reads.
