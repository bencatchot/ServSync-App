...
Current next step:
Validate the estimate start-choice, Save as template modal, Advanced Trade Tools cleanup, clarified Jobs/Templates taxonomy, Saved Work Template action clarity, and customer-context-only manual invoice draft creation with connected homeowners, contractor-private local customers, and inline-created local customers in preview/beta. Keep global template-to-invoice without selected customer context, direct job start design for mapping template line items into job work items, Home Setup Templates/Home Map, broader recipe-library expansion, homeowner pre-visit checklist work, pricing suggestions, recipe catalog UX, and trade-coverage expansion separately approved.
