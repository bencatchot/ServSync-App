...
Current repository evidence through PR #367 shows these relevant states:
...
| --- | --- | --- | --- | --- | --- |
| FB-003B | Request-Free Operational Document Delivery Authorization | Local customers, estimates, invoices, operational delivery | Implementation In Progress | High | Draft PR #369 implements Request-Free Local Invoice Delivery Slice 1 behind a same-origin service-role Vercel Function with an official Firewall entry check, private atomic database rate buckets, direct browser-RPC revocation, exact 100-line/262,144-byte bounds, and saturated open history. The corrected browser synchronously removes the original bearer from visible location/history and uses one abortable serialized request lane before exchanging it for a random invoice-scoped 30-minute `Secure`, `HttpOnly`, `SameSite=Strict`, `Path=/`, no-`Domain` cookie; unsuccessful gateway paths expire that cookie, SQL retains only its SHA-256 digest, lock acquisition precedes prior-session invalidation, and deterministic bounded cleanup runs after every successful bootstrap. The original migration is installed only in Sandbox and remains byte-identical; the gateway-hardening and session migrations are unapplied. The Vercel Function has not been deliberately deployed or runtime-validated; no authorized Firewall or server configuration exists. Normal automatic PR Preview checks may build source, but direct PostgREST closure and live catalog, trusted-IP, throttling, cookie, response-bound, history-ordering, concurrency, cleanup, and session-invalidation proof remain pending. Demo/Production remain untouched. FB-003B stays active, draft, unmerged, and blocked from rollout pending another independent source review plus separately authorized Sandbox SQL, secret/configuration, Firewall, deployment, and live security/abuse-control validation. Anonymous invoice PDFs, immutable/versioned estimate delivery, finalized reports, claimed-account transition, email/SMS, reminders, approval/payment/reply actions, and broader delivery maturity remain future work. |
