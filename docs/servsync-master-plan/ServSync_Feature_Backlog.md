...
| FB-027 | Contractor Pipeline / Follow-Up Lite | Estimates, requests, reminders, contractor dashboard | Started / Partial | Medium-High | Contractor dashboard now has a lightweight `Workflow overview` / `Needs review` summary derived from existing loaded request, appointment, accepted-estimate, invoice, and profile setup state, plus compact navigation-only CTAs for accepted estimates needing jobs, completed/closed jobs ready to invoice, and status-based invoice attention. Saved/focused estimate records include `Create invoice from estimate` for draft/sent/accepted estimates and now show all linked non-void invoices in a contractor summary instead of assuming one active invoice. The action opens an invoice type chooser for total, deposit, progress, or final drafts; deposit drafts ask for a dollar amount or percentage and seed one deposit line, while the other types use the existing editable invoice composer seeded from the estimate with selected `invoice_type`, optional sequence, copied context/line items, draft-inclusive remaining summary, and a warning-only over-estimate notice. The contractor estimate creation cleanup slice makes the normal start choices `Build blank estimate`, `Choose estimate template`, and `Build draft estimator`, keeps blank/template estimates in the normal single-page form, treats templates and Draft Builder as start paths, exposes saved charges and Price Book as contextual line-item sources (`Add saved item`, `Add from Price Book`), starts blank estimates with an empty line-item choice, keeps add controls below existing line rows, hides broader create controls while a draft is active, and puts advanced line metadata behind `More details` without changing persistence, statuses, jobs, invoices, SQL/RLS/RPCs, or homeowner behavior. The multiple-invoice frontend MVP remains contractor-side only: it does not auto-calculate progress/final amounts, enforce remaining balance, track estimate line-level invoicing, change homeowner invoice UI, add payments, or add send/email/SMS/PDF behavior. Future slices should separately consider stale estimates, fuller filtering/queue needs, CTA consolidation, customer-after-estimate save redesign, remaining-balance enforcement, line-level invoicing, multi-invoice scheduling, change orders, Price Book assemblies/autopricing, Draft Builder coverage, Estimate Helper placement, and review-related follow-up only after moderation/public-display policy matures. |
