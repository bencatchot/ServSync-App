...
| FB-011A | Backlog-Aware Partial Invoicing | Jobs, work items, invoices, backlog closeout | Phase 1 Foundation / Sandbox Validated | High | Phase 1 adds SQL/type/test foundation for durable job work items, invoice line linkage, backlog snapshots, and a guarded partial-invoice RPC. Sandbox SQL validation passed; production SQL, contractor UI, homeowner invoice display, PDF backlog sections, backfill, and estimate mapping remain future approval-gated slices. |
