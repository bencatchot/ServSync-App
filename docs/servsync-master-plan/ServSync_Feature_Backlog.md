...
- `Save Draft` is the pause-and-return option, not a required step before direct outcome selection. `Create Job` now follows that model in code by persisting the current Draft state before activation; future Create Estimate and Create Invoice actions should do the same before continuing into their workflows.
...
5. Launch the unified Start New Draft composer outcomes incrementally, beginning with Create Job from the current Draft composer state.
6. Add Create Estimate and Create Invoice from the shared composer.
...
Current next step:
Audit and validate the Create Job-from-Draft slice. Do not ship Create Estimate, Create Invoice, invoice allocation changes, direct-job replacement, or additional contextual Draft entry points until separately approved.
