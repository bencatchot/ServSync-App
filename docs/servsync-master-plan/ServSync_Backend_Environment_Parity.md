...
Final Business Marketing Profile + Planning migration `servsync-business-marketing-profile-plan.sql` (SHA-256 `60ec19e374004cf4e87c2794e99095bc7a99823a463a60b60e326433137077c5`) is applied and validated only in Sandbox as of 2026-08-10. The final RPC definition adds transaction-scoped serialization for concurrent replay of one request UUID. Its exact 174-object catalog addition is recorded as a temporary intentional Sandbox group: four forced-RLS/policy-free tables, nine functions, browser execution only on five platform-admin RPCs, and no direct table grants. Sandbox retains one ServSync-specific internal profile and initial revision, zero plans/revisions, and unchanged zero content/package/event counts. Demo and Production are pending a separate migration-first rollout; the Slice 4 client must not merge for supported-environment runtime before that authorization and rollout complete.
