...
Approved Direction Content Preparation v1 migration `servsync-approved-direction-content-preparation.sql` (SHA-256 `884ef1f93871a293291c3133c64754553c03aa88a518b3ca9a14887505873331`) is applied only in Sandbox. Its 27 strictly additive lineage columns, constraints, indexes, trigger, functions, and grants are recorded as one exact fingerprinted temporary Sandbox group. The changed preparation-package recipe constraint and internal Content list definition remain visible as ordinary logical drift; the manifest does not mask them. Rollback-only six-Direction validation passed with zero Sandbox package/content/event/Direction residue. Demo and Production remain pending; Production/Demo correspondence must be restored migration-first before the client can merge.
