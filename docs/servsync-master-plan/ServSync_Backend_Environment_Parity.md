...
Marketing Direction + Copy Guardrails v1 is part of the supported schema in Sandbox, Demo, and Production after the exact Demo-first/Production-second rollout on 2026-08-10. Its temporary Sandbox-only helper exception is retired. The private `servsync_private_marketing_direction_is_safe(text)` helper, postgres-only grant, and changed `servsync_ingest_internal_marketing_package(uuid,text,text,text,jsonb)` definition must now compare as ordinary supported schema; the manifest does not hide future drift in those objects.
...
Final Business Marketing Profile + Planning migration `servsync-business-marketing-profile-plan.sql` (SHA-256 `60ec19e374004cf4e87c2794e99095bc7a99823a463a60b60e326433137077c5`) is applied and validated in Sandbox, Demo (`2026-08-10T17:31:33Z` through `17:31:42Z`), and Production (`2026-08-10T17:33:46Z` through `17:34:02Z`). The final RPC definition adds transaction-scoped serialization for concurrent replay of one request UUID. All three environments contain the same four forced-RLS/policy-free tables and nine functions, with browser execution only on five platform-admin RPCs and no direct table grants. Each retains one ServSync-specific internal profile and initial revision with zero plans/revisions. Production's two packages, ten content records (three approved and seven draft), sixteen status events, and exact content-history fingerprints were unchanged. Production versus Demo remains `PASS WITH INTENTIONAL DIFFERENCES`; the temporary 174-object Sandbox exception is retired while the unrelated Project Collaboration exception and 22 existing unexplained Sandbox findings remain visible.

Marketing Planner Quality v2 migration `servsync-marketing-planner-quality-v2.sql` (SHA-256 `c05d5e84704d15ccc134970fd71dd297f26e936bbd4091e5a860d40a8ca2800`) is applied in Sandbox, Demo, and Production after the controlled Demo-first/Production-second rollout on 2026-08-10. It replaces the eight-argument planning RPC with an otherwise-compatible nine-argument signature carrying explicit recommendation-contract version, expands the existing Marketing audience constraint, and extends the existing ingestion definition for Truth Pack v3. Production versus Demo remains `PASS WITH INTENTIONAL DIFFERENCES`; no parity-manifest exception was needed or broadened. Production versus Sandbox returns to the 22 unrelated supported-schema findings plus the exact approved Project Collaboration additions; the six temporary Slice 4A findings are retired through ordinary correspondence.

Marketing Planner Coherence + Relevance v3 migration `servsync-marketing-planner-coherence-relevance-v3.sql` (SHA-256 `c7360421519d5bf494a874aa5ec257a428b204e50d624d0f1139d0a1959ed81b`) is applied in Sandbox, Demo, and Production after the controlled Demo-first/Production-second rollout on 2026-08-10. The additive authenticated-only v3 planning RPC is `postgres`-owned, fixed-path, and unavailable to anon and service-role while existing v1/v2 definitions remain exact. Production versus Demo remains `PASS WITH INTENTIONAL DIFFERENCES`; Production's historical planner-v1/v2 and content records remained fingerprint-exact. The temporary planner-v3 Sandbox exception is retired without changing the unrelated Project Collaboration exception or existing unexplained findings.

Accepted Plan Marketing Directions v1 migration `servsync-accepted-plan-marketing-directions.sql` (SHA-256 `165208592583b97cbd7abe95fb3ef95ff8adff93141ea5a4038cedd1f45cae93`) is applied in Sandbox, Demo, and Production after the controlled Demo-first/Production-second rollout on 2026-08-11. The two private tables and four authenticated platform-admin RPCs correspond across all three environments; rollback-only six-item preparation, exact lineage, replay, revisions, approval, role denial, and zero downstream content effects passed in both targets. Production retained zero real Directions/revisions and exact Plan, Profile, package, content, status-event, and sampled business fingerprints. The temporary Slice 5 Sandbox exception is retired while unrelated Sandbox drift remains visible.

Marketing Media Assets v1 migration `servsync-marketing-media-assets.sql` (SHA-256 `4a516ad53ac2d5e75c2ed70103616c3dcce247c6c651604c240601f72b07fbf9`) is applied in Sandbox, Demo, and Production after the controlled Sandbox-first rollout on 2026-08-16. Each environment has one private 100 MB MP4-only `marketing-assets` bucket, three forced-RLS tables with zero direct grants, immutable asset/pairing identity, append-only media-review history, and authenticated platform-admin RPCs. Registration plus exact approved-Content-revision pairing is one transaction. Sandbox and Demo retained zero media/pairing rows; Production preserved the exact 16-content/3-package/34-status-event fingerprint and zero publication/event counts with Facebook publishing still disabled. Provider media publishing remains absent.

Narrated Marketing Media + Publication Snapshot v1 migration `servsync-narrated-marketing-media-publication.sql` (SHA-256 `e165fb94f6214bdfdee68204694eee5af6658d639ce0ef88c4c3526c2d261167`) is applied in Sandbox, Demo, and Production after the controlled Sandbox-first rollout on 2026-08-16. It adds a bounded silent-master/narrated-derivative distinction, non-secret narration provenance, public-disclosure metadata, approved-pairing carry-forward for status-only Content approval, and an immutable exact-media publication snapshot. Sandbox and Demo remained empty; Production preserved its existing two assets/two pairings and zero publications/events before one separately authorized owner-approved flagship derivative was added through the managed bucket and authenticated RPC. Public-post gates and provider capabilities did not change.

Approved Direction Content Preparation v1 migration `servsync-approved-direction-content-preparation.sql` (SHA-256 `884ef1f93871a293291c3133c64754553c03aa88a518b3ca9a14887505873331`) is applied in Sandbox, Demo, and Production after the controlled Demo-first/Production-second rollout on 2026-08-11. Its additive lineage columns, constraints, indexes, trigger, functions, grants, changed preparation-package recipe constraint, and changed internal Content list definition now compare as ordinary supported schema. The temporary Sandbox exception is retired. Rollback-only six-Direction validation passed in both targets; Demo retains zero Marketing execution rows, while Production retains its exact historical packages, content, events, Directions, and unrelated-business fingerprints with zero first-class lineage rows.
