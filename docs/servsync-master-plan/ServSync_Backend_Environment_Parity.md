...
Read-only validation after Sandbox Legacy Storage Policy Containment v1 on 2026-08-07 established:
...
Stripe Connect Online Payments Foundation v1 is a second exact Sandbox experiment group. Its 178 entries cover only the three private payment relations, 15 fixed-signature functions and grants, 53 columns, 42 constraints, 12 indexes, two cross-scope guard triggers, and owner table grants identified by the reviewed migrations. The key fingerprint is `sha256:d675e9797fc6b24002e8055547463d5b4a0bcddfcd60a73eefeae2e38be5dba7` and the logical catalog fingerprint is `sha256:1e1fbf7a2ab49fe1a8898879c08f68f24fdff1854a32150a025d0413f9457367`, including the reviewed `ch_`/`py_` provider-payment identifier compatibility constraint. It does not approve changes to Production-supported Invoice, offline-ledger, Customer, tenant, ACL, or policy objects. Provider-backed behavior remains fail-closed outside the approved Sandbox TEST configuration.

With Project Collaboration's 297 entries and the Stripe test foundation's 178 entries approved exactly, Production versus Sandbox still fails on the same 22 unrelated supported-schema findings. Those consist of missing legacy Stripe subscription columns/indexes/functions/grants; two differing service-request-media policies; and differing notification, support, appointment, and field-work function definitions/grants. Any repair or additional experiment approval requires a separate audit and repository/configuration decision, with SQL authorization where applicable.
