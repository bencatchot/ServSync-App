...
Read-only validation after the provider-neutral Stripe foundation rollout on 2026-08-08 established:

- Production vs Demo: `PASS WITH INTENTIONAL DIFFERENCES`. All 104 Production relations, the bounded `storage.objects` security fingerprint, 1,446 columns, 885 constraints, 459 indexes, 110 triggers, 218 policies, 349 functions, 1,056 function grants, 2,185 table grants, 80 column grants, and 96 expanded default-ACL grants match. Demo adds one exact fingerprinted 129-object scenario group: 3 reviewed scenario relations, 6 reviewed operator functions, and 120 supporting catalog entries.
