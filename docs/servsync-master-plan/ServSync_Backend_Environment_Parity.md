...
Marketing Planner Quality v2 migration `servsync-marketing-planner-quality-v2.sql` (SHA-256 `c05d5e84704d15ccc134970fd71dd297f26e936bbd4091e5a860d40a8ca2800`) is applied only to Sandbox `zpzdkoaubyjtsomccxya` from `2026-08-10T18:55:42.015Z` through `18:55:42.590Z`. It replaces the eight-argument planning RPC with an otherwise-compatible nine-argument signature carrying explicit recommendation-contract version, expands the existing Marketing audience constraint, and extends the existing ingestion definition for Truth Pack v3. The parity guard intentionally does not classify replacements or logical drift as additive experiments. Production versus Sandbox therefore reports six exact new Slice 4A findings in addition to the 24 findings already visible immediately before this migration: one changed audience constraint, one changed ingestion function, one missing old planning function, one new planning function, and its two new grants. No parity-manifest exception was broadened. Demo and Production remain on planner v1 and Truth Pack/recipe v2 until a separately authorized migration-first rollout.
