...
The signup-function difference is a separate priority security correction, not an exception to approve. Production and Demo currently retain an older profile-creation trigger that does not adequately constrain self-service role metadata. `servsync-public-signup-role-hardening.sql` is the canonical reconciliation: it permits only homeowner and contractor public account roles, defaults all other metadata to homeowner, preserves referral attribution, prevents authenticated profile-role rewrites, and is final in both blank-install sequences. The corrected migration (SHA-256 `4fe4f48a57cfeb3a46a07bcfe67f2e716ea9be29e369324da225b244d4be6f7a`) is installed and authenticated in Sandbox `zpzdkoaubyjtsomccxya`. Validation found and corrected an unqualified permanent-referral random-byte call that was incompatible with Supabase's fixed-path extension layout; the canonical function now uses an explicitly qualified PostgreSQL catalog UUID source while preserving the 12-character invite-code contract. Production and Demo remain unchanged and require the same exact migration in one coordinated security rollout before merge.
