...
Slice 2B packages the durable backend Draft Launch Foundation for audit and sandbox-first approval. It introduces the proposed dedicated `contractor_work_drafts`, `contractor_work_draft_items`, and private success-only `contractor_work_draft_launches` model, durable intended output, private Draft notes and subject audit snapshots, consumed state, deletion-compatible private-ledger linkage to launched Estimates or Jobs, one-Draft/one-launch constraints, contractor-serialized idempotency, non-enumerating tenant authorization, launch-time relationship locking/revalidation, legacy-import relationship validation before snapshotting, Job property-sharing parity, shared database-level contractor/service-request Job deduplication, contractor-history retention, contractor-only RLS/grants, canonical save/resume RPCs, and an atomic launch RPC. Live output FKs may become null when an eligible output is physically deleted, while immutable private output-ID snapshots retain the original audit identity; the consumed Draft remains consumed and retries cannot create a replacement. Private Draft IDs are not stored on homeowner-readable Estimate or Job rows. Current Estimate, Job, and Draft permission checks are compatibility boundaries with the corresponding existing workflows, not a finalized employee-role or configurable company-permissions model. Typed UUID RPC parameters retain PostgREST/PostgreSQL argument coercion before ServSync function error handling. The Slice 2B SQL is not applied to sandbox or Production by the implementation PR, no launch UI is exposed, another focused read-only audit is required before sandbox application, and Slice 2C remains the first planned coherent Preview-visible workflow with both Create Estimate and Create Job.
