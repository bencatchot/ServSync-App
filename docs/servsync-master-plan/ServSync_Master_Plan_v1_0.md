...
| Job | Jobs are backed by inspections table; simple service jobs and checklist/report jobs exist. Contractor-facing terminology has been partially cleaned up, while internal schema names may still use inspection terminology. Partial-invoicing foundation work adds durable job work item records, and the first UI wiring now displays durable work items on contractor job detail when those rows exist. Phase 2B-1 adds a pending SQL patch so accepted-estimate job creation can seed durable work items from estimate line items after environment-specific SQL approval. Jobs without durable work items continue using existing task/checklist behavior without fake migration or global backfill. | Continue reducing user-facing inspection language where it confuses service work, validate estimate-to-work-item seeding in sandbox/production before treating it as live, and separately plan simple-service task sync, manual work item creation, inspection finding conversion, and any legacy conversion/backfill. |
