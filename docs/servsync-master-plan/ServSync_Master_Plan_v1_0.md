...
| Contractor local customers | Contractor-created local customers exist for customers who are not yet on ServSync. The first local multi-property slice lets contractors save and view multiple local properties for one local customer and requires explicit property selection before starting property-specific work when more than one local property exists. Contractors can update unclaimed contractor-created local property labels, addresses, and private notes through a guarded RPC while blocking edits after the local property is linked to a homeowner profile. The next local customer profile edit slice lets contractors update unclaimed local customer name, phone, email, and contractor-private notes only; claimed/homeowner-linked customer profiles remain homeowner-controlled, and edits to copied claim-invite fields revoke pending claim invites so stale links do not remain active. The connected-homeowner property proposal flow now has the consent-first table/RPC foundation, contractor-side suggest/revoke UI, and homeowner-side review UI so homeowners can explicitly accept suggestions into a new or existing home, choose whether to share accepted properties with the suggesting contractor, or reject suggestions. Pending/rejected/revoked suggestions remain unusable for work records. | Homeowner local-customer claim/accept remains single-property for now; multi-property homeowner claim, invite, notification delivery, operational document delivery authorization, and any email/SMS/push behavior must be planned separately before expanding those flows. |
