...
`Save Draft` is the pause-and-return option, not a required step before choosing an outcome. A contractor should be able to enter work once and proceed directly from the current composer state to `Create Estimate`, `Create Job`, or `Create Invoice` when those outcome actions are implemented. Each outcome action should persist the current Draft state before continuing into the selected workflow, without requiring the user to save, exit, and reopen the Draft first.
