...
The official feature backlog has been structurally reconciled through PR #349 / main SHA `74498d90466af4f134c170fd747fc2f8130624bd`. `docs/servsync-master-plan/ServSync_Feature_Backlog.md` now tracks unfinished outcomes only, while `docs/servsync-master-plan/ServSync_Completed_Features.md` archives completed v1 milestones and `docs/servsync-master-plan/CHANGELOG.md` remains the detailed implementation and rollout history. Backlog and archive wording must continue to separate live beta capabilities from source-only, Sandbox-only, gated, cohort-bound, or future capabilities.
...
| Contractor local customers | Contractor-created local customers exist for customers who are not yet on ServSync. The first local multi-property slice lets contractors save and view multiple local properties for one local customer and requires explicit property selection before starting property-specific work when more than one local property exists. Contractors can update unclaimed contractor-created local property labels, addresses, and private notes through a guarded RPC while blocking edits after the local property is linked to a homeowner profile. The local customer profile edit slice lets contractors update unclaimed local customer name, phone, email, and contractor-private notes only; claimed/homeowner-linked customer profiles remain homeowner-controlled, and edits to copied claim-invite fields revoke pending claim invites so stale links do not remain active. The claim-invite delivery hardening source adds a guarded manual link/QR preparation boundary for contractor owner/admin/office users, keeps field tech/viewer users from preparing or revealing claim tokens, moves normal invite reads toward token-free metadata, hardens create/lookup eligibility, and keeps email/SMS/provider delivery explicitly excluded until separately approved and deployed. The multi-property local-customer claim redesign source adds an explicit invitation-to-local-property membership model, lets contractors select the exact unclaimed local properties included in one claim invite, and gives homeowners a per-property create-or-match review before atomic acceptance. Existing single-property invites remain compatible, and older accept clients reject multi-property invitations before mutation. The connected-homeowner property proposal flow now has the consent-first table/RPC foundation, contractor-side suggest/revoke UI, and homeowner-side review UI so homeowners can explicitly accept suggestions into a new or existing home, choose whether to share accepted properties with the suggesting contractor, or reject suggestions. Pending/rejected/revoked suggestions remain unusable for work records. | Multi-property local-customer claim still needs owner-reviewed Production release gates after Sandbox/preview readiness; notification delivery, email/SMS/provider delivery, and broader operational document delivery authorization remain separate approval-gated slices. |
