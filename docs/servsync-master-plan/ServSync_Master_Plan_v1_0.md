...
The approved Slice 2C architecture audit split frontend adoption into four bounded slices. Slice 2C-A adds hidden typed durable save/get/list/import/launch adapters, structured error normalization, canonical mapping and item-identity utilities, helper-derived compatibility capabilities, and privacy-safe per-Draft idempotency-attempt storage without wiring `App.tsx`, the Composer, routes, lists, or visible actions. Its final safety correction validates complete canonical RPC and stored-attempt state, removes positional item-ID reconciliation, and fails closed when durable browser retry storage is unavailable. PR #318 remains open and draft pending an abbreviated read-only re-audit, and Slice 2C-B has not started. Slice 2C-B will make durable save/resume/list/import and consumed presentation canonical. Slice 2C-C will add coherent Estimate/Job launch UX, idempotency execution, errors, and navigation. Slice 2C-D will perform sandbox/Preview runtime and mobile validation plus documentation closeout. No 2C-A SQL, gate, environment, Vercel, sandbox, or Production change is included, and the user-visible Draft-first workflow remains incomplete.
