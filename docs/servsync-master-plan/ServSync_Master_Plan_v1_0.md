...
| Invoices | Working, including invoice-to-Home-History filing v1 | Payment processing and contractor-side filing remain deferred. Structured invoice type/order and estimate payment schedule foundations exist for future billing terms, and contractor draft estimates can now store structured schedule rows that appear on estimate PDFs. Homeowner in-app schedule display and invoice generation from schedule are not live. |
