...
FB-032 Service Agreements Foundation is a future contractor maintenance-plan workflow. Slice 2 packages a SQL/RLS/RPC foundation for contractor-owned templates, property-scoped offers to connected homeowners, homeowner accept/decline, and accepted agreement snapshots. The SQL was applied and probed in sandbox only; production SQL apply remains a separate approval step before UI work. Service Agreements remain separate from requests, estimates, jobs, invoices, scheduling, reminders, notifications, payments, renewals, e-signatures, and external delivery until later explicit slices are approved.
