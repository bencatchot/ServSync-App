...
| Invoice | Invoices exist. Homeowner invoices can be filed to Home History through a durable invoice relationship without duplicate PDF storage. Partial-invoicing UI wiring can create draft invoices from selected completed, billable, priced durable job work items and shows invoice backlog snapshots separately in contractor invoice cards, homeowner invoice cards, and invoice PDFs. Backlog rows are context only and do not affect subtotal, tax, or total. The whole-job invoice path remains a fallback for legacy/no-work-item jobs, while the guardrail slice routes work-item-backed jobs toward item-based invoicing and adds a SQL patch to reject new whole-job invoice creation when durable work items exist. FB-020 immutable invoice hardening now treats direct invoice and invoice-line edits as draft-only, leaving customer-facing status changes to explicit RPC transitions such as send, homeowner view, mark paid, and void. A multiple-invoices foundation adds structured invoice purpose/order fields (`invoice_type`, nullable `invoice_sequence`) for total/deposit/progress/final workflows. The first contractor-side frontend MVP now shows all non-void invoices linked to an estimate, opens an invoice type chooser from `Create invoice from estimate`, asks for a dollar/percentage amount before starting deposit drafts, seeds deposit drafts with one deposit line, seeds other invoice types through the existing editable composer with copied estimate context/lines, and shows draft-inclusive remaining summaries plus warning-only over-estimate copy. | Keep invoice actions focused, test invoice-to-history filing, and continue work-item seeding without changing invoice totals or removing the existing whole-job invoice fallback for legacy jobs. Defer full PDF/final snapshot strategy, broader accounting audit logs, automatic progress/final calculations, remaining-balance enforcement, line-level estimate item invoicing, payment behavior, homeowner invoice redesign, and multi-invoice scheduling until separately approved. |
...
- Contractor should see clear next actions instead of hunting across tabs. Recent cleanup work made request-to-estimate, accepted-estimate-to-job, completed-job-to-invoice, and service/checklist terminology clearer without changing stored status values or internal database names. Saved/focused estimate records support an estimate-led `Create invoice from estimate` action for draft/sent/accepted estimates; the action now uses a total/deposit/progress/final type chooser, asks for a dollar/percentage amount before opening deposit drafts, and uses the existing editable invoice composer without requiring an ordinary service contractor to create or visit a job first. Linked invoice summaries are contractor-side only and do not add payment collection, Home History filing, homeowner invoice redesign, send/email/SMS/PDF changes, or new SQL/RLS/RPC changes in the frontend MVP.
