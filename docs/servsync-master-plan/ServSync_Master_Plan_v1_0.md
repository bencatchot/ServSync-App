...
The approved Slice 2C architecture audit split frontend adoption into four bounded slices. PR #318 is merged and Slice 2C-A provides hidden typed durable save/get/list/import/launch adapters, structured error normalization, canonical mapping and item-identity utilities, helper-derived compatibility capabilities, privacy-safe per-Draft idempotency-attempt storage, complete canonical response validation, non-positional reconciliation, and fail-closed storage handling. Slice 2C-B exists as open draft PR #319 and integrates dedicated durable state as authoritative only when Shared Draft Composer + Draft Job are enabled and Demo Presentation is off: the gated Jobs workspace supports atomic save, canonical resume, active/consumed lists, lazy legacy import and exact de-duplication, contractor-private notes, and read-only consumed/discarded/deleted-output presentation. Its audit correction prevents targetless editor deadlocks, canonicalizes first-save/import identity, rejects stale contractor/editor responses, clears incompatible hidden service-request identity, adds guarded recovery actions, and exercises core races through rendered tests. It adds no launch controls and does not call launch or launch-attempt mutation utilities. Slice 2C-C has not started and remains coherent Estimate/Job launch UX, idempotency execution, errors, and navigation. Slice 2C-D remains authenticated sandbox/Preview runtime and mobile validation plus documentation closeout. Gates remain default-off; no Slice 2C-B SQL, Supabase mutation, environment, Vercel, sandbox, or Production change is included, and the full Draft-first workflow remains incomplete.
