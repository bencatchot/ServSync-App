...
Durable Draft contractor rollout must add a tenant-specific default-deny exposure decision without changing backend Draft authority. The source implementation on `codex/durable-draft-cohort-gating-v1` proposes `contractor_billing_accounts.durable_draft_beta_enabled boolean NOT NULL DEFAULT false` plus a narrow owner/active-team-member RPC. The frontend may show durable Work only when all three existing strict global gates are true, Demo Presentation is off, a current contractor/session entitlement resolves true for the exact tenant, and the existing independent persistence or output authority permits each action. Loading, missing, invalid, foreign, stale, and failed entitlement states show legacy Jobs. Platform-admin status alone is not contractor context. The entitlement SQL is source-only and unapplied; no contractor is enrolled. SQL application, exact-UUID enrollment, Sandbox validation, Production application, and gate rollout each require separate approval, and structured privacy-safe telemetry remains required before external beta.
