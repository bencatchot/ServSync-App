...
FB-026 now includes a contractor-to-contractor referral path as a distinct trust/growth track. The production-applied SQL/RLS/RPC foundation stores `contractor_refers_contractor` referral invite leads submitted by contractor owner/admin/office users and reviewed by platform admins. The first contractor-facing slice adds only a compact submit UI in the contractor `Invites & Referrals` area so eligible contractors can send another contractor's details to the ServSync team for follow-up. This remains intentionally separate from homeowner contractor invite leads, contractor-to-homeowner invites, reward referral codes, contractor team invites, connection requests, service requests, email delivery, rewards, billing, paywalls, contractor permissions, contractor team access, public referral links, and automatic contractor profile creation. Platform-admin tracking UI, email outreach, public landing pages, rewards, and any signup attribution beyond nullable admin match fields remain future slices requiring separate approval.
