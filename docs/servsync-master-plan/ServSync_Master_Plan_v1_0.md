...
| Job | Jobs are backed by inspections table; simple service jobs and checklist/report jobs exist. Contractor-facing terminology has been partially cleaned up, while internal schema names may still use inspection terminology. Partial-invoicing foundation work adds durable job work item records, and the first UI wiring now displays durable work items on contractor job detail when those rows exist. Phase 2B-1 seeds durable work items from accepted estimate line items after environment-specific SQL approval. Phase 2B-2 adds a pending SQL/RPC/app foundation so simple service `Work Items` tasks can sync into durable source-backed work items after environment-specific SQL approval, without converting general inspection findings or globally backfilling legacy jobs. Phase 2C-1 adds a sandbox-validated SQL/RPC/app foundation for contractor-created manual work items, guarded so only safe unbilled manual items can be edited or removed; production SQL rollout remains pending separate approval. Jobs without durable source-backed rows continue using existing task/checklist behavior without fake migration or global backfill. | Continue reducing user-facing inspection language where it confuses service work, validate simple-service task sync and manual work-item editing in sandbox/production before treating them as live, and separately plan inspection finding conversion and any explicit legacy conversion/backfill. |
...
| Invoice | First-class invoices exist. Homeowner invoices can be filed to Home History through a durable invoice relationship without duplicate PDF storage. Partial-invoicing UI wiring can create draft invoices from selected completed, billable, priced durable job work items and shows invoice backlog snapshots separately in contractor invoice cards, homeowner invoice cards, and invoice PDFs. Backlog rows are context only and do not affect subtotal, tax, or total. Phase 2B-1 hardens linked work-item invoice validation so Price Required work items cannot be billed through API/table bypasses once the SQL patch is applied. | Keep invoice actions focused, test invoice-to-history filing, and continue work-item seeding without changing invoice totals or removing the existing whole-job invoice path. |
