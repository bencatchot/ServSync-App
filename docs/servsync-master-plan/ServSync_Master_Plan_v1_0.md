...
The Draft-first Inspection Checklist Job path is implemented as hidden source work in draft branch `codex/draft-inspection-checklist-job-path-v1`. It adds a separate `inspection_checklist` work format for private durable Drafts, lets a contractor snapshot a starter, contractor-owned, or eligible Home-specific Inspection Checklist source, and routes checklist Draft save/launch through dedicated checklist-specific RPCs. Home-specific checklist eligibility now requires the exact selected property, starter snapshots must match the canonical source catalog, launched checklist findings start in neutral `Monitor` state rather than pre-answered `Pass`, and duplicate checklist room/item structures fail validation. The additive SQL source remains unapplied, no gate is enabled, no operational report or Job is created before launch, Create Estimate for checklist Drafts is not implemented, and direct Draft-to-Invoice remains deferred.
