...
Slice 2B implements the durable backend Draft Launch Foundation in merged PR #317 (`895d14c4f50435b079b02c34e63aa02238a2c755`). It introduces the dedicated `contractor_work_drafts`, `contractor_work_draft_items`, and private success-only `contractor_work_draft_launches` model, durable intended output, private notes and subject snapshots, consumed state, deletion-compatible private linkage and immutable output snapshots, one-Draft/one-launch constraints, idempotency, tenant authorization, relationship locking/revalidation, legacy import, Job property-sharing parity, operational service-request Job deduplication, retention, contractor-only RLS/grants, save/get/import RPCs, and atomic Estimate/Job launch. ServSync Sandbox has both the foundation and its permission-parity correction installed. The full runtime matrix passed one-Draft/one-launch, adversarial rollback, concurrency, relationship revocation and property-unsharing locking, deletion/retry, privacy, tenant isolation, RLS, grants, PostgREST, retention, and owner/admin/office/field-technician/viewer/platform-admin permission parity; all disposable fixtures were removed and no known Slice 2B runtime blocker remains. Draft persistence uses the union of existing billing-management and Job-write helpers, while Estimate and Job launch remain output-specific. These helpers remain compatibility mechanisms, not permanent role policy, and no universal field-technician Estimate restriction was introduced. The foundation remains hidden: no visible Create Estimate or Create Job action was added and no feature gate was enabled. Production remains untouched on the legacy workflow. Fresh environments, including Production only after separate approval, should apply corrected canonical foundation hash `ac9e600ece3075e2d171da5571aab2b26e7a1f6f234239b02194fa7be3d2354f` and must not apply the correction afterward. Sandbox received the earlier foundation before the capability-union fix and therefore subsequently received correction hash `b4a98f33acd99083bea4497268393f6277ef330cdcb31bdc5d253adb94b14c7f`; neither SQL file should be reapplied there.

The approved Slice 2C architecture audit split frontend adoption into bounded stages. PR #318 is merged and Slice 2C-A provides typed durable adapters, strict canonical validation, mappings, capability evaluation, and privacy-safe fail-closed attempt storage. PR #319 is merged and Slice 2C-B makes dedicated durable save/resume/list/import state authoritative only when Shared Draft Composer + Draft Job are enabled and Demo Presentation is off, with active/consumed presentation and guarded existing-output opening. PR #320 is merged and Slice 2C-C1 provides explicit Estimate/Job confirmation, canonical save-before-launch, refreshed output-specific authority, verified idempotency attempts/removal, same-key ambiguous recovery, fail-closed lifecycle/cross-tab reconciliation, strict launch-state transitions, canonical consumed adoption, list synchronization, and explicit Open Output. Slice 2C-C2 is implemented in open, draft, unmerged PR #321: same-session confirmed Create and explicit Retry can create a one-time memory-only navigation intent only after canonical consumed adoption; existing side-effect-free loaders and synchronous adoption remain shared with manual Open; complete Estimate/Job records are strictly runtime-validated before adoption; malformed records remain on the consumed fallback; and deferred focus is exact-output and operation-token owned, including real production-path same-family and cross-family race coverage. External, mount, list, lifecycle-only, and background recovery remain explicit-only; navigation failure preserves the consumed Draft without relaunch or a new key; stale Back/unmount/contractor/client/target/gate contexts cannot adopt or focus an output. The corrected C2 suite contains 33 pure, 78 rendered, 3 production focus-integration, and 1 source-integrity case, for 115 unique tests. C2 adds no Draft-to-Invoice path. Slice 2C-D still owns authenticated Sandbox/Preview runtime, physical-device mobile validation, and closeout. Gates remain default-off, Production durable Draft SQL remains unapplied, and no SQL, RPC, Supabase, environment, Vercel, Sandbox, Production, or visible Production change is included.
