...
Slice 2B packages the durable backend Draft Launch Foundation for audit and sandbox-first approval. It introduces the proposed dedicated `contractor_work_drafts`, `contractor_work_draft_items`, and `contractor_work_draft_launches` model, durable intended output, private Draft notes, consumed state, source-Draft linkage to launched Estimates or Jobs, one-Draft/one-launch constraints, idempotency rules, contractor-only RLS/grants, canonical save/resume RPCs, and an atomic launch RPC. The Slice 2B SQL is not applied to sandbox or Production by the implementation PR, no launch UI is exposed, and Slice 2C remains the first planned coherent Preview-visible workflow with both Create Estimate and Create Job.
