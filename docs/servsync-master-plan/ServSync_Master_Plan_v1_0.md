...
| Invoice | First-class invoices exist. Homeowner invoices can be filed to Home History through a durable invoice relationship without duplicate PDF storage. Partial-invoicing UI wiring can create draft invoices from selected completed, billable, priced durable job work items and shows invoice backlog snapshots separately in contractor invoice cards, homeowner invoice cards, and invoice PDFs. Backlog rows are context only and do not affect subtotal, tax, or total. | Keep invoice actions focused, test invoice-to-history filing, and later add reviewed work-item seeding/backfill and estimate-to-work-item mapping without changing invoice totals. |
