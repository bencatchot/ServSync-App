...
| Contractor local customers | Contractor-created local customers exist for customers who are not yet on ServSync. The first local multi-property slice lets contractors save and view multiple local properties for one local customer and requires explicit property selection before starting property-specific work when more than one local property exists. The next narrow edit slice lets contractors update unclaimed contractor-created local property labels, addresses, and private notes through a guarded RPC while blocking edits after the local property is linked to a homeowner profile. | Homeowner claim/accept remains single-property for now; multi-property homeowner claim, invite, and connection handling must be planned separately before expanding that flow. Contractor-proposed properties for already connected homeowners remain future work and should preserve homeowner ownership/control. |
