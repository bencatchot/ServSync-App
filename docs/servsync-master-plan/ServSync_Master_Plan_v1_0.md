...
Slice 2B packages the durable backend Draft Launch Foundation for audit and sandbox-first approval. It introduces the proposed dedicated `contractor_work_drafts`, `contractor_work_draft_items`, and private success-only `contractor_work_draft_launches` model, durable intended output, private Draft notes, consumed state, private-ledger reverse linkage to launched Estimates or Jobs, one-Draft/one-launch constraints, contractor-serialized idempotency, contractor-only RLS/grants, canonical save/resume RPCs, and an atomic launch RPC. Private Draft IDs are not stored on homeowner-readable Estimate or Job rows. Current Estimate, Job, and Draft permission checks are compatibility boundaries with the corresponding existing workflows, not a finalized employee-role or configurable company-permissions model. The Slice 2B SQL is not applied to sandbox or Production by the implementation PR, no launch UI is exposed, another focused read-only audit is required before sandbox application, and Slice 2C remains the first planned coherent Preview-visible workflow with both Create Estimate and Create Job.
