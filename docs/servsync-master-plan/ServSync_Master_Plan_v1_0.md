...
- Home Setup should grow from structured property data rather than floor-plan/CAD tooling. The lightweight Home Setup guide shell is live, and the first homeowner Rooms UI lets owner/admin home managers add, edit, and soft-archive non-sensitive `home_rooms` basics while shared member/viewer roles see read-only room basics where surfaced. Reminder-room UI lets homeowners optionally tag newly created Home Reminders to active room basics, shows non-sensitive room chips on homeowner-owned reminder cards, and locally filters the selected-property Home History reminder list by room, while shared reminder shells, contractor visibility, edit-reminder room changes, and reminder grouping remain deferred. The document-room backend/upload foundations add optional same-home room links for `home_documents` and manual upload registration, and the first homeowner document-room UI lets homeowners tag newly uploaded manual documents to active rooms and see non-sensitive room chips on homeowner-owned document cards. The first homeowner-owned Room Detail panel lets owner/admin homeowners treat rooms as simple organizing objects by viewing room basics, manager-visible notes, and compact linked reminder/document lists from existing loaded state only. The `home_assets` foundation is the first backend assets/systems step after durable rooms, adding optional same-home room links and owner/admin-only asset management semantics without asset UI, shared member/viewer browsing, contractor visibility, serial numbers, sensitive access fields, or migration/backfill. Room notes are not shown with documents, and storage bucket/policy changes, shared-home Room Detail or document expansion, contractor visibility, contractor Room Detail, document edit-room assignment, document filters, photo tagging, report/job media tagging, Key Home Locations, Home Map media context, and floor-plan behavior remain deferred. Future Systems & Assets UI, Key Home Locations, Home Map, and Home Setup Templates should build on durable rooms/assets/location models in separate slices; Home Map should start as a simple not-to-scale room/system map, not measured floor-plan generation, LiDAR scanning, CAD, or 3D modeling.
