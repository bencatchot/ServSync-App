...
Slice 2B packages the durable backend Draft Launch Foundation for sandbox-first approval. It introduces the dedicated `contractor_work_drafts`, `contractor_work_draft_items`, and private success-only `contractor_work_draft_launches` model, durable intended output, private Draft notes and subject audit snapshots, consumed state, deletion-compatible private-ledger linkage to launched Estimates or Jobs, one-Draft/one-launch constraints, contractor-serialized idempotency, non-enumerating tenant authorization, launch-time relationship locking/revalidation, legacy-import relationship validation before snapshotting, Job property-sharing parity, shared database-level contractor/service-request Job deduplication, contractor-history retention, contractor-only RLS/grants, canonical save/resume RPCs, and an atomic launch RPC. Live output FKs may become null when an eligible output is physically deleted, while immutable private output-ID snapshots retain the original audit identity; the consumed Draft remains consumed and retries cannot create a replacement. Private Draft IDs are not stored on homeowner-readable Estimate or Job rows. The reviewed foundation hash was applied once to ServSync Sandbox and passed the completed runtime matrix except for a Draft-persistence permission-parity defect: Job-authorized users could create normal Jobs but could not save the durable Draft required by Draft-first Job launch. The corrected canonical foundation permits Draft persistence through the union of existing billing-management and Job-write capability helpers, without role-name checks; Estimate and Job launch remain separately governed by their corresponding normal creation boundaries. PR #317 remains open and draft. A narrow correction SQL package for the already-installed sandbox remains packaged but unapplied; its exact-predecessor fingerprint includes public RPC defaults so default-expression drift blocks application. Another short read-only audit and owner approval are required before application, and the focused owner/admin/office/field-technician/viewer/platform-admin permission matrix must run afterward. Production remains untouched and will eventually use the corrected canonical foundation rather than the sandbox correction patch. Current permission helpers are compatibility behavior, not a finalized employee-role or configurable company-permissions model. No launch UI is exposed, and Slice 2C remains the first planned coherent Preview-visible workflow with both Create Estimate and Create Job.
