...
| --- | --- | --- |
| Homeowner Request | Homeowners can create requests tied to homes/properties. Direct connected-contractor flow exists. Recent UI copy clarifies what happens after a request is sent and after an estimate is accepted. FB-021 Scheduling + Appointment Confirmation Foundation Phase 1 adds a SQL/RPC/RLS foundation for contractor-proposed appointment windows, homeowner acceptance, contractor reschedule/cancel, and appointment event history while keeping existing single appointment summaries for compatibility. Phase 2 wires read-only request-card display for confirmed appointment summaries and pending proposed visit windows on homeowner and contractor request surfaces; proposal/accept/decline/reschedule controls remain later slices. | Harden end-to-end coverage and beta QA; wire appointment-window action UI in small follow-up slices without jumping to calendar sync, dispatch, or routing automation. |
