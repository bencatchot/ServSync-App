...
The approved Slice 2C architecture audit split frontend adoption into bounded stages. PR #318 is merged and Slice 2C-A provides typed durable adapters, strict canonical validation, mappings, capability evaluation, and privacy-safe fail-closed attempt storage. PR #319 is merged and Slice 2C-B makes dedicated durable save/resume/list/import state authoritative only when Shared Draft Composer + Draft Job are enabled and Demo Presentation is off, with active/consumed presentation and guarded existing-output opening. Slice 2C-C is further split: C1 now exists in open, draft, unmerged PR #320, adding explicit Estimate/Job confirmation, canonical save-before-launch, refreshed output-specific authority, verified idempotency attempts and removal, same-key ambiguous recovery, fail-closed lifecycle/cross-tab reconciliation, strict launch-state transitions, canonical consumed adoption, list synchronization, and explicit Open Output only. Its final corrections make lifecycle and canonical-reconciliation locks outrank external prepared/launching/ambiguous recovery, make Workspace ownership mutation depend on the reducer-derived transition decision, preserve locally owned pending work when weaker, different-key, or different-family recovery is rejected, preserve validated external-success reconciliation, and add rendered stale response, verified-discard, intent-mismatch, Back, unmount, client/target/listener, and local-pending external-attempt coverage. C1 does not auto-open the created output. C2 remains automatic post-launch navigation plus final navigation/mobile/accessibility polish, and Slice 2C-D remains authenticated Sandbox/Preview runtime and mobile validation plus closeout. Gates remain default-off; no SQL, Supabase, environment, Vercel, sandbox, or Production change is included, and the full Draft-first workflow remains incomplete.
