...
Planning base: `origin/main` at `97f529db9783fb12c04a9b43b65b9682932bd48c`
...
| Primary active outcome | FB-040A Tutorial Readiness — TUT-006 homeowner review/Home History tutorial |
| Completed control-point evidence | Phase 0 is complete. PR #521 merged Phase 0.7 at main commit `0bddbb20682bbcac7945150e6f2d70e57e54aed7`. TUT-001, TUT-003, TUT-004, and TUT-005 are published under the narration/caption standard. TUT-005 Production revision 1 passed exact package ingestion, complete `1x` sound-on playback, muted top-safe caption review, transcript/disclosure verification, immutable homeowner-only audience inspection, and published-media health checks. TUT-002 remains visually current but still needs its standards upgrade. The controlled-pilot inventory now contains five published walkthroughs and one missing protected workflow. See [Pilot Tutorial Coverage](ServSync_Pilot_Tutorial_Coverage.md). |
| Next recommended assignment | Complete TUT-006 **How to review work and keep Home History** for `homeowner.records`, then upgrade TUT-002 to the narration/caption/transcript standard before the pilot onboarding dry run. Reuse the established bounded recording, one-request Cedar, compact caption, Help Studio review, publication, and role-aware verification path. |
| Assignment outcome | TUT-005 **How to connect and request service** is published and verified as immutable Production revision 1. Final request `b417c413-9cd5-4950-883d-b903483134de` accepted the 119.2-second narrated package, 16 synchronized 10-14-word English cues, matching transcript, Cedar disclosure, and exact validated Demo provenance. Full sound-on and muted review passed, the visible flow ends **Waiting on contractor**, published playback is healthy, and immutable audience is exactly Homeowner at `homeowner.service`. The sole successful Cedar key is revoked to **Inactive** and no credential remains in ServSync or repository artifacts. See [Pilot Tutorial Coverage](ServSync_Pilot_Tutorial_Coverage.md). |
| Expected following slice | Complete TUT-006, then the TUT-002 standards upgrade before the pilot onboarding dry run. |
