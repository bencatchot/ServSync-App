...
| Next recommended assignment | With separate explicit approval, apply the identical reviewed legacy-retirement bytes to Production using read-only post-install catalog/security and exact preservation checks only. Do not create Production fixtures or call mutating product RPCs. |
| Assignment outcome | PR #517 merged at `6d28655ac5a81eb8fb1b243161b164e8d4d7acc3`; exact durable SHA-256 `b864e61a...fb4461` remains installed across Sandbox, Demo, and Production. Deployed Demo UI acceptance passed actual report finalization plus manual Home History with and without a private document, reload reconciliation, canonical private Documents/Home History/notification delivery, and zero duplicate/orphan outcomes. Exact retirement SHA-256 `edde0d89...ce5289` is now applied identically in Sandbox and Demo. In both, dependency-safe legacy functions remain cataloged but unexecutable by client/trusted roles, the old report-upload policy is absent, and the durable prepared-upload policy remains. The post-retirement Demo UI/API matrix again produced one canonical report lineage, two manual results, two registered objects, zero orphans, legacy RPC/upload denial, homeowner-only private downloads, exact cleanup, and unchanged 165-label/412-row Demo fingerprint `2ee2c08b73fc44101a4512871473efaf` covering Recorder/Help/Marketing/provider/publication/public/Auth/Storage state. The live backend comparator correctly fails closed on only the five retirement-stage policy/grant differences plus the separately approved Demo scenario additions; runtime parity passes. Production retirement remains pending under separate approval. See [the FB-039E application acceptance](../qa/ServSync_Core_State_Reliability_Acceptance_2026-08-20.md), [FB-039E1 acceptance](../qa/ServSync_Core_Authoring_Durable_Idempotency_Acceptance_2026-08-20.md), and [FB-039E2 acceptance](../qa/ServSync_Core_Record_Finalization_Durable_Idempotency_Acceptance_2026-08-24.md). |
...
| Roadmap return point | FB-039E2 source, durable rollout, deployed Demo observation, and identical-byte Sandbox/Demo retirement are complete. Phase 0.4 remains the control point until separately approved Production retirement passes. Do not advance to Phase 0.5 before that gate. |
