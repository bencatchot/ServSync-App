...
Planning base: `origin/main` at `e20ed5a16e7b59a216d175a35881f8ab7cffb708`
...
| Next recommended assignment | Under one fresh explicit owner authorization, run exactly one protected TUT-005 silent `homeowner-connect-service-request` recording against Ready durable Demo deployment `dpl_92JCtRbghxMX5R1BDg2unej2znFR` at exact current main commit `e20ed5a16e7b59a216d175a35881f8ab7cffb708`. The prior stopped authorization is not reusable. Production Help approval/publication and every Cedar action remain separate gates. |
| Assignment outcome | PR #563 merged the browser-proven exact expanded-detail readback, and PR #564 merged the credential-preflight incident record as main commit `e20ed5a16e7b59a216d175a35881f8ab7cffb708`. The separately approved Demo-only recovery replaced the exposed legacy keys with the unexposed publishable/secret pair, updated the Demo browser and local recorder consumers, rotated only the exact two Demo-owned primary passwords, disabled both exposed legacy keys, and redeployed current main as Ready deployment `dpl_92JCtRbghxMX5R1BDg2unej2znFR`. Fresh homeowner and contractor browser sign-ins passed after legacy deactivation, protected server reads passed, the bundle remained mode `600`, and registry/connection/Request/output residue stayed zero. No recorder was invoked and no media exists. See [Pilot Tutorial Coverage](ServSync_Pilot_Tutorial_Coverage.md). |
