...
| Next recommended assignment | Review the FB-039E application-reliability candidate, then authorize the bounded backend idempotency follow-up required for Request creation, direct Estimate/Invoice saves, finalized-report document registration, and manual Home History/document creation. |
| Assignment outcome | The application candidate keeps canonical identity/count/lifecycle data behind one stable loading treatment, preserves destination and record context across retry, suppresses raw backend errors, and prevents repeated same-page core mutations. Existing durable Draft launch, Estimate/Job-to-Invoice, Estimate-to-Job, source-to-Home-History, and payment idempotency remain proven. Full Phase 0.4 acceptance remains open because client guards cannot protect refresh, cross-tab, or lost-response retries for four older multi-write operations. See [the FB-039E acceptance record](../qa/ServSync_Core_State_Reliability_Acceptance_2026-08-20.md). |
...
| Roadmap return point | Keep FB-039E active through application-candidate review and the separately authorized backend idempotency follow-up; advance to Phase 0.5 only after exact-head acceptance proves both layers. |
