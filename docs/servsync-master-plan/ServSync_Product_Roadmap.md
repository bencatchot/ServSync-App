...
8. Treat voiceover, synchronized captions, and a durable transcript as one product-wide tutorial standard. Burned-in scene callouts do not replace captions. Human and AI narration remain provider-neutral options; any AI voice requires recorded provenance and a visible disclosure, and paid generation remains separately approval-gated.
...
| Next recommended assignment | Define and implement the provider-neutral Help narration/caption package and playback contract, then create a narrated and captioned TUT-001 replacement. Return for owner choice before using a paid AI provider or selecting a durable AI voice. |
| Assignment outcome | FB-040A is in progress in draft PR #522 on `codex/fb-040a-tutorial-readiness`. The six-workflow pilot coverage matrix, context-preserving Request-to-Estimate recorder, Help Studio allowlist, contextual placement, and freshness contracts are implemented. Production Help Studio job `e4e6d6dc-7358-44ff-ac8b-f424ece6662e` preserves a validated 16.04-second silent MP4/poster candidate at `Ready for review`, but it is not publication-eligible under the newly established voiceover + synchronized captions + transcript standard. It remains unapproved and unpublished. The existing published Estimate revision 3 remains available while its narration/caption upgrade is tracked for pilot readiness. See [the FB-040A acceptance record](../qa/ServSync_FB_040A_Tutorial_Readiness_Acceptance_2026-08-26.md). |
| Expected following slice | Create and publish the six narrated/captioned protected tutorials in evidence order, then run the first task-based pilot onboarding dry run before recruiting the 3-5-contractor cohort. |
