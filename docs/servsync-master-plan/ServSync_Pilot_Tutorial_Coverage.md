...
| TUT-005 | How to connect and request service | Homeowner | Connect with a contractor, choose the home, and submit a request with useful work details. | `homeowner.service` | `homeowner-connect-service-request` | **Primary active outcome; source and seven corrections merged, latest authorization stopped at credential preflight, not published.** PR #563 merged at `17aa4ee6e0a78eccd59c50810b24a5112a4fe080`, and exact durable Demo deployment `dpl_C4xHy37UFMxatrW8CRDBD9RB6f1s` is Ready with HTTP 200. Before recorder invocation, both primary fictional bundle sign-ins returned `invalid_credentials` even though the exact users retain required Demo ownership metadata; alternate entries authenticated but lacked that metadata and were rejected. No fixture, recording, or media resulted. A diagnostic redaction error placed Demo anon/service-role values in the private Codex transcript, requiring separately approved Demo-key rotation plus primary identity-bundle recovery before any fresh one-run authorization. Cedar, Help approval, and Production publication remain separately gated |
