...
| TUT-003 | How to complete work and save the service record | Contractor Owner, Admin, Office, Field Technician | Move an accepted Estimate into a Job, record the visit, complete the work, and finalize the report. | `contractor.work` | `contractor-complete-work` | **Source implementation in draft PR #524.** Approved process-only Demo credential use authenticated and seeded the exact accepted Estimate, then failed closed before Job creation because generic recorder presentation mode hid **Create Job**. The branch now exposes only that action behind the exact TUT-003 recorder boundary while preserving ordinary Demo/Production/Sandbox guards. Protected rerecord awaits merge and durable Demo deployment; no artifact, Cedar generation, approval, or publication has occurred |
