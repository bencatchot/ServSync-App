...
| TUT-005 | How to connect and request service | Homeowner | Connect with a contractor, choose the home, and submit a request with useful work details. | `homeowner.service` | `homeowner-connect-service-request` | **Primary active outcome; source and five corrections merged, sixth silent run materially failed, not published.** PR #559 merged at `79459e0cb228df2cff60c8968a97cc5483b241e3`. The authorized run passed protected preflight, the complete contextual connection path, corrected pending-review locator, real contractor acceptance, exact connection adoption, selection-based Request-title replacement, Request submission, and exact Request adoption at `request_ready`. It then timed out because the already-open contractor page retained its pre-Request in-memory list; a fresh read-only contractor session showed the exact persisted title, description, and Demo Bay Home. No retry or artifact occurred; guarded reset removed six disposable rows and direct audit left zero disposable residue. Browser-prove a deterministic contractor-page refresh or equivalent state reload before final retrieval, retain exact assertions, merge, and deploy before requesting another one-run authorization; Cedar credentials, Help approval, and Production publication remain separately gated |
