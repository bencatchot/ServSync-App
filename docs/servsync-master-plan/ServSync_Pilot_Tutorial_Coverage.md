...
| TUT-005 | How to connect and request service | Homeowner | Connect with a contractor, choose the home, and submit a request with useful work details. | `homeowner.service` | `homeowner-connect-service-request` | **Primary active outcome; source and four corrections merged, fifth silent run materially failed, not published.** PR #557 merged at `bdd0b9193904666c2d4e3d746883c29964f1f89e`. The authorized run passed protected preflight, the browser-proven pinned Optional-message readback, contextual request, corrected pending-review locator, real contractor acceptance, and exact connection adoption at `connected_request_ready`. It then failed before Request submission because the Request-title helper required an empty intermediate while the rendered controlled field immediately restored its 20-character generated default. No retry or artifact occurred; guarded reset removed four disposable rows and direct audit left zero disposable residue. Browser-prove a selection-based human-paced title replacement that avoids an empty intermediate, retain exact final verification, merge, and deploy before requesting another one-run authorization; Cedar credentials, Help approval, and Production publication remain separately gated |
