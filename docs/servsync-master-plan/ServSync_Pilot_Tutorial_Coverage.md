...
| TUT-005 | How to connect and request service | Homeowner | Connect with a contractor, choose the home, and submit a request with useful work details. | `homeowner.service` | `homeowner-connect-service-request` | **Primary active outcome; source and three corrections merged, fourth silent run materially failed, not published.** PR #555 merged at `2ce5eedfc90c370a4f225fde1c4f0e2f81325d6f`. The authorized run passed protected preflight and reached the real contextual connection dialog, but the page-scoped semantic correction still timed out resolving **Optional message** for exact post-entry readback after human-paced controlled input. Connection submission never occurred; no retry or artifact occurred, and guarded reset plus direct audit left zero disposable residue. Replace this unsuccessful field reference with one proven against the real dialog, retain exact verification, add browser-level regression coverage, merge, and deploy before requesting another one-run authorization; Cedar credentials, Help approval, and Production publication remain separately gated |
