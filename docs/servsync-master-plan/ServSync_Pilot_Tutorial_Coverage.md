...
| TUT-005 | How to connect and request service | Homeowner | Connect with a contractor, choose the home, and submit a request with useful work details. | `homeowner.service` | `homeowner-connect-service-request` | **Primary active outcome; source and six corrections merged, seventh silent run materially failed, not published.** PR #561 merged at `7c8ec14ce500295d007f177b8b89d2685f6554dc`. The authorized run passed protected preflight, the complete contextual connection path, pending-review discovery, real contractor acceptance, exact connection adoption, selection-based Request-title replacement, Request submission/adoption, and deterministic contractor-page reload. The contractor page found and expanded the exact Request card, but strict exact-description verification failed because the truthful summary and expanded detail both rendered the same full description. Read-only verification proved exact title/description/home/connection lineage and zero downstream records. No retry or artifact occurred; guarded reset removed six disposable rows and direct audit left zero disposable residue. Browser-prove a deterministic expanded-detail description check without weakening exact assertions, merge, and deploy before requesting another one-run authorization; Cedar credentials, Help approval, and Production publication remain separately gated |
