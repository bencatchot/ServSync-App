...
| TUT-005 | How to connect and request service | Homeowner | Connect with a contractor, choose the home, and submit a request with useful work details. | `homeowner.service` | `homeowner-connect-service-request` | **Primary active outcome; source and first locator fix merged, second silent run materially failed, not published.** PR #551 merged at `a93b63a9d50007a3dcb779590fb66a93ac983697`. The authorized run passed disconnected-homeowner connection, corrected pending-review discovery, real contractor acceptance, and exact connection adoption. Request adoption then refused the title **Plumbing help neededReplace leaking water heater**, created when the recorder appended the canonical title to the control's retained default; all other Request predicates matched. No retry or artifact occurred, and exact compensation plus guarded reset left zero disposable residue. Correct, regression-test, merge, and deploy bounded title replacement before requesting another one-run authorization; Cedar credentials, Help approval, and Production publication remain separately gated |
