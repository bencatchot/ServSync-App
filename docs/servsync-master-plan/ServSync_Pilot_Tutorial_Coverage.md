...
| TUT-003 | How to complete work and save the service record | Contractor Owner, Admin, Office, Field Technician | Move an accepted Estimate into a Job, record the visit, complete the work, and finalize the report. | `contractor.work` | `contractor-complete-work` | **Correction required after owner review.** Production request `708a66db-0c34-461f-a512-3ba0dccbc0b2` holds the 58.84-second package from exact protected Demo source `e5ff54c9032d5f272368d2d6da2087e190790d68`, but native bottom captions cover the recorded action area and the candidate must not be approved or published. The same immutable MP4, one-request Cedar audio, five scene anchors, transcript, and disclosure have been reprocessed without another provider request; a versioned compact top safe area keeps the click targets, cursor, and lower orientation callouts clear in browser playback. Next: merge the caption-safe source correction, then separately authorize returning the current job, creating the replacement request, and attaching the exact top-safe package for a fresh owner 1x/sound-off review. Nothing is approved or published |
