...
| TUT-004 | How to deliver an invoice and record an outside payment | Contractor Owner, Admin, Office | Create/deliver the Invoice, explain that payment collection is outside ServSync, and record a payment received elsewhere. | `contractor.financials` | `contractor-invoice-outside-payment` | **Protected UI flow verified; silent media still missing and not published.** Against exact durable Demo commit `067fe3f947d2e37a7aa720fbf9053e3b9fac0791`, the one authorized run preserved Customer/Home/Request/Estimate/Job/Invoice/payment lineage, delivered and opened the Invoice, recorded the fictional `$400.00` external bank transfer through the offline ledger, and verified **Partially Paid**, `$400.00` paid, and `$1,765.00` due. Promotion failed closed because the recorder mistook an inert first-party JavaScript chunk and the exact internal read-only online-payment-history RPC for provider traffic; no artifact was promoted and reset left zero registered residue. A bounded guard correction is ready for review and still rejects Stripe, checkout, payment-intent, and online-payment action paths. Remaining gates: merge/deploy that correction; separately authorize one exact-head silent rerun; then separately authorize Cedar narration/captions/transcript packaging, full `1x` review, Help approval/publication, and post-publication contextual/audience verification |
