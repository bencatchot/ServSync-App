...
| TUT-003 | How to complete work and save the service record | Contractor Owner, Admin, Office, Field Technician | Move an accepted Estimate into a Job, record the visit, complete the work, and finalize the report. | `contractor.work` | `contractor-complete-work` | **Corrected Production package ready for owner review.** Rejected request `708a66db-0c34-461f-a512-3ba0dccbc0b2` is **Needs rerecord** with the exact owner findings preserved. Replacement request `41efd0eb-8789-4419-b85b-4240272cd0ff` holds a 57.32-second 1440×900 narrated package from validated source `9dfff32a940e9a3b1b396253cf3a38eb8dd92c2f`. It reuses the immutable one-request Cedar audio without a provider call, places five native cues at 2, 8.5, 34, 40, and 45 seconds in the compact top safe area, contains no recorder callouts, preserves the accepted scope while appending the completion note, and ends **Filed to Documents**. Production review loaded the exact duration, one showing caption track, full transcript/disclosure, and completed at 1x. Next: owner 1x/sound-on and sound-off product-truth decision. Nothing is approved or published |
