...
Production currently contains two published video tutorials: TUT-001 **How to handle a homeowner service request**, revision 1, and TUT-002 **How to create an estimate**, revision 3. TUT-002 remains available because its visible workflow is current, but it predates the narration/caption standard and must receive a compliant replacement before FB-040A is complete. The table below records both published tutorials and the four missing pilot walkthroughs. A missing tutorial is not represented as available in the product until an approved revision is published.
...
| TUT-003 | How to complete work and save the service record | Contractor Owner, Admin, Office, Field Technician | Move an accepted Estimate into a Job, record the visit, complete the work, and finalize the report. | `contractor.work` | `contractor-complete-work` | **Correction required after owner review.** Production request `708a66db-0c34-461f-a512-3ba0dccbc0b2` holds the rejected 58.84-second package from exact protected Demo source `e5ff54c9032d5f272368d2d6da2087e190790d68`. Its native captions cover the action area and duplicate burned-in callouts; its Work Notes interaction visibly clears the accepted scope before typing. The source candidate moves WebVTT captions to a compact top safe area, suppresses the optional TUT-003 recorder callouts, and appends a labeled completion note while failing closed unless the original scope remains intact. The existing Cedar audio remains reusable without another provider request, but the prior MP4 is rejection evidence and cannot be attached as the replacement. Next: merge, confirm exact durable Demo deployment, rerun the protected workflow, provider-free align the existing Cedar audio, then separately authorize the Production return/replacement/attachment for fresh owner 1x/sound-off review. Nothing is approved or published |
