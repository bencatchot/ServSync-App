...
| TUT-005 | How to connect and request service | Homeowner | Connect with a contractor, choose the home, and submit a request with useful work details. | `homeowner.service` | `homeowner-connect-service-request` | **Primary active outcome; source merged, first silent run materially failed, not published.** PR #549 merged at `2667d3ec1952800faa5e6273888bd09c6227948f`. The authorized run truthfully created the disconnected-homeowner contextual request, but stopped before contractor acceptance because the recorder's stale **needs review** locator did not match the current **need review** control. The contractor RPC returned the exact request, no retry or artifact occurred, and guarded reset left zero disposable residue. Correct, regression-test, merge, and deploy the locator before requesting a new one-run authorization; Cedar credentials, Help approval, and Production publication remain separately gated |
