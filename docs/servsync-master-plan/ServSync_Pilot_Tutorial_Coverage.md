...
| TUT-005 | How to connect and request service | Homeowner | Connect with a contractor, choose the home, and submit a request with useful work details. | `homeowner.service` | `homeowner-connect-service-request` | **Primary active outcome; source and seven corrections merged, protected credential recovery complete, not published.** PR #565 merged the recovery record as current main `7a491411af636a019f1d5ec964a149db8d2d7e73`; GitHub Demo deployment `6295280468` completed from that exact commit and the canonical Demo routes returned HTTP 200. The fresh 2026-09-06 authorization was consumed by one invocation, which failed closed before fixture setup because the operator process sourced the broader local bundle and inherited `RESEND_API_KEY`. The guard ran before any Supabase client or recorder browser was created; no fixture, connection, Request, media, provider action, or retry occurred, and local/durable output remained empty. A corrected read-only preflight now passes only `DEMO_RECORDING_ENV_KEYS`, contains all required Demo inputs, and contains zero forbidden provider credentials. One new authorization is required for exactly one protected silent run through that allowlisted path. Cedar, Help approval, and Production publication remain separately gated |
