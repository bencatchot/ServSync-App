...
| TUT-005 | How to connect and request service | Homeowner | Connect with a contractor, choose the home, and submit a request with useful work details. | `homeowner.service` | `homeowner-connect-service-request` | **Primary active outcome; source and seven corrections merged, protected credential recovery complete, not published.** PR #564 merged the preflight record as current main commit `e20ed5a16e7b59a216d175a35881f8ab7cffb708`. The approved Demo-only recovery replaced and disabled the exposed legacy keys, updated Demo browser/local recorder consumers to the unexposed publishable/secret pair, and rotated only the two exact Demo-owned primary passwords without changing identities, metadata, scenario, or roles. Ready durable Demo deployment `dpl_92JCtRbghxMX5R1BDg2unej2znFR` serves that exact commit with HTTP 200; fresh post-deactivation homeowner and contractor browser sign-ins and protected server reads passed, while registry/connection/Request/output residue remained zero. No recorder was invoked and no media resulted. One fresh owner authorization is required for exactly one protected silent run; Cedar, Help approval, and Production publication remain separately gated |
