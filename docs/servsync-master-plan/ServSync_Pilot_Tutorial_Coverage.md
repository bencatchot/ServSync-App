...
| TUT-004 | How to deliver an invoice and record an outside payment | Contractor Owner, Admin, Office | Create/deliver the Invoice, explain that payment collection is outside ServSync, and record a payment received elsewhere. | `contractor.financials` | `contractor-invoice-outside-payment` | **Protected UI flow verified; corrected recorder deployed; silent media still missing and not published.** PR #547 merged at `ab542a5c08bebfb226d5d5fc5934372a16d2695d`, and the successful durable Demo deployment is bound to that exact commit. The preceding run preserved Customer/Home/Request/Estimate/Job/Invoice/payment lineage, delivered and opened the Invoice, recorded the fictional `$400.00` external bank transfer through the offline ledger, and verified **Partially Paid**, `$400.00` paid, and `$1,765.00` due, but promoted no artifact. The corrected guard allows only the inert first-party bundle and exact internal read-only payment-history RPC while continuing to reject Stripe, checkout, payment-intent, and online-payment action paths. Remaining gates: separately authorize one exact-commit silent rerun; then separately authorize Cedar narration/captions/transcript packaging, full `1x` review, Help approval/publication, and post-publication contextual/audience verification |
