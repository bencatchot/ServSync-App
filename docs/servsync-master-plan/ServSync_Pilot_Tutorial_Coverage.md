...
Production currently contains four published video tutorials: TUT-001 **How to handle a homeowner service request**, revision 1; TUT-002 **How to create an estimate**, revision 3; TUT-003 **How to complete work and save the service record**, revision 1; and TUT-004 **How to deliver an invoice and record an outside payment**, revision 1. TUT-002 remains available because its visible workflow is current, but it predates the narration/caption standard and must receive a compliant replacement before FB-040A is complete. The table below records all four published tutorials and the two missing protected workflows. A missing tutorial is not represented as available in the product until an approved revision is published.
...
| TUT-005 | How to connect and request service | Homeowner | Connect with a contractor, choose the home, and submit a request with useful work details. | `homeowner.service` | `homeowner-connect-service-request` | **Validated silent source and local Help wrapper retained; Cedar retry separately gated; not published.** PR #567 merged the silent-source closeout as current main `97f529db9783fb12c04a9b43b65b9682932bd48c`; all checks and automatic deployments passed. The 119.2-second 1440×900 source remains exact at MP4 SHA-256 `6a2cfcbdebe3a1e64883b6b23e7b9f99043f6277e1350f72875c6fba4c01dc2c` and WebM SHA-256 `efc8b9616ddf64f6ba9d8e18beef3dd6e31867507e1b944cb57dc8d31b07d6bc`. It is now locally packaged as a checksum-bound Help MP4/WebM/poster/manifest wrapper with the bounded five-part narration script and no Help mutation. One owner-authorized Cedar attempt used a new restricted key for exactly one `gpt-4o-mini-tts` request immediately after key creation; OpenAI returned HTTP 401, the runner removed narration staging, and no audio, narrated MP4, captions, transcript, spend, or successful use resulted. The secret was cleared and the exact key was separately revoked; exact-name status is **Inactive**. A fresh authorization is required for any new key or provider request, with an activation interval before the single call. Production Help request/attachment, owner review, approval, and publication remain separate gates |
