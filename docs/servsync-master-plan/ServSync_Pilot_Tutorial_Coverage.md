...
| TUT-005 | How to connect and request service | Homeowner | Connect with a contractor, choose the home, and submit a request with useful work details. | `homeowner.service` | `homeowner-connect-service-request` | **Primary active outcome; protected workflow passed, duration correction pending, not published.** PR #565 merged the recovery record as current main `7a491411af636a019f1d5ec964a149db8d2d7e73`; its durable Demo deployment is healthy. The first 2026-09-06 invocation failed closed on an external-credential guard before fixture setup, after which the corrected allowlisted preflight passed. Under a second fresh authorization, the exact main recorder completed and verified the real contextual connection, contractor acceptance, home-scoped Request, exact adoption, and contractor title/description/home readback at `request_ready` with zero checkpoint issues. Its 118.72-second silent WebM exceeded the original 110-second maximum, so the candidate was deleted and not promoted. Exact cleanup removed six disposable rows, retained the canonical home graph, and returned active registry state and local/durable TUT-005 media to zero. The bounded correction raises only this scenario's truthful human-paced ceiling to 125 seconds. Review/merge/deploy it, then obtain one new authorization for exactly one protected silent run. Cedar, Help approval, and Production publication remain separately gated |
