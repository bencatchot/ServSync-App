...
| TUT-004 | How to deliver an invoice and record an outside payment | Contractor Owner, Admin, Office | Create/deliver the Invoice, explain that payment collection is outside ServSync, and record a payment received elsewhere. | `contractor.financials` | `contractor-invoice-outside-payment` | **Recorder source merged and automatically deployed; not published.** PR #542 merged at `c0a27b92ed977109c59c8960364a08cfe1626c8e`; Production app, durable Demo, and Stripe Sandbox deployments passed. The protected Demo-only scenario uses fictional data and ordinary UI from one completed Job through exact Invoice creation/delivery, fictional homeowner view, and a `$400.00` external bank-transfer ledger entry. It preserves Customer/Home/Request/Estimate/Job/Invoice/payment lineage and ends **Partially Paid** with `$1,765.00` due. Remaining gates: explicit protected credential access; exact-head recording; one Cedar narration package with synchronized captions, durable transcript, exact disclosure, and silent provenance; full `1x` sound-on/sound-off review; explicit owner Help approval and Production publication; post-publication contextual/audience verification |
