...
- FB-007 Trade Pack boundary: Trade Pack Domain Contracts v1, Property Asset Bridge v1, and Durable Trade Section Instances v1 are hidden Sandbox-only foundations within the active backlog, not completed user-facing Trade Pack milestones. The section foundation preserves exact definition/asset snapshots, canonical Draft/Estimate/Job and claim lineage, strict values, append-only revisions, concurrency, and capability downgrade behavior. The current client remains on Production/Demo-compatible data paths and calls no Sandbox-only Trade Pack, asset-bridge, or section RPC. Coordinated bridge/client adoption, reviewed HVAC content, assignment-scoped Field Technician access, visible workflows, customer projections, and Production/Demo rollout remain active work.
