...
| FB-003B Slice 1 | Request-Free Local Invoice Delivery repository foundation | Implemented in repository source on 2026-08-03 as an invoice-only, document/property-bound secure Copy Link workflow for eligible unclaimed local customers. The source uses 256-bit one-time bearer tokens, stores only SHA-256 hashes, exposes token-free management history, issues the first link through the immutable sent lifecycle, and provides a customer-safe unauthenticated HTML view outside the authenticated app shell and analytics. | This bounded source milestone is not applied, deployed, or rollout-approved. Sandbox SQL/security and effective gateway abuse-control gates remain mandatory. FB-003B stays active for invoice PDFs, immutable/versioned estimate delivery, finalized reports, claimed-account transition, email/SMS/reminders, and approval/payment/reply actions. |
