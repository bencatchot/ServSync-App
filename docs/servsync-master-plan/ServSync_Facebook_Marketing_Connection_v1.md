...
The owner then completed explicit Page selection successfully. Production now reports Page `ServSync` as connected with readiness `ready_except_live_post_verification`; the durable Page token remains Vault-backed and server-only. Database `publishing_enabled` remains false, `SERVSYNC_FACEBOOK_PUBLIC_POSTS_ENABLED` remains absent/false, and no publication, publication event, or Facebook feed POST has been created. Owner Visual Publication Preview v1 uses this non-posting connection state to show destination-specific previews while preserving both public-post gates.
