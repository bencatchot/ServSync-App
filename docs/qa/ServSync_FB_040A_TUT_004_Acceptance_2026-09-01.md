...
Published and verified in Production as revision 1. Help Studio request `29766df4-5080-4b9a-9025-c871bed06e17` preserved the exact durable Demo source at commit `ab542a5c08bebfb226d5d5fc5934372a16d2695d`; one separately authorized `gpt-4o-mini-tts` Cedar request produced the scene-synchronized derivative with top-safe English WebVTT, matching transcript, exact AI disclosure, and complete `1x` sound-on/sound-off review. Production playback, `contractor.financials` Owner retrieval, immutable Owner/Admin/Office audience, and homeowner exclusion now pass.
...
- With separate owner authorization, **Publish for Help** created immutable Production revision 1 and increased the published inventory from three to four with no new media objects. Help Studio contains exactly one published TUT-004 entry and one originating Approved request; no duplicate active or published TUT-004 exists.
- Published Preview loaded the exact 57.96-second 1440×900 media at ready state 4 and completed 57.96/57.96 seconds unmuted at `1x` with one English caption track showing. The five exact steps, full transcript, and **AI-generated voiceover using OpenAI's Cedar voice.** rendered; browser diagnostics were empty.
- The approved Production Owner smoke identity resolved exactly one revision-1 TUT-004 result at `contractor.financials`; its caption RPC returned English captions, the exact script hash `ae3efce556544fd799769ff24ff92dd42eea96f4463d39961789adff386957ff`, and the exact disclosure. The approved homeowner smoke identity could not resolve contractor Financials Help. The immutable revision includes exactly Owner/Admin/Office and excludes Field Technician, Viewer, and Homeowner; Production has no separate Admin/Office smoke identities, so their equivalent allow result is established by the deployed session-derived role resolver and exact immutable audience, matching the established TUT-001 acceptance method. Across 29 read-only application/Auth/Help requests, server `5xx` count and payment/provider traffic count were zero.

## Tutorial freshness

Tutorial impact: `UPDATED`.

Affected tutorial: TUT-004 **How to deliver an invoice and record an outside payment**. Production revision 1 is published and verified with the exact validated narrated/captioned package, `contractor.financials` placement, Owner retrieval, exact Owner/Admin/Office immutable audience, homeowner exclusion, full playback, captions, transcript, disclosure, browser health, and zero provider/payment effects.

## Owner gate

Minimum owner action when ready: approve merge of documentation-only PR #548 after its exact refreshed head, diff, reviews, mergeability, and required checks are reverified. TUT-005 is the next tutorial assignment after canonical closeout merges.
