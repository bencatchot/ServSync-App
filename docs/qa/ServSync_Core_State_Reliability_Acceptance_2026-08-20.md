...
| Job report finalization | The browser preserves one semantic operation key, prepares one server-canonical Job report identity, reuses the deterministic private upload, and reconciles a succeeded receipt before another upload. | FB-039E2 source adds one Job-scoped receipt/lock, canonical conflict detection, verified Storage manifest, and atomic Job/document/Home History/notification success. | Source-complete durable idempotency; protected rollout and exact-head acceptance pending. |
| Manual Home History plus optional document | The browser preserves one semantic operation key and uses one prepare/upload/commit helper for document and no-document variants. | FB-039E2 source adds primary-owner-only receipts, deterministic optional Storage identity, metadata verification, and atomic document/History success. | Source-complete durable idempotency; protected rollout and exact-head acceptance pending. |
...
Full Phase 0.4 acceptance still requires explicit approval for exact-byte Sandbox -> Demo -> Production rollout, shared-environment preservation, exact-head Preview, and the outstanding shared-home member/viewer fixture run. The database-ahead migration temporarily retains the legacy six-argument finalizer/upload bypass; the separately staged retirement migration is included in source and may run only after the new application deploys and is observed. Client hiding or button disabling is not treated as backend evidence.
...
The application layer merged through PR #509 at `4a2fb59`, and FB-039E1 remains active across Sandbox, Demo, and Production. FB-039E2 source now closes the remaining implementation gaps for report/Home History/document finalization without broadening authority. FB-039E Phase 0.4 must not be marked fully complete until both exact FB-039E2 migrations are rolled out in their staged order and validated, the application head passes Preview against that backend, and the remaining shared-home member/viewer acceptance passes. Existing authority was not weakened to manufacture evidence.
