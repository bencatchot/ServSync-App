...
- [ ] FB-016 daily role/fixture smoke implementation has passed local read-only API and desktop/mobile browser validation; require the first natural `05:23 UTC` GitHub Actions run before marking the recurring milestone complete. Retain only sanitized JSON evidence, never authenticated traces/screenshots/network captures.
