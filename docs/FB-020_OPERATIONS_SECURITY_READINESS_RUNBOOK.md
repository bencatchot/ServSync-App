...
- `servsync-durable-draft-preview-rollout-mode.sql` adds the private `durable_draft_preview_all_contractors_enabled` runtime setting and rollout-mode-aware entitlement helpers. The setting defaults `false`/`cohort`; approved Sandbox Preview validation may explicitly set it to `all_contractors`, while Production requires a separate release gate.
