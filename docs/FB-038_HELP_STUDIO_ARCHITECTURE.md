...
Production media maintenance completed on 2026-08-22 after the contractor navigation changed from **Jobs** to **Work**. The same protected `contractor-create-estimate` scenario produced a validated 45.12-second 1440x900 package from current source commit `ae610867b80138974e6c8b36de2c5e7ccf8ecb5a`. After owner review and explicit approval, Help Studio published walkthrough revision 3 from media asset `d8da7746-c271-4b34-9d6d-6b82b5d42c6b`; its MP4 SHA-256 is `f9a0a6d6498aceae1c9355d12d5590293d4bce3c0dc1c858521c63b601983ad7` and poster SHA-256 is `9135f291ae9e12e795b915977d00c37b56601541fc8b6e36cfc36f74a93d602c`. Revisions 1 and 2 remain immutable history, and post-publication Help Studio Preview resolves revision 3.

## Tutorial Freshness Verification

Every pull request records one tutorial-impact status: `NOT APPLICABLE`, `NONE`, `UPDATE REQUIRED`, or `UPDATED`. A user-facing feature change requires a Help Studio search by feature area, route context, screen/workflow terms, and synonyms, followed by Preview comparison of every matching published walkthrough against the proposed or current Demo behavior. The evidence must cover visible navigation, labels, controls, workflow order, role assumptions, and outcome rather than relying only on the walkthrough's source commit.

The pull-request quality check rejects a missing or unsupported declaration and requires a named tutorial plus bounded follow-up for `UPDATE REQUIRED`. Semantic staleness remains a reviewed judgment because a file diff cannot reliably determine whether recorded pixels or spoken instructions are obsolete. If a replacement must wait for the merged Demo build, the existing immutable revision remains served, the tutorial update stays open in the completion report, and Production publication remains separately approval-gated. `UPDATED` is appropriate only after the replacement revision has been reviewed, published, and verified through the applicable Help surface.
