...
- `tests/e2e/admin-contractor-prospects.spec.ts`: nine passing browser tests covering 1440px/390px public profiles and Discover, no contact links/actions, admin create/save/issue/edit/revoke, recipient review/acceptance, wrong-recipient denial, anonymous auth entry, unavailable-backend compatibility, claimed-directory continuity, and nontechnical unavailable-claim messaging. Components use mocked RPC transport; PostgreSQL tests independently exercise the real SQL. These are not claims of hosted or shared-environment acceptance.
...
Draft PR: [#574](https://github.com/bencatchot/ServSync-App/pull/574). Normal source Preview: [ServSync feature branch](https://serv-sync-app-refresh-git-codex-adm-fb4282-bencatchots-projects.vercel.app). The first source head passed PR quality and all three normal Vercel builds. A protected Preview smoke using the existing project-specific automation credential returned HTTP 200, rendered the landing page, and reported zero page errors. The Sandbox project's distinct Preview protection was not opened with this credential; no protection settings were changed. The new workflow still requires separately approved Sandbox SQL and authenticated hosted acceptance.
