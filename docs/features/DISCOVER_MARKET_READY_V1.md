...
RISKS: The core live Sandbox referral journey passed; human pilot and hosted/account-verification checks remain open; Sandbox shortlist acceptance and tutorial freshness are complete.
NEXT STEP: Run the controlled human pilot and hosted/account-verification checks; confirm supply and response quality before advertising. The app merge and all three migrations are complete; no advertising or external outreach was performed.
...
## Live Sandbox referral journey — September 25

Owner requested the next validation step after installation. The merged application source passed the complete existing-account referral path at 390px against live Sandbox, without mocked backend responses: anonymous contractor profile → real UI password sign-in → same profile with no automatic writes → property setup → Add property → save profile/property → return link to exact contractor → choose property/contact/overview/address permissions → explicit connection request → pending status. The homeowner RPC rejected a service request before acceptance. The actual contractor session accepted through `servsync_respond_to_connection_request`; the homeowner reloaded, opened Request service, and explicitly submitted. Exactly one service request persisted with the intended contractor, property, title and description, and the contractor retrieved that ID through the canonical reader. Reload preserved the one request; no horizontal overflow or uncaught browser errors occurred. Final mobile screenshot was visually inspected.

All disposable accounts/records were removed using their exact Auth IDs, and all 169 initial relation fingerprints matched after cleanup. Two earlier harness selector assumptions (onboarding form instead of the Properties workspace, then a datalist rather than its input) were corrected; both runs cleaned up fully. No application repair was needed. Reusable operator command: `node scripts/validation/validate-discover-live-journey-sandbox.mjs --execute-sandbox`. It refuses migration application and uses only the fixed Sandbox project.

Limits: this was local frontend source against the real Sandbox backend. Disposable accounts were precreated/confirmed through the Auth admin API, followed by real UI sign-in; signup email delivery was not exercised. Contractor acceptance was a real authorized RPC, not a contractor-browser interaction. This proves the technical referral/service-request path, not cold-user usability, inquiry response times, or a completed paid-advertising pilot. Hosted release smoke, account-verification experience, participating supply, and human pilot metrics remain appropriate launch checks. Production/Demo were not modified during this test. Tutorial impact NONE: tests/evidence only, no changed screen or workflow requiring another recording.
