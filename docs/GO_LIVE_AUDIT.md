...
- FB-020 non-production restore planning led to the 2026-08-13 FB-016 Production-backup drill. Full PostgreSQL/Auth restore, approved password authentication, sanitized integrity checks, and local application smoke passed in an isolated target. Public-launch recovery remains blocked because Storage object bytes were absent, no independent Storage-byte backup exists, August 9-11 backup points remain unexplained, PITR is disabled, and external configuration must be rebuilt separately.
...
Current recovery evidence supersedes the historical drill summary below: the 2026-08-13 FB-016 drill in `docs/FB-016_RECOVERY_RUNBOOK.md` restored database/Auth state into an isolated target in 3m57s, proved approved authentication and local application smoke, and kept Production read-only. It remains `BLOCKED` overall because Storage bytes had no independent backup and the August 9-11 backup gap remains unexplained. FB-016 now owns public-launch recovery readiness.
...
Controlled private beta baseline status: met for the current private beta stage. FB-016 owns public-launch recovery readiness. Full database/Auth restore is proven, but Storage-byte recovery, backup-gap resolution/PITR posture, external configuration recovery, optional production role/record smoke, public go-live, and paid-subscription readiness remain separate gates.
