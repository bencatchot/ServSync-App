...
- FB-020 non-production restore planning led to the 2026-08-13 FB-016 Production-backup drill. Full PostgreSQL/Auth restore, approved password authentication, sanitized integrity checks, and local application smoke passed. The follow-up R2 milestone independently backed up and hash-restored all four Production Storage objects into a second isolated target. Supabase ticket `SU-445711` resolved the visible August 9-11 restore-point absence as a provider visibility incident and confirmed the underlying data remained safe and recoverable. Public-launch recovery remains blocked on natural daily scheduler evidence and a timed full-system drill; formal provider RPO and hidden-point monitoring remain bounded follow-ups.
...
Current recovery evidence supersedes the historical drill summary below: the 2026-08-13 FB-016 drills in `docs/FB-016_RECOVERY_RUNBOOK.md` restored database/Auth state, independently recovered all Storage bytes from private R2, proved approved authentication and application/reference access paths, and kept Production read-only. Supabase ticket `SU-445711` closed the prior August 9-11 recoverability concern as a restore-point visibility incident. Recovery remains `BLOCKED` overall because successful natural scheduling and a timed secure configuration/application recovery remain unproven. FB-016 owns public-launch recovery readiness.
...
Controlled private beta baseline status: met for the current private beta stage. FB-016 owns public-launch recovery readiness. Database/Auth and independent Storage-byte restore are proven bounded milestones; backup-gap resolution/PITR posture, scheduler observation, external configuration recovery, optional production role/record smoke, public go-live, and paid-subscription readiness remain separate gates.
