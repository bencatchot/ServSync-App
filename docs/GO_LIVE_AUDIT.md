...
- FB-020 non-production restore planning led to the 2026-08-13 FB-016 layer drills and the 2026-08-14 full closeout. Natural Vercel Cron execution matched independent R2 evidence, and a timed isolated drill restored PostgreSQL/Auth plus all four R2-backed Storage objects, passed authenticated desktop/mobile ServSync smoke, a real Invoice PDF, recovered media, private Storage authorization, and cleanup in 27m03.393s. Supabase ticket `SU-445711` resolved the visible August 9-11 restore-point absence as a provider visibility incident and confirmed the underlying data remained safe and recoverable. Public-launch recovery closeout is no longer blocked on scheduler/full-drill evidence; formal provider RPO wording, hidden-point monitoring, recurring drills, and broader launch operations remain bounded follow-ups.
...
- FB-016 recurring role/fixture smoke passed its first genuine `schedule` event on 2026-08-15 in GitHub Actions run `31867784065` at default-branch SHA `4ce5b579f9fe08825e275754783d9e5fb9454214`. The Sandbox role/fixture/cross-tenant/browser/aggregate-R2-health matrix, Demo approved read-only navigation, and Production public plus dedicated Owner/homeowner read-only navigation all passed. Four sanitized JSON files were retained without authenticated traces, screenshots, network captures, emails, private identifiers, or credentials. This completes the bounded first-natural-run milestone, not long-term scheduler reliability or public go-live readiness.
...
Current recovery evidence supersedes the historical drill summary below: the 2026-08-13/14 FB-016 evidence in `docs/FB-016_RECOVERY_RUNBOOK.md` restores database/Auth state, independently recovers all Storage bytes from private R2, proves natural scheduling, approved authentication, application/PDF/media access, private authorization, and full cleanup. Supabase ticket `SU-445711` closed the prior August 9-11 recoverability concern as a restore-point visibility incident. The bounded recovery closeout is `PASS WITH FINDINGS`; FB-016 remains active for recurring evidence and broader public-launch readiness.

Historical 2026-07-02 restore evidence: the approved Sandbox database-only drill restored public schema into an isolated target, then stopped before Auth/data restore under that drill's authorization boundary. The target and local artifacts were deleted. Current evidence is the 2026-08-13 FB-016 drill above.

Controlled private beta baseline status: met for the current private beta stage. FB-016 owns public-launch recovery readiness. Database/Auth/R2 recovery, one natural Storage scheduler run, one natural recurring role/fixture smoke, and one timed full isolated application recovery are proven bounded milestones; provider RPO/monitoring policy, repeated scheduled evidence, external-configuration automation, PITR posture, public go-live, and paid-subscription readiness remain separate gates.
