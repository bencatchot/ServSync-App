...
- FB-016 recurring role/fixture smoke is implemented for daily GitHub Actions execution at `05:23 UTC`: full role/fixture/cross-tenant coverage remains Sandbox-only, Demo uses approved read-only identities, and Production retains public plus dedicated Owner/homeowner read-only scope. R2 health is aggregate/read-only and cannot trigger backup execution. Local API and browser evidence pass; public-launch evidence must still record the first natural scheduled run before this bounded milestone is complete.
...
Controlled private beta baseline status: met for the current private beta stage. FB-016 owns public-launch recovery readiness. Database/Auth/R2 recovery, one natural scheduler run, and one timed full isolated application recovery are proven bounded milestones; provider RPO/monitoring policy, recurring evidence, external-configuration automation, optional role/fixture smoke, PITR posture, public go-live, and paid-subscription readiness remain separate gates.
