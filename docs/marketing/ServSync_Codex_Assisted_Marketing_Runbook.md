...
FB-037G-C adds an ordinary in-product publishing path after preparation: an authorized internal or contractor user explicitly selects an immutable package, opens its exact Preview, approves it, then separately chooses Publish Now or Schedule. Those actions occur in ServSync, not through this Codex-assisted preparation operator. Approval alone never publishes. Global provider-submission controls remain platform-admin operations and are not exposed to contractors. See [Marketing Publishing Queue + Authorization v1](../servsync-master-plan/ServSync_Marketing_Publishing_Queue_Authorization_v1.md).

Narrated Marketing media remains a separate review boundary. A narrated upload must use the private `marketing-assets` bucket, preserve the silent master provenance, match one exact Content revision, include the reviewed public AI-voice disclosure in that Content body, and record media approval separately from text approval. A status-only Content approval may carry the already-approved exact pairing to the new revision; any copy edit requires a fresh pairing decision. Neither a managed asset nor an approved pairing authorizes publication. The Facebook managed-video adapter can execute only after an exact Publish Now/Schedule authorization and all provider-submission controls permit a new request.
