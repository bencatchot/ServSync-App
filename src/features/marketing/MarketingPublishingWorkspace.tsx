import { hasCaptionVerificationIssue, publicationReviewUrl } from './publicationOutcome';
...
export function publicationStatusLabel(publication: MarketingPublication) {
  if (hasCaptionVerificationIssue(publication)) return 'Uploaded to Facebook — caption needs review';
  ...
  return (
    <section data-testid="marketing-publishing-workspace" className="space-y-5 rounded-lg border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
      ...
      <section aria-labelledby="active-heading"><h3 id="active-heading" className="text-sm font-bold text-slate-950">Scheduled / Publishing / Needs Attention</h3>{activePublications.length === 0 ? <p className="mt-3 border-y border-dashed border-slate-200 py-7 text-center text-sm text-slate-500">No active publications.</p> : <div className="mt-3 divide-y divide-slate-200 border-y border-slate-200">{activePublications.map(publication => {
        ...
        const reviewUrl = publicationReviewUrl(publication);
        const captionIssue = hasCaptionVerificationIssue(publication);
        const replacementPrepared = state?.packages.some(item => item.id === publication.packageId && item.status === 'ready') === true;
        return <article key={publication.id} aria-live="polite" className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center"><QueueThumbnail asset={asset} /><div className="min-w-0 flex-1"><p className="text-sm font-bold text-slate-950">{publication.snapshot.title}</p><p className="mt-1 text-xs text-slate-500">{publication.destinationLabel} · {publicationStatusLabel(publication)}</p>{captionIssue ? <p className="mt-2 text-sm text-amber-800">Facebook returned the uploaded video, but its caption did not match the approved text. Review the post on Facebook before making another publishing decision.</p> : publication.failureMessage && <p className="mt-2 text-sm text-rose-700">{publication.failureMessage}</p>}{reviewUrl && <a href={reviewUrl} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex min-h-10 items-center gap-1 text-sm font-semibold text-blue-700"><ExternalLink size={14} />Review on Facebook</a>}{publication.status === 'failed' && !publication.retryEligible && !publication.replacementEligible && !replacementPrepared && <p className="mt-2 text-xs text-slate-600">This result cannot be retried safely. Review it before taking another publishing action.</p>}{publication.status === 'failed' && publication.replacementEligible && <p className="mt-2 text-xs text-slate-600">Facebook was not contacted. You can prepare the exact approved post for a new publishing decision.</p>}{publication.status === 'failed' && replacementPrepared && <p className="mt-2 text-xs text-slate-600">The exact approved package is Ready again. Open its preview above to make a new publishing decision.</p>}</div><div className="flex flex-wrap items-end gap-2">{publication.status === 'scheduled' && publication.mode === 'scheduled' && <><ScheduleEditor publication={publication} busy={saving} onReschedule={(scheduledAt, timezone) => props.onReschedule(publication, scheduledAt, timezone)} /><button type="button" aria-label={`Cancel ${publication.snapshot.title}`} disabled={saving} onClick={() => void props.onCancel(publication)} className="flex h-10 w-10 items-center justify-center rounded-md border border-slate-300"><X size={16} /></button></>}{publication.status === 'failed' && publication.retryEligible && <button type="button" disabled={saving} onClick={() => void props.onRetry(publication)} className="inline-flex min-h-10 items-center gap-2 rounded-md border border-slate-300 px-3 text-sm font-bold"><RotateCcw size={15} />Retry</button>}{publication.status === 'failed' && publication.replacementEligible && <button type="button" disabled={saving} onClick={async () => { setActionError(null); try { await props.onPrepareReplacement(publication); setActionNotice(`The exact approved package for "${publication.snapshot.title}" is Ready again. Publishing still requires Publish Now or Schedule.`); } catch (cause) { setActionError(cause instanceof Error ? cause.message : 'ServSync could not prepare a replacement.'); } }} className="inline-flex min-h-10 items-center gap-2 rounded-md border border-blue-300 px-3 text-sm font-bold text-blue-700"><RotateCcw size={15} />Prepare replacement</button>}</div></article>;
      })}</div>}</section>
