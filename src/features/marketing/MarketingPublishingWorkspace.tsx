...
import { CalendarClock, ChevronLeft, ChevronRight, Globe2, Link2, Loader2, PencilLine, RefreshCw, RotateCcw, ShieldCheck, Unplug, XCircle } from 'lucide-react';
...
export function MarketingPublicationComposer({
  ...
  const publishingEnabled = connectionReady && facebook.capabilities.publishingEnabled;
      ...
      <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(13rem,0.65fr)_minmax(0,1.35fr)]">
        ...
        <div className="min-w-0 space-y-4">
          ...
          <article data-testid="facebook-public-preview" className="mx-auto w-full max-w-[34rem] overflow-hidden rounded-md border border-slate-300 bg-white shadow-sm">
            ...
            <div className="border-t border-slate-200 px-4 py-3 text-xs font-semibold text-slate-500">Media: {preview.mediaLabel}</div>
          </article>
...
export function MarketingPublishingWorkspace({ state, contentItems, selectedContentId, loading, error, saving, onSelectContent, onReturnForRevision, onCreate, onReload, onCancel, onRetry, onConnectFacebook, onSelectFacebookPage, onRecheckFacebook, onDisconnectFacebook }: {
  ...
  return <section data-testid="marketing-publishing-workspace" className="space-y-4">
    ...
    <MarketingPublicationComposer contentItems={contentItems} selectedContentId={selectedContentId} facebook={facebook} busy={saving} onSelectContent={onSelectContent} onReturnForRevision={onReturnForRevision} onCreate={onCreate} />
