...
TEST_ROOT="$(mktemp -d "/tmp/servsync-marketing-content.XXXXXX")"
PGDATA="$TEST_ROOT/data"
PGSOCKET="$TEST_ROOT/socket"
PGPORT="${SERVSYNC_MARKETING_CONTENT_TEST_PORT:-55447}"
...
select md5(concat_ws('|',
  (select count(*) from public.marketing_workspaces),
...
if psql_run --file "$ROOT_DIR/servsync-internal-marketing-content-approval.sql" >/dev/null 2>&1; then
  echo "Repeated internal Marketing migration unexpectedly succeeded." >&2
  exit 1
fi
...
if [[ "$before_fingerprint" != "$after_fingerprint" ]]; then
  echo "Rejected repeated migration changed the installed foundation." >&2
  exit 1
fi
...
if "$PSQL_BIN" "$PREFLIGHT_URL" --set=ON_ERROR_STOP=1 --file "$ROOT_DIR/servsync-internal-marketing-content-approval.sql" >/dev/null 2>&1; then
  echo "Missing-prerequisite internal Marketing migration unexpectedly succeeded." >&2
  exit 1
fi
...
if [[ "$preflight_residue" != "0" ]]; then
  echo "Failed preflight left internal Marketing relation residue." >&2
  exit 1
fi

echo "Internal Marketing content approval validation passed."
