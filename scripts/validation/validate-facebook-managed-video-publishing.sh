...
TEST_ROOT="$(mktemp -d "/tmp/servsync-facebook-managed-video.XXXXXX")"
PGDATA="$TEST_ROOT/data"
PGSOCKET="$TEST_ROOT/socket"
PGPORT="${SERVSYNC_FACEBOOK_MANAGED_VIDEO_TEST_PORT:-55477}"
...
create schema storage authorization postgres; create schema vault authorization postgres;
...
create function vault.create_secret(new_secret text,new_name text default null,new_description text default null,new_key_id uuid default null) returns uuid language plpgsql security definer set search_path=pg_catalog,vault as $$ declare v_id uuid; begin insert into vault.secrets(secret,name,description,key_id) values(new_secret,new_name,new_description,new_key_id) returning id into v_id; return v_id; end; $$;
SQL
...
for migration in \
  ...
  servsync-facebook-managed-video-publishing-adapter.sql; do
  psql_run --file "$ROOT_DIR/$migration" >/dev/null
done

psql_run --file "$ROOT_DIR/tests/sql/facebook-managed-video-publishing-validation.sql" >/dev/null

if psql_run --file "$ROOT_DIR/servsync-facebook-managed-video-publishing-adapter.sql" >/dev/null 2>&1; then
  echo "Repeated Facebook managed-video publishing migration unexpectedly succeeded." >&2
  exit 1
fi

echo "Facebook managed-video publishing adapter validation passed."
