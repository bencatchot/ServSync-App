...
TMP="$(mktemp -d "/tmp/servsync-price-book-cost.XXXXXX")"
PGDATA="$TMP/data"
PGSOCKET="$TMP/socket"
PGPORT="${PGPORT:-55473}"
...
run --file "$ROOT/servsync-price-book-cost-margin-foundation.sql" >/dev/null
run --file "$ROOT/tests/sql/price-book-cost-margin-validation.sql" >/dev/null

if run --file "$ROOT/servsync-price-book-cost-margin-foundation.sql" >"$TMP/reapply.log" 2>&1; then
  echo 'Expected repeated cost migration application to fail closed.' >&2
  exit 1
fi
if ! grep -q 'already installed' "$TMP/reapply.log"; then
  echo 'Repeated cost migration failed for an unexpected reason.' >&2
  exit 1
fi

echo 'Price Book cost and margin PostgreSQL validation passed.'
