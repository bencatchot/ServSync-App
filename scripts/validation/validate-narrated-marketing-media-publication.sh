...
TEST_ROOT="$(mktemp -d "/tmp/servsync-narrated-marketing.XXXXXX")"
PGDATA="$TEST_ROOT/data"
PGSOCKET="$TEST_ROOT/socket"
PGPORT="${SERVSYNC_NARRATED_MARKETING_TEST_PORT:-55476}"
...
for migration in \
  ...
  servsync-narrated-marketing-media-publication.sql; do
  psql_run --file "$ROOT_DIR/$migration" >/dev/null
done

psql_run --file "$ROOT_DIR/tests/sql/narrated-marketing-media-publication-validation.sql" >/dev/null

if psql_run --file "$ROOT_DIR/servsync-narrated-marketing-media-publication.sql" >/dev/null 2>&1; then
  echo "Repeated narrated Marketing media migration unexpectedly succeeded." >&2
  exit 1
fi

echo "Narrated Marketing media and publication snapshot validation passed."
