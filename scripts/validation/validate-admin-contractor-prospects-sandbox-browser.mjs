...
export async function verifyBrowser({ admin, other, homeowner, ref, marker, registerProspect, results }) {
  ...
  try {
    ...
    await adminPage.getByRole('button', { name: 'Contractors', exact: true }).click({ timeout: 30000 });
    const section = adminPage.getByRole('region', { name: 'Prospect contractor profiles' });
    await section.getByRole('button', { name: 'Create prospect profile' }).click();
    const slug = `${marker}-browser`;
    await section.getByRole('textbox', { name: 'Profile address', exact: true }).fill(slug);
    ...
    const claimPath = new URL(await linkField.inputValue()).hash;
