...
export function createJournalWriter(root, { journalAuthSecret = null, allowPrepared = false, provenanceMode = null } = {}) {
  ...
  const append = (input) => {
    ...
    if (nextSequence > BROWSER_LIMITS.journal_records) throw new EvidenceError('BROWSER_RECORD_LIMIT', 'Browser journal record limit exceeded.');
    const recordInput = { ...input, sequence: nextSequence, provenance_mode: mode };
