...
} from './approved-direction-package-contract.mjs';
...
async function readInput(path) {
  ...
  try {
    ...
    fail('Approved-Direction Marketing package is not valid JSON.');
  }
}

async function main() {
  const inputPath = process.argv[2];
  if (!inputPath) fail('Usage: ingest-approved-direction-marketing-package.mjs <package.json|->');
  ...
  try {
    ...
    const [{ data: source, error: sourceError }, { data: recentContent, error: contentError }] = await Promise.all([
      client.rpc('servsync_get_internal_marketing_directions'),
      client.rpc('servsync_list_internal_marketing_content', { p_status: 'all' }),
    ]);
    if (sourceError || !source) fail('Approved Marketing Directions could not be loaded.');
    if (contentError || !Array.isArray(recentContent)) fail('Recent Marketing content could not be loaded.');

    const marketingPackage = validateApprovedDirectionPackage(input, contracts, source, recentContent);

    const { data: receipt, error: ingestionError } = await client.rpc('servsync_ingest_internal_marketing_direction_package', {
      p_preparation_request_id: marketingPackage.preparation_request_id,
      p_source_plan_id: marketingPackage.source_plan_id,
      p_expected_plan_revision: marketingPackage.source_plan_revision,
      p_truth_pack_version: marketingPackage.truth_pack_version,
      p_contract_key: marketingPackage.preparation_contract_key,
      p_items: marketingPackage.items,
    });
    if (ingestionError) fail(`Approved-Direction Marketing ingestion failed (${ingestionError.code || 'server_error'}).`);
    if (
      !receipt
      || receipt.status !== 'draft'
      || receipt.strategic_source !== 'approved_direction'
      || receipt.generator_source !== 'codex_assisted'
      || receipt.item_count !== marketingPackage.items.length
      || !Array.isArray(receipt.content_ids)
      || !Array.isArray(receipt.direction_ids)
    ) {
      fail('Approved-Direction Marketing ingestion returned an invalid receipt.');
    }
    ...
    const draftsById = new Map(drafts.map(item => [item.content_id, item]));
    const prepared = receipt.content_ids.map(contentId => draftsById.get(contentId));
    if (
      ...
      || prepared.some((item, index) => (
        ...
        || item.source_plan_id !== marketingPackage.source_plan_id
        || item.source_plan_revision !== marketingPackage.source_plan_revision
        || item.source_direction_id !== marketingPackage.items[index].direction_id
        || item.source_direction_revision !== marketingPackage.items[index].direction_revision
        || item.content_role !== marketingPackage.items[index].content_role
      ))
    ) {
      fail('Prepared approved-Direction drafts failed durable lineage verification.');
    }
...
main().catch(error => {
  process.stderr.write(`${error instanceof Error ? error.message : 'Approved-Direction Marketing ingestion failed.'}\n`);
  process.exitCode = 1;
});
