...
For every pull request, record the tutorial freshness result in the PR body. For user-facing changes, finish implementation, in-scope repairs, and application/Preview validation before beginning the tutorial freshness check or tutorial updates. Do not interleave Help Studio searches or walkthrough playback with ongoing implementation. Then search Help Studio by affected feature, route context, screen, and workflow and Preview any matching published walkthrough against the finished changes. If subsequent revisions affect guidance, finish and validate those revisions before repeating the affected tutorial check. Use exactly `NOT APPLICABLE`, `NONE`, `UPDATE REQUIRED`, or `UPDATED`. `UPDATE REQUIRED` must name the affected tutorial and bounded follow-up, and the task must not be reported complete until the replacement revision is published and verified.
